...
#----------------------------------------------------------------------
    ...
    # kajla: this probably does not do what you expect it to do. ~/.ncbirc seems to be regarded and this is ineffective.
    if( $envBlastpMat && ! -e $envBlastpMat ) { confess("blast matrix '$envBlastpMat' missing");} else { $ENV{'BLASTMAT'} = $envBlastpMat; }
    if( $envBlastpDb && ! -e $envBlastpDb ) { confess("blast db '$envBlastpDb' missing"); } else { $ENV{'BLASTDB'} = $envBlastpDb; }
                                # ------------------------------
                                # run BLASTP
                                # ------------------------------
    ...
    # kajla: -B is 'Number of concatenated queries, for blastn and tblastn'. I think -b: 'Number of database sequence to show alignments for' is meant.
    my $command_base = qq|$exeBlastp -p blastp -d "$ENV{BLASTDB}/$parBlastpDb" -b $nhits|;
#    $command="$niceLoc $exeBlastp $parBlastpDb $fileInLoc B=$nhits > $fileOutLoc";
    $command = qq|$niceLoc $command_base -i "$fileInLoc" -o "$fileOutLoc"|;
    $msg="--- $sbr '$command'\n";
#warn $msg;
    ($Lok,$msgSys)=
	&cachedExtCall( $command, $fhTraceLoc, { md5string => "$command_base -o --seq=".$inseq->seq(), md5files => [], cache_files => { 'blres' => $fileOutLoc }, enable => $predictPP::docache } );
...
sub blastpsiRun {
    local($niceLoc,$exeBlastPsi,$exeBlast2Saf,
	  ...
	  $fileOutMat,$fileTraceBlast,$fileBlastMatTmb,$fhTraceLoc,
          $envBlastMat )=@_;
...
#     
    ...
    ###
                                ...
                                # ------------------------------
                                # set env
    if( $envBlastMat && ! -e $envBlastMat ) { confess("blast matrix '$envBlastMat' missing");} else { $ENV{'BLASTMAT'} = $envBlastMat; }
                                # ------------------------------
    ...
    $command=qq|$niceLoc $command_base -i "$fileInLoc" -o "$fileOutTmp" -C "$fileOutCheck"  |;
    # GY - added 3/18/2004
    ...
    ($Lok,$msgSys)=
	&cachedExtCall( $command, $fhTraceLoc, { md5string => "$command_base -o -C".( $fileBlastMatTmb ? ' -Q' : '' )." --seq=".$inseq->seq(), md5files => [], cache_files => {
                'blres' => $fileOutTmp, 'checkpoint' => $fileOutCheck, ( $fileBlastMatTmb ? ( 'matrix' => $fileBlastMatTmb ) : () ) }, enable => $predictPP::docache } );
#        &sysSystem( $command, $fhTraceLoc );
    if(! $Lok){
        return(0,"*** ERROR $sbr '$Lok'\n".$msg."\n".$msgSys); }
    if(! -e $fileOutTmp ){
    ...
    $msg="--- $sbr '$command'\n";
    ($Lok,$msgSys)=
	&cachedExtCall( $command, $fhTraceLoc, { md5string => "$command_base -o -Q --seq=".$inseq->seq(), md5files => [ $fileOutCheck ], cache_files => {
                'blres' => $fileOutLoc, 'matrix' => $fileOutMat }, enable => $predictPP::docache } );
#        &sysSystem( $command, $fhTraceLoc );
    return(0,"*** ERROR $sbr '$Lok'\n".$msg."\n".$msgSys)
	if (! $Lok);
    return(0,"*** ERROR $sbr no output '$fileOutLoc'\n"."$msg")
...
#-------------------------------------------------------------------------------
	...
	cluck "*** ERROR lib-col:open_file: Cannot open file '$file_name' " ;
	if ( $log_file ) {
...
sub prodomRun {
    local($fileInLoc,$fileOutTmpLoc,$fileOutLoc,$fhErrSbr,$niceLoc,
	  $exeBlastp,$envBlastDb,$envBlastMat,$parBlastDb,$parBlastN,$parBlastE,$parBlastP)=@_;
...
#-------------------------------------------------------------------------------
    ...
    $exeBlastp=   "/usr/pub/bin/molbio/blastp"             if (! defined $exeBlastp);
    ...
    return(0,"*** $sbrName: no in file '$fileInLoc'!")    if (! -e $fileInLoc &&
				...
				# set env
    if( $envBlastMat && ! -e $envBlastMat ) { confess("blast matrix '$envBlastMat' missing");} else { $ENV{'BLASTMAT'} = $envBlastMat; }
    if( $envBlastDb && ! -e $envBlastDb ) { confess("blast db '$envBlastDb' missing"); } else { $ENV{'BLASTDB'} = $envBlastDb; }
				# ------------------------------
				# security erase
    ...
    if( !$inseq ) { confess("failed to read FASTA sequence from ``$fileInLoc''"); }
				...
				# run BLAST
    $dbTmp = $parBlastDb; $dbTmp =~ s/\/$//o;

    # kajla: -B is 'Number of concatenated queries, for blastn and tblastn'. But we do not run blastn, we run blastp... ???
    my $command_base = qq|$exeBlastp -p blastp -d "$dbTmp" -B "$parBlastN"|;
#    $cmd=  "$niceLoc $exeBlastp $dbTmp $fileInLoc E=$parBlastE B=$parBlastN >> $fileOutTmpLoc ";
#    $cmd=  "$exeBlastp -p blastp -d $dbTmp -i $fileInLoc -E $parBlastE -B $parBlastN >> $fileOutTmpLoc ";
    $cmd = qq|$command_base -i "$fileInLoc" -o "$fileOutTmpLoc"|;

    print $fhErrSbr "--- $sbrName: system \t $cmd\n";
    &cachedExtCall( $cmd, undef, { md5string => "$command_base -o --seq=".$inseq->seq(), md5files => [], cache_files => { 'blres' => $fileOutTmpLoc }, enable => $predictPP::docache } );
    #system("$cmd");
				# ------------------------------
				# read BLAST header
    ($Lok,$msg,%head)=
...
#       in:                     $cmd:   will do system($cmd) or system( @$cmd ) if ARRAY ref
...
#-------------------------------------------------------------------------------
    ...
    if ($fhLoc)
    {
	print $fhLoc "---  system: \t ", ( ref( $cmdLoc ) eq 'ARRAY' ? join(' ', @$cmdLoc ) : $cmdLoc ), "\n";
	#cluck( "---  system: \t $cmdLoc" );
    }

				# ------------------------------
				# run system
    $Lsystem = ( ref( $cmdLoc ) eq 'ARRAY' ? system( @$cmdLoc ) : system( $cmdLoc ) );
...
sub cachedExtCall {
    ...
    my($sbrName,$Lok);
    %$__p = ( md5string => '', md5files => [], cache_files => {}, enable => 1, %$__p );
    $[ =1 ;
#-------------------------------------------------------------------------------
#   cachedExtCall             read cached external call results if any, otherwise run
#                               external call, store results
#       in:                     $cmdLoc: will do system($cmd) or system( @$cmd ) if ARRAY ref
#       in:                     $fhLoc:  will write trace onto fhLoc
#                                 =<! defined> -> STDOUT
#                                 =0           -> no output
#       __p => {
...
#       err:                    (1,'ok'), (0,'message')
#-------------------------------------------------------------------------------
    if( !$__p->{enable} ) { return sysSystem( $cmdLoc, $fhLoc ); } # no caching, original behaviour
    ...
    if ($fhLoc)
    {
	print $fhLoc "---  cachedExtCall: \t ", ( ref( $cmdLoc ) eq 'ARRAY' ? join(' ', @$cmdLoc ) : $cmdLoc ), "\n";
	#cluck( "---  cachedExtCall: \t $cmdLoc" );
    }
    ...
    my $extcmdmd5 = Digest::MD5->new();
    $extcmdmd5->add( $__p->{md5string} );
    foreach my $md5file ( @{$__p->{md5files}} ) { my $fh; open( $fh, '<', $md5file ) || confess( $! ); $extcmdmd5->addfile( $fh ); close( $fh ); }
...
#warn(Data::Dumper::Dumper( $__p->{cache_files} ));
    ...
    {
        $cache_dir = "$ENV{PP_ROOT}/extrescache";
        if( -d $cache_dir )
        {
            $cmd_cache_dir = $cache_dir.'/'.$extcmdmd5->hexdigest();
            if( -d $cmd_cache_dir )
            {
                $rerun = 0;
...
#warn("*** zipping: $path $cachefilepath");
    ...
    else
    {
        ...
        utime undef, undef, $cmd_cache_dir; # extend cache /dir/ lifetime too
    ...
    if( -d $cache_dir ) {
        ...
        remove_old_files({ root => $cache_dir, mmin => '+300' }); # 5 hours
    }

    if( $fhLoc ){ print $fhLoc "\n"; }
    return(1,$Lsystem);
}				# end of cachedExtCall
    ...
    eval {
        ...
        if( !-e $lockfile || time() - File::stat::stat($lockfile)->mtime() > 3600 )
        {
            ...
            if( flock( $lh, LOCK_EX | LOCK_NB ) )
            {
                ...
                my @cmd = ( '/usr/bin/find', $__p->{root}, '-depth', '-mindepth', '1', '-mmin', $__p->{mmin}, '-exec', '/bin/rm', '-rf', '{}', ';' );
