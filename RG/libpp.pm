...
# kajla: this is slow, so candidate to caching. I just have to find out the inputs and outputs.
sub copfLocal {
...
#    $cmd.=" fileOutScreen=".$file_copfScreen." fileOutTrace=".$file_copfTrace." dbg";
    ...
    print $fhSbr2 "--- $sbrName2 system \t",$cmd,"\n" if ($LdebugLoc);
				...
				# system call
    system("$cmd") && confess("In '".Cwd::cwd()."' '$cmd' failed: $!, ".($? >> 8));
				# ------------------------------
				# conversion failed!
    if (! -e $fileOutLoc) {
	$Lok=0;
...
#-------------------------------------------------------------------------------
    ...
    #system("echo '$messageLoc' | $exe_mailLoc -s PP_ERROR $userLoc"); #disabled on Altix
    return(1,"ok $sbrName");
