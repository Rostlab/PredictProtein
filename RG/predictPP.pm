...
INIT: {
    ...
    $Lok=
	require "$env_pack";
    $predictPP::fileNameResStr = "";	# This package global holds the randomized string that is used for the pub file name
				# ------------------------------
...
#===============================================================================
sub validate_env_and_run {
  # kajla: I would like to have a new command line interface to this library in addition to predictPP.pl, because I would like to use long options, but I do not feel like modifying predictPP.pl. In order to avoid duplicating code, all non-command-line-parameter-checking code is moved here from predictPP.pl. This routine can then be shared by predictPP.pl and the new interface.
  my %__p = @_;
  %__p = ( 
    fileTo_pred   => undef,
    file_result   => undef,
  ...
  &ctrlAbort("*** $scrName failed to require env_pack ($env_pack)"."\n*** err=2", \%__p ) 
      if (! $Lok);
  				...
  				# ------------------------------
  foreach $des ("dir_work","file_errLog","file_predMgrLog",
  	...
  	&ctrlAbort(Carp::longmess("*** $scrName failed to get envPP{$des} for ARCH '$envPP::ARCH' from env_pack ($env_pack)\n".
  		   "*** err=3"), \%__p );}}
  				# ------------------------------
  				# include phd prediction package
  # kajla: Someone made an included library a variable name. What does this achieve?
  # It certainly makes regular built-in library checking (for presence and syntax) delayed until runtime - making debugging less efficient. It also invalidates regular expectations about the order of processing as compilation and then running occurs.
  # Benefits? The location of the library is determined dynamically as it is ultimately initialized from environment variables. However $PERL5LIB achieves the same, only in the clean documented way.
  # All in all requiring libraries like this only makes code harder to read, more error-prone and harder to debug.
  foreach $libKwd ("lib_ctime"){
      $Lok=0;
      $Lok=require $envPP{"$libKwd"};
  				# ******************************
      if (!$Lok){			# *** error in require
  
  	&ctrlAbort(Carp::longmess("*** $scrName failed to get envPP{$libKwd} for ARCH '$envPP::ARCH' from env_pack ($env_pack)\n".
  		   "*** err=4"), \%__p );}}
  				# get date
  if (defined &ctime){@Date= split(' ',&ctime(time));
  		    shift (@Date); $Date = join(':',@Date);}
  				...
  				# ------------------------------
  if ($__p{silence})			# redirect STDOUT and STDERR
                                        # kajla: a bad idea. Silence should control the level of verbosity of messages, but STDERR and STDOUT should be left alone, because those ARE very necessary in case things do not go as expected, and you would like to find out what is going on. There always are /some/ messages that you want to see.
  {
  #    open (STDOUT, "> $filePred_out");
  #    open (STDERR, "> $filePred_err");
  #    printf "\n=====> START PREDICTION: PID %-s at %s" , $$,`date`;
  #    print  "--- predict (predPackPP.pl) ".
  #	"args: '$__p{fileTo_pred},$__p{file_result},$__p{submitter},$__p{password},$__p{origin},$__p{debug}'\n";
  ...
  #    $filePred_err=$dir_work . "/" . "err" . "_" . $$;
  	...
  	"args: '$__p{fileTo_pred},$__p{file_result},$__p{submitter},$__p{password},$__p{origin},$__p{debug},$__p{dbid},$__p{keep_inputfile}'";
  ...
  # kajla: why do we need this?
  #$hname= `hostname`;open (FO, ">$__p{fileTo_pred}.$hname");close FO;
...
#	"rost\@columbia.edu" if (! defined $envPP{"pp_admin"});
    ...
    $Date=localtime(time) if (! defined $Date);
				...
				# compose message
    $header=   "*** $scrName: $Date"; 
    $message=  "$header" . "\n" . "$message, fileIn=$__p->{fileTo_pred} res=$__p->{file_result}\n";
				# ------------------------------
...
#===============================================================================
sub predict {
    # these all are globals
    ($File_name,$File_result,$User_name,$Password,$Origin,$DBid, $Debug, $predictPP::Keep_inputfile, $predictPP::MailMessage, $predictPP::docache ) = @_;
...
#                               # note: global names envPP{} if you want to keep next line
	...
	$Lok=0;
	$Lok=require($envPP{$kwd}) if (-e $envPP{$kwd}); # kajla: this essentially is an include, equivalent to eval `cat $envPP{$kwd}`
...
#		     $envPP{"exeConvertSeq"},$envPP{"exeHsspFilterFor"},$envPP{"exeCopf"},
    ...
    &ctrlDbgMsg("end of (6g) runPredCC: $packName",$fhTrace,$Debug); 
				...
				# Run CHOP
				# -----------------------------------------------
# TODO chop is disabled for now, will enable once done fixing it.
...
#  if ($job{"run"} =~/[\,]isis[_only]\,/i )   # NOTE the token isis needs to be seperated by spaces, otherwise the system will confuse it with disis
    ...
    &ctrlDbgMsg("end of (6k) runLocTar: $packName",$fhTrace,$Debug);
                               ...
                               # -----------------------------------------------
                                # Run Predict Localization Tree - Rajesh Nair
                                # -----------------------------------------------

    if ($job{"run"} =~/loctree/i) {
        ($Lok,$err,$msg)=
            &runLocTree(
                         $filePID,$fhOut,$fhTrace,
                         $envPP{"dir_work"},$filePredTmp,$fileHtmlTmp,$fileHtmlToc,
                         $Debug,$job{"run"},$job{"out"},
                         $envPP{"exeLocTree"},
                         $envPP{"dirLocTree"},
        ...
        if (!$Lok){
            print $fhTrace "*** ERROR in runLocTree\n*** err=$err\n*** msg=$msg\n" if ($Debug);
            print "*** ERROR in runLocTar\n*** err=$err\n*** msg=$msg\n";

            # Add error message to result file
	    $msg = "--- Attempt to run LOCTree failed. Please contact the service administrator\n";
	    $command = "echo $msg>>";
	    if ($job{"out"}=~/ret html/){ $command .= $fileHtmlTmp;}
	    else{ $command .= $filePredTmp;}			
	    ($Lok,$msgSys)=&sysSystem("$command");
            ...
            if (0){
                ($Lok,$msg2)= &ctrlAbortPred($err,"abort ".$msg);
	        if ($Debug && ! $Lok){ print $fhTrace "$msg2\n"; }
                                # **************************************************
                return(0,"$msg");} # <<<<< ****** this is bad end
                                # **************************************************
        }
    }
    &ctrlDbgMsg("end of (6k) runLocTree: $packName",$fhTrace,$Debug);
...
#		       $envPP{"exeTopitsWrtOwn"},
    ...
    if ( length($strXMLContents)>0 ||  length($errMsg)>0)
    {
        ...
        else
        {
            ...
            $XMLLINT_ERRNO = do_xmllint( { schema => "$envPP::dppXsd", instance => $strXMLContents, debug => $Debug } );
        }
...
#    	if(! defined $tmpRes) { warn "-*- setXMLResults failed.System message:$msg"; }
        # kajla: For now we just hard-code here to save the XML file next to the result file.
        # This however (whether to save or store in a database) could be controlled by some configuration value (from e.g. environment, command line parameter).
        #
        if( $predictPP::File_result )
        {
          eval {
            my %dir_basename = getResPubDir( $filePID, $predictPP::File_result );
          
            storeXML(
              %dir_basename,
              xml => $strXMLContents,
              XMLLINT_ERRNO => $XMLLINT_ERRNO,
              convert_error => $errMsg,
              debug => $Debug
            );
          };
        }
...
#-------------------------------------------------------------------------------
	...
	&modFin($Origin,$Date,$niceRun,$filePID,$fhTrace,$envPP{"dir_work"},
		$filePredTmp,$fileHtmlTmp,$fileHtmlToc,
		$Debug,$job{"run"},$job{"out"},$filePredFin,$Password, $predictPP::Keep_inputfile );
...
#		  "envFastaLibs",
	...
	if (! defined $envPP{$kwd}){
	    	$msg.=  __FILE__.':'.__LINE__."*** initPackEnv ERROR no def of kwd=$kwd, (envPP) ";
 	     	cluck( "$msg" );
		#$Lerr=1;
	}
				# skip
	...
	if (! -e $envPP{$kwd} && ! -l $envPP{$kwd}){
	    $msg.=("\n*** initPackEnv ERROR missing ".$envPP{$kwd}.", ($kwd) ".__FILE__.':'.__LINE__);$Lerr=1;}}
				# ------------------------------
				# directories
				# ------------------------------
...
#"dirBigTremblSplit",
		  ...
		  "dirIsis","dirDisis","dirNLProt", "dirProfTmb",  "dirLocTar", "dirLocTree", "dirPfam",  "dirAgape","dirR4S", "dirConBlast"){
	next if (length($kwd)<1);
...
#		  "exeTopitsWrtOwn",
		  ...
		  "exeMview", "exeProfCon", "exePcc", "exeChop", "exeChopper",
                  "exeIsis", "exeDisis", "exeProfTmb", "exeLocTar", "exeLocTree","exeAgape","exeConBlast"
#,"exeNLProt", "exeR4S",
#"exe_svcSubmitter",
		  ){
	next if (length($kwd)<1);
...
#	$envPP{$kwd}=         &envPP'getLocal("$kwd");    #e.e'
    ...
    foreach $kwd (
		  ...
		  "chop","chopper","isis", "disis", "proftmb", "loctar", "loctree",  "agape","conblast", "NLProt",  "conseq",  "consurf"
		  ){
	$kwd2="version_".$kwd;
    ...
    foreach $kwd ("fileHelpText","fileHelpConcise","fileNewsText","fileLicText",
		  ...
		  "fileAppLocTar",    "fileAppLocTree",         "fileAppPfam",         "fileAppAgape",
...
#	$envPP{$kwd}=         &envPP'getLocal("$kwd");    #e.e'
    ...
    $niceRun=" " if (length($niceRun)<1);
				...
				# ----------------------------------------
## TODO (GY)  PP exits here if there are missing files or folders. that needs to be re organised since some missing non critical files or folders are causeing the whole pipeline to crahs
## further more some of those files are being removed by some "mysterious" programs so we need to get rid of those remove commands
...
#    $filePID=     $envPP{"dir_work"}.$filePID; # set in working dir
    ...
    {
	$fhTrace=     "TRACE_FILE";
	$Lok=open($fhTrace, ">&", \*STDOUT ) ||
	    warn "-*- $packName'iniPredict cannot open fhTrace=$fhTrace: $!\n";


    # kajla: redirecting STDERR and STDOUT is a bad idea. There always are /some/ messages that you want to see. Moreover what about the STDERR and STDOUT outside this block? Now you will have to look at the file you redirect STDERR to, the file you redirect STDERR to in another section and the STDERR before this redirection... You will want to look at STDERR when things go wrong. At those times you will probably not want the added mess of STDERR files this code is about to create.
#	$fhTrace="STDOUT";	# xx
	$fhOut=$fhTrace;
...
#    $job{"run"}.=",snap"       if ($job{"out"}=~ /snap/);
    ...
    $job{"run"}.=",loctar"       if ($job{"out"}=~ /loctar/);
				...
				# Localization Tree 
    $job{"run"}.=",loctree"       if ($job{"out"}=~ /loctree/);
...
#	  $parFastaNhits,$parFastaThresh,$parFastaScore,$parFastaSort,
	  ...
	  $blastMaxAli,$exeBlastRdbExtr4pp,
				# blast
	  $exeBlastp,$exeBlastFilter,$envBlastMat,$envBlastDb,$parBlastNhits,$parBlastDb,
				# maxhom
...
#     return(0,$errTxt,$msg."fileMaxMetrAll!")  if (! defined $fileMaxMetrAll);
    ...
    return(0,$errTxt,$msg."exeBlastp!")        if (! defined $exeBlastp);
...
# 	return(0,$errTxt,$msg."$file!")       if (! -e $file && ! -l $file); }
    ...
    foreach $exe ($exeCopf,$exeConvertSeq,$exeProsite,$exeBlastp,$exeBlastFilter,
...
#	    } 	       
	...
	$msgHere.="\n--- $sbr \t run BLASTP ($dirData,$dirSwiss,$exeBlastp,$exeBlastFilter,".
	    "$envBlastMat,$envBlastDb,$parBlastNhits,$parBlastDb,".
		$file{"seqBlast"}.",".$file{"aliBlastp"}.",".$file{"aliBlastpFil"}.",$fhErrSbr)\n";
	
	($Lok,$msg)=
	    &blastpRun($nice,$dirData,$dirSwiss,$exeBlastp,$exeBlastFilter,
		       $envBlastMat,$envBlastDb,$parBlastNhits,$parBlastDb,
		       $file{"seqBlast"},$file{"aliBlastp"},$file{"aliBlastpFil"},$fhErrSbr);
				# none found
	...
	($Lok,$err,$msg)=
	    &runProdom($file{"seqFasta"},$file{"prodomTmp"},$file{"prodom"},$file{"prodomHtml"},
		       $filePredTmp,$fileHtmlTmp,$fileHtmlToc,$optOut,
		       $exeBlastp,$envProdomBlastDb,$envBlastMat,
		       $parProdomBlastDb,$parProdomBlastN,$parProdomBlastE,$parProdomBlastP,
...
#	    return(0,"err=423",$msgErrIntern."\n".$msgHere);
	...
	if ($optRun=~/maxhom/o && $pdbidFound && length($pdbidFound)>=4 && $optRun !~ /nors/o ){
...
sub modFin {
    ...
    local($origin,$date,$nice,$fileJobId,$fhErrSbr,$dirWork,
	  $filePredTmp,$fileHtmlTmp,$fileHtmlToc,$Ldebug,$optRun,$optOut,$filePred,$password);

         ($origin,$date,$nice,$fileJobId,$fhErrSbr,$dirWork,
	  $filePredTmp,$fileHtmlTmp,$fileHtmlToc,$Ldebug,$optRun,$optOut,$filePred,$password,
          $__keep_inputfile )=@_;
...
#    $msgErr=    $msgErr{":"};
	...
	push(@kwdRm,"disulfind");
				...
				# set env DISULFINDDIR (required for the program)
                                # kajla: well, sure, but this code sets DISULFIND... so I don't quite undertand... 
	if (! defined $ENV{"DISULFIND"} ) { $ENV{"DISULFIND"} = $dirDisulfind; }


	$disulfindOutputFormat = "ascii";
	$disulfindOutputFormat = "html" 	if ($optOut=~/ret html/);

	$command="$nice $exeDisulfind -p".
...
#-------------------------------------------------------------------------------
	...
	$msgHere="--- ".__FILE__.':'.__LINE__." $sbr system ".Cwd::getcwd()." '$command'";
	($Lok,$msgSys)=&sysSystem( $command );

        if (! $Lok || ! -e $file{"nls"} || -s $file{"nlsTrace"} || ! -e $file{"nlsSum"} )
        {
...
#    return(0,$errTxt,$msg."!")       if (! defined $);
    ...
    if (! -e $fileSeq && ! -l $fileSeq)
    {
        # kajla: the problem with this kind of error reporting is that in enigmatic cases multiple errors occur and so the original error that leads to the cascade is overwritten and does not get reported. Then you can no clue of what is going on, because the reported error does not fit into the picture. Another excellent example of life made harder through programming.
        confess("$errTxt ${msg}$fileSeq!");
        return(0,$errTxt,$msg."$fileSeq!"); 
    }
    if (! -e $filePrositeData && ! -l $filePrositeData)
    {
        # kajla: the problem with this kind of error reporting is that in enigmatic cases multiple errors occur and so the original error that leads to the cascade is overwritten and does not get reported. Then you can no clue of what is going on, because the reported error does not fit into the picture. Another excellent example of life made harder through programming.
        confess("$errTxt ${msg}$filePrositeData!");
        return(0,$errTxt,$msg."$filePrositeData!");
    }
...
#----------------------------------------------------------------------
	...
	($Lok,$txt)=
	    &sysMvfile($fileTmp,$envPP{"dir_bup_res"}," ");}
				...
				# --------------------------------------------------
    if ($Lerror && ! $Debug && !$predictPP::Keep_inputfile )	# if error move to error directory
    {
	$dir=$envPP{"dir_err"};
	...
	if (0)
#	if (1)
        {			# yy still commented out in one day??? yyDO_ONE_DAY
	...
	if ( $predictPP::fileNameResStr ) {   # first try to assign the string that was extracted from hash line
	    ($Lok,$randomString) = (1, $predictPP::fileNameResStr);
...
#	$name=$tmp;
#	if( $Lok && ($fileJobId !~ /predict_s/o ) ){ $name = "${randomString}.$envPP::envL{extFileStore}"; }
				# add '.html' to name
	

#        my %dir_basename = getResPubDir( $fileJobId, $predictPP::File_result );
#        $dir = $dir_basename{dir};
#        $name = "$dir_basename{filename_suffix}";

	$predictPP::fileStore = $predictPP::File_result;	# name of file to store for user

        if( $fileFinal =~ /"/o || $fileStore =~ /"/o ) { confess("fileFinal(=$fileFinal) or fileStore(=$fileStore) is invalid because it contains `\"' and this code is susceptible to quote-trickery (among others)"); }

        # kajla: attention, $fileFinal is in $PP_WORK
        my $cmd = qq|mv "$fileFinal" "$fileStore"|;
	($Lok,$msgSys)=&sysSystem( $cmd );

                if( $predictPP::MailMessage )
                {
                  # kajla: surely we never want to overwrite the results file. There are so many possible file names, let's just give another to the mail message we generate.

        $file{"fileResultMail"} = "${File_result}.mail";

	open($fhoutLoc, ">", $file{"fileResultMail"} ) || warn qq|*** predictPP:$sbr: failed open new=$file{"fileResultMail"}\n|;
	$userTmp=$User_name;
...
#	    $urlRes = "http://www.predictprotein.org/get_results.php?req_id=".$randomString;
	    $urlRes = "http://www.predictprotein.org/get_results.php?req_id=".( $resId || 'n.a.' );
	    $urlRes .= "\n\nNEW (beta mode): visual results can now be accessed through: \nhttp://www.predictprotein.org/get_visual_results.php?res_id=".( $resId || 'n.a.' );
	    $urlRes .= "\n\nNEW: XML structured data of the results can now be accessed through: \nhttp://www.predictprotein.org/visualpp-cgi-bin/predictprotein_xml_source?res_id=".( $resId || 'n.a.' );
	}
...
# 	           " days!!!\nPlease save the file if you are going to keep it!\n";
	...
	$command = qq|cat "$envPP{fileAppGoogleGroups}" >> "$file{fileResultMail}"|;
		$msgHere="--- ".__FILE__.':'.__LINE__." $sbr system ' $command'";
		($Lok,$msgSys)=&sysSystem("$command");
	...
	if ( $errMsgLoc =~ /cafasp/ ) {	unlink $fileFinal; }
    }
...
#	&sysSystem("echo '$message' | $exe_mail -s \' $mailSubject\' $User_name",0);	    
    # kajla: I think we should not remove the results when the process ends - so NO push(@kwdRm,"fileResult");
	...
	close(LOGFILE);}
				# --------------------------------------------------
				# kajla: store certain files in $envPP::dppTrace
				# Of course these files are not created just like that. In `pp` mode (manual mode) we know that they are NOT created.
                                # Now I take a deep breath and remove any redirection of STDERR and STDOUT to files. This makes the code below obsolete.
				# --------------------------------------------------
# OBSOLETE
#    if( ! -e "$envPP::dppTrace" ) { mkdir( "$envPP::dppTrace", 0775 ); }
#    if( -e $file_error || -e $file_outpp )
#    {
#        preserve_err_out({ 
#            jobid => $jobid,
#            preserve => [
#                ( -e $file_error ? $file_error : () ), # ERRpp*
#                ( -e $file_outpp ? $file_outpp : () )  # SCRpp*
#            ],
#            root => "$envPP::dppTrace",
#            mtime => '+7'
#        });
#    }
	...
	foreach $kwd (sort{$a cmp $b }@kwdRm)
        {
	    if(
              ! defined $kwd ||
	      ( ( $Origin !~ /^mail|^html|^testPP/i ) && 
	      ...
	      ! defined $file{$kwd} ||
	      ! -e $file{$kwd} ||
#	      $file{$kwd} eq $File_result || 
              $file{$kwd} eq $file_sv_pred
            ) { next; }

	    $msgHere.="$sbr unlink 1=".$file{$kwd}."\n";

#warn( join( ', ', $File_result, $kwd, $file{$kwd} ) );
...
#    }
    ...
    if( $File_name =~ /"/o ) { warn("input file name unsafe, contains \""); }
    else
    {
      if ( !$predictPP::Keep_inputfile && ! $Debug && -e $File_name){ ($Lok,$msgSys) = &sysSystem(qq|rm "$File_name"|); }
    }
...
sub               preserve_err_out
{
    ...
    #}
    eval {
        %$__p = ( jobid => undef, preserve => [], root => "$envPP::dppTrace", mtime => '+7', %$__p );
    
        remove_old_err_out_files({ root => $__p->{root}, name => 'log_*.tgz', mtime => $__p->{mtime} });

        my $logtracefile = "$envPP::dppTrace/log_$__p->{jobid}.tgz";
        if( @{$__p->{preserve}} )
        {
...
sub               remove_old_err_out_files
{
    ...
    %$__p = ( root => "$envPP::dppTrace", name => 'log_*.tgz', mtime => '+7', %$__p );
...
#     }
   ...
   if ($lineLoc =~ /nlprot_only/){	# Only NLProt, ignore others
    ...
    $optRun ="loctar_only,prof"        if ($lineLoc=~/(run|pred\S*|do)\s+loctar_only/i);
                             ...
                             # normal Loc Tree
    $optRun.=",loctree"          if ($lineLoc=~/(run|pred\S*|do)\s+loctree/i);
    $optRun=~s/,loctar//          if ($lineLoc=~/(no)\s*loctar/i);
				# loctar only
    $optRun ="loctree_only"        if ($lineLoc=~/(run|pred\S*|do)\s+loctree_only/i);
...
#				    $lineLoc=~/prof\S?.*[ -]htm|prof\S?.*membrane|profhtm/);
 ...
 &extrHdrExpertLoctree($lineLoc)
	if ($lineLoc=~/exp(ert)? loctree|loctree exp(ert)?/i &&
	    $lineLoc=~/(orgType|protId)\s*=/i);
...
#===============================================================================
sub extrHdrExpertLoctree {
    local ($lineLoc2) = @_ ;
    local ($tmp);
    $[ =1 ;
#--------------------------------------------------------------------------------
#   extrHdrExpertLoctree         extracts parameters for LocTree
#       in GLOBAL:              $optRun
#       out GLOBAL:             $optRun
#       in:                     $txt    =
#       out GLOBAL:             $optRun
...
#    $lineLoc2=~tr/[A-Z]/[a-z]/;	# make case independent!
    ...
    $build=~s/,$//g;		# final comma
				...
				# close brackets for syntax
    if (length($build)>=1) {
	$optRun.=",parLoctree(".$build."),";
	$optRun=~s/\s//g; }
}				# end of extrHdrExpertLoctree
...
#	    "$file{\"seq4profcon\"} $file{\"hsspPsiFil\"} $file{\"profRdb\"} > $file{\"profcon\"} ";
	...
	$fileErrTmp  = $dirProfCon."/profconerr.log";
...
#    $msgErr=    $msgErr{":"};
	...
	push(@kwdRm,"pcc");
				...
				# set env PCC dir (d for the program)
	if (! defined $ENV{"PCC"} ) {
	    $ENV{"PCC"} = $dirPcc;
	}
...
#   return(0,$errTxt,$msg."exeConvertSeq!")   if (! defined $exeConvertSeq);
    ...
    return(0,$errTxt,$msg."exeProfTmb ($exeProfTmb)!")        if (! defined $exeProfTmb);
#   return(0,$errTxt,$msg."fileAliPhdIn")     if (! defined $fileAliPhdIn);
    ...
    foreach $exe ($exeProfTmb) {
        if (! -e $exe && ! -l $exe) {         return(0,$errTxt,$msg."$exe!"); }
    }
...
#	    "$file{\"seq4ProfTmb\"} $file{\"hsspPsiFil\"} $file{\"profRdb\"} > $file{\"ProfTmb\"} ";
	    ...
	    if (! $Lok || ! -e $file{"ProfTmb"}) { return (0,"err=560","*** ERROR $sbr\n"."$msgHere\n"."$Lok\n"); }
#	}
				# -------------------------------------------------
				# check result, 
				# -------------------------------------------------
...
#	$file{"seq4ProfBval"}=    $dirWork. $fileJobId.  ".fastaSingle";
	...
	$file{"seq4ProfBval"} = $file{"seqFasta"};
				...
				# -------------------------------------------------
    if ($optRun =~/profbval/) {
	$target_name=$fileJobId;
	    if ($optRun=~/window\=([0-9]+)/ ) {
		$parProfBvalWindow = $1;}
	    if ($optSeq =~/description=(\w+)/){   # this will try to set user defined name for the profbval files
		$target_name = uc $1;}
	    $mode = 0;
...
#	    print "$mode\n";
	    ...
	    push(@kwdRm,"ProfBval");
	    $dirProfBval = $ENV{PP_PROFBVAL} || $PP_ROOT."/pub/profbval";
...
#	$file{"seq4NORSnet"}=    $dirWork. $fileJobId.  ".fastaSingle";
    ...
    if ($optRun =~/norsnet/) {
		$target_name=$fileJobId;
	    if ($optSeq =~/description=(\w+)/){ # this will try to set user defined name for the norsnet temp files
		$target_name = uc $1;}
...
#warn( Data::Dumper::Dumper( \@_ ) );
    ...
    my $tinfh = undef;
    {
        ...
        if( @filenames )
        {
	    if( ! -e "$envPP::dppHome/tmp" ) { mkdir( "$envPP::dppHome/tmp", 0775 ); }
            $tinfh = File::Temp->new( TEMPLATE => "pptin_${job_file_name}_XXXXXXXX", DIR => "$envPP::dppHome/tmp",
                UNLINK => 1,
                SUFFIX => '.tar' ) || cluck( $! );
            if( $tinfh )
            {
...
#===============================================================================
# GY ADDED 5-2009: Localization Tree
#===============================================================================
sub runLocTree {
    local($fileJobId,$fhOutSbr,$fhErrSbr,
	    $dirWork,$filePredTmp,$fileHtmlTmp,$fileHtmlToc,$Ldebug,
	  $optRun, $optOut,
	  $exeLocTree,$dirLocTree,
          $origin )=@_;
...
#-------------------------------------------------------------------------------
#   runLocTree                 : predict Localization Target
#       in:                     many
#       in GLOBAL:              $msgErr{""}, $file{""}, @kwdRm, $envPP{"file*"}
#       in GLOBAL:              $file{"seqFasta"}     seq in FASTA format (from modInterpret)
#       err:                    ok=(1,'ok','blabla'), err=(0,errNumber,'msg')
#-------------------------------------------------------------------------------
#    require  $ENV{PP_LIB}."/lib-pp.pm";
# require  $ENV{PP_LIB}."/lib-col.pm";
# require  $ENV{PP_LIB}."/lib-pp.pm";
    ...
    return(0,$errTxt,$msg."exeLocTree!")        if (! defined $exeLocTree);
    ...
    $errTxt="err=544"; $msg="*** $sbr: no exe =";
    foreach $exe ($exeLocTree) {
	return(0,$errTxt,$msg."$exe!")        if (! -e $exe && ! -l $exe); }
    $file{"seq"}  = "$dirWork/$fileJobId.fasta";
...
# ---------------------------------------------
# predict Loc Tree can only take single sequence
# in case input is list, get the first fasta file
# ---------------------------------------------
    if ( $optSeq =~ /saf|msf/) {
	$file{"seq4loctree"}=    $dirWork. $fileJobId.  ".fastaSingle";
	push(@kwdRm,"seq4loctree");
	...
	($Lok,$msg)=&fastaWrt($file{"seq4loctree"},$id,$seq);
	return(0,"err=552",$msgErrConvert."\n"."*** fastaWrt\n".$msg."*** id=$id, seq=$seq\n")
	    if (! $Lok);
	return(0,"err=553",$msgErrConvert."\n"."*** fastaWrt\n"."*** id=$id, seq=$seq\n".
	       "*** no file '".$file{"seq4loctree"}."' written \n") if (! -e $file{"seq4loctree"});
    } else {
	$file{"seq4loctree"} = $file{"seqFasta"};
    }
    ...
    # --------------------------------------------------
    # Predict Localiaztion Tree
    # --------------------------------------------------

    if ($optRun =~/loctree/) {
	#set defaults in case user did not provide info	
	$parLocTreeProtID  = $fileJobId;
	$parLocTreeOrgType = "plan";	
	# user defined params
	if ( $optRun =~/parLocTree\(([^\(]+)\)/ ) {
	    $parLocTreeExp = $1;
	   
	    if ($parLocTreeExp=~/orgType\=(\w*)/i ) {
		$parLocTreeOrgType = $1;
	    }else {
		$parLocTreeOrgType= "";
	    }
	    if ($parLocTreeExp=~/protId\=(\w*)/i ) {
		$parLocTreeProtID = $1;
	    }
	}
	...
	my $dirWorkLocTree = "$dirWork/loctree/$fileJobId";
	unless ( (-d $dirWorkLocTree)){	
		my @created = File::Path::make_path(
			$dirWorkLocTree,{verbose => $Ldebug, mode => 0750},
		);
		warn("Created $_") for @created;
	}
	
# 


	$fileLoctreeTemp = $file{"loctree"}=        $dirWork.$fileJobId.".loctree";   
	push(@kwdRm,"loctree");


	my %hash_p	=	(
		"-debug"	=> 	$Ldebug,
		"-fasta"	=>	$file{seq4loctree},
		"-dir"		=>	$dirWorkLocTree,
		"-dirTmp"	=>	$dirWorkLocTree,
		"-outHdr"	=>	$parLocTreeProtID,
		"-org"		=>	$parLocTreeOrgType	
	);

	$fileTmpResLocTree = "$dirWorkLocTree/$fileJobId.fasta.txt";
	


            # --------------------------------------------------
            # do run Predict Loc Tree
            # --------------------------------------------------
        my $inseq = Bio::SeqIO->new( -file => $file{seq4loctree}, -format => 'Fasta' )->next_seq();
        if( !$inseq ) { confess("failed to read FASTA sequence from ``$file{seq4loctree}''"); }

        my @cmd = ( $exeLocTree, map { ( $_ => $hash_p{$_} ); } keys(%hash_p) );
	$msgHere="--- ".__FILE__.':'.__LINE__." $sbr @cmd'";
	if($Ldebug){ warn ($msgHere); }

	my( undef, $ret ) = &cachedExtCall( \@cmd, undef, { md5string => "$exeLocTree --seq=".$inseq->seq()." --org=$parLocTreeOrgType", md5files => [], cache_files => { 'LOCtreeres' => $fileTmpResLocTree },
                enable => $predictPP::docache
        } );
        $ret && confess( "'@cmd' failed" );
#        system( @cmd ) &&  confess($!);
	

	eval {
		copy (   $fileTmpResLocTree ,  $fileLoctreeTemp  )   
	};
	if ($@) {
		$msgHere ="--- ".__FILE__.':'.__LINE__." $sbr ".confess ("$!"); 
		warn $msgHere  if ($Ldebug);
		return (0,"err=560","*** ERROR $sbr\n"."$tmpErr\n"."\n");
	}
	...
	# -------------------------------------------------
	$fhloctree = 'FHLOCTREE';
	$Lok=       &open_file($fhloctree,$file{"loctree"});
	return(0,"*** ERROR $sbr: '$file{\"loctree\"}' not opened\n") if (! $Lok);


		if ( $Lok == 1 ) {
		$#tmpAppend=0;	#   
		push(@tmpAppend,$envPP{"fileAppLocTree"},
			$file{"loctree"},$envPP{"fileAppLine"});
				
				# ------------------------------
				# append HTML output
				# ------------------------------
	
			if ($optRun =~/loctree_only/) { # HACK: remove previouse output loctree should only 
				#                           # have its own output
				unlink $fileHtmlTmp; unlink $fileHtmlToc;
			}
	
			if ($optOut=~/ret html/ && -e $file{"loctree"}){ 
				# # append file
				($Lok,$msg)=&htmlBuild($file{"loctree"},$fileHtmlTmp,$fileHtmlToc,1,"loctree");
				if (! $Lok) { $msg="*** err=2250 ($sbr: htmlBuild failed on kwd=loctree)\n".$msg."\n";
					print $fhTrace $msg;
					return(0,"err=2250",$msg); }
			}		# 
			}
		...
		if ($origin =~ /^mail|^html|^testPP/i){
			...
			if ($optRun =~/loctree_only/) {
				# # --------------------------------------------------------------------------
				# # HACK: rebuild result text files so they would contain only loctree results
				# # --------------------------------------------------------------------------
			open($fhinLoc, $fileLoctarTemp);
			open($fhoutLoc,">".$filePredTmp);
					print $fhoutLoc
		...
		#Clean up
		File::Path::remove_tree(   $dirWorkLocTree ,     { verbose => $Ldebug, result  => \my $unlink_results }        ) if (!$Ldebug);
		#&ctrlDbgMsg("Unlinked $_",$fhTrace,$Debug) for @unlink_results;
	}
...
sub convertToXML {
    ...
    #   loctree, loctar
    eval {
      my $inputfile = $job_dir."/".$job_name.'.loctree';
      if( ! -e $inputfile ) { $inputfile = $job_dir."/".$job_name.'.loctar'; }
      if( -e $inputfile )
      {
          my $istream = new IO::File( $inputfile, '<' ) || confess("could not open '$inputfile': $!");
...
sub               getResPubDir
{
  my( $__fileJobId, $__File_result ) = @_;

        # We have a $File_result parameter for predict - let's use it! And let's use its basename to set the base for the XML as well.
        # Now we also have getResPubDir( $filePID ) - normally $PP_XCH_RES_PUB hidden in $envPP{"dir_resPub"}.
        # If there is NO path in $File_result, use $PP_XCH_RES_PUB for the path, but if there is, that takes precedence.

  my $dir = $envPP{"dir_resPub"}; 
  if( $__fileJobId =~ /predict_s/o ){ $dir = $envPP{"dir_resGT"}; }

#&ctrlDbgMsg("filejobid= $fileJobId dir_resGT=$dir",$fhTrace,Debug);	# 

  if( $dir !~ /\/$/o ){ $dir .= "/"; }

  my( undef, $filename_suffix ) = File::Basename::fileparse( $__File_result );
  my( $filename, $directories, $suffix ) = File::Basename::fileparse( $__File_result, qr/\.[^.]*$/o );
...
sub               storeXML
{
  my %__p = ( dir => undef, basename => undef, xml => undef, XMLLINT_ERRNO => undef, convert_error => undef, debug => undef, @_ );
  #warn( Data::Dumper::Dumper( \%__p ) );

  my $xmlfile =       "$__p{dir}/$__p{basename}.xml";
  my $xmlstatusfile = "$__p{dir}/$__p{basename}.xml.err";
  
  eval {
    my $xmlfh = IO::File->new( $xmlfile, '>' ) || confess( $! );
    print $xmlfh $__p{xml};
    undef $xmlfh;

    my $gzippedit = 0;

    my @cmd = ( 'gzip', '-f', $xmlfile );
    system( @cmd ) && confess( "@cmd: $!" );
    $gzippedit = 1;
  };
  my $xmlsaveproblems = $@;

      if( $__p{debug} ) { warn( "---  saved:\t`$xmlfile".( $gzippedit ? '.gz' : '' )."'" ); }

  if( $xmlsaveproblems || $__p{convert_error} || $__p{XMLLINT_ERRNO} )
  {
    my $xmlstatusfh = IO::File->new( $xmlstatusfile, '>' ) || confess( $! );
    if( $__p{convert_error} ) { print $xmlstatusfh "convert_error=$__p{convert_error}\n"; }
    if( $__p{XMLLINT_ERRNO} ) { print $xmlstatusfh "xmllint_errno=$__p{XMLLINT_ERRNO}\n"; }
    if( $xmlsaveproblems ) { print $xmlstatusfh "xmlsaveproblems=$xmlsaveproblems\n"; }

        if( $__p{debug} ) { warn( "---  saved:\t`$xmlstatusfile'" ); }
  }
}
