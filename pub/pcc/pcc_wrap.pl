...
($Fasta)=@ARGV;                     #input comes from the submission form and is a single fasta file

if ($ENV{"PP_ROOT"}){
    $root=$ENV{"PP_ROOT"};
}else{
    $root =  "/nfs/data5/users/ppuser/server";
}
#print "$root\n"x10;
$BlastTraindb="/data/blast/big";    #psiblast iteration database
$BlastFinaldb="$root/pub/pcc/data/cc_control4"; #PredictCellCycle Trusted Set
#$BlastFinaldb="/misc/freesia/orchid_copy/databases/cc_control4"; #PredictCellCycle Trusted Set
$CellCycleDB="$root/server/pub/pcc/data/cellcycledb.txt";
#$CellCycleDB="cellcycledb.txt";    
    $id=$Fasta;
    $id=~s/^.*\/|\..*$//g;
    $id=~s/.fasta$|.f$//;
    $id=~s/pred_//;
...
#FILE: foreach $m8File (@m8Files){
	...
	#print $_;
	@data=split(/\s+/, $_);
	if($#data != 11){ print  "wrong format of m8File=$m8File, continue..."; }  #check format
	...
	#die "alignment lengths in file=$m8File $Query $Subject calculated in independent ways not eqal: $aliLen vs $aliLenCheck in line:\n$_\n, stopped"
	  #if($aliLen != $aliLenCheck); #die if statement commented out 04/08/2005 when fixing after fasta failure
...
print "\n\n*******************\nPredictCellCycle & CellCycleDB are currently being updated\nIf you assume your protein as part of the\ncell cycle process but was not found by our method\nwe would appreciate it if you can please contact:\nkaz\@cubic.bioc.columbia.edu  \nto have your protein included in our database after further evaluation.\nThank You for your interest and using CellCycleDB!\n*******************";
...
(system "rm $Ufile4"); #removes NewHSSP.rdb fil 
