# -*- perl -*-
# Copyright (c) 1997-1999  Nigel P. Brown. $Id: HSSP.pm,v 1.18 1999/01/26 23:18:22 nbrown Exp $
...
sub get_entry {
    ...
    new Parse::Format::HSSP(undef, $text, $offset, $bytes);
}
...
###########################################################################
package Parse::Format::HSSP::HEADER;
...
sub new {
	...
	if ($line =~ /^PARAMETER/) {
	    ...
	    $tmp  = new Parse::Record_Stream($self, 11, \$line);
	    $line = $tmp->scan_lines(0);
	    chomp $line;
	    $line = Parse::Record::strip_english_newlines($line);
	...
	if ($line =~ /^AVAILABLE/) {
	    ...
	    $tmp  = new Parse::Record_Stream($self, 11, \$line);
	    $line = $tmp->scan_lines(0);
	    $self->{'available'} = Parse::Record::strip_english_newlines($line);
	    next;
	} 
	...
	if ($line =~ /^HEADER/) {
	    ...
	    $tmp  = new Parse::Record_Stream($self, 11, \$line);
	    $line = $tmp->scan_lines(0);
	    $self->{'header'} = Parse::Record::strip_english_newlines($line);
	    next;
	} 
	...
	if ($line =~ /^COMPND\s+(.*)/) {
	    ...
	    $tmp  = new Parse::Record_Stream($self, 11, \$line);
	    $line = $tmp->scan_lines(0);
	    $self->{'compnd'} = Parse::Record::strip_english_newlines($line);
	    next;
	} 
	...
	if ($line =~ /^SOURCE\s+(.*)/) {
	    ...
	    $tmp  = new Parse::Record_Stream($self, 11, \$line);
	    $line = $tmp->scan_lines(0);
	    $self->{'source'} = Parse::Record::strip_english_newlines($line);
	    next;
	} 
	...
	if ($line =~ /^AUTHOR\s+(.*)/) {
	    ...
	    $tmp  = new Parse::Record_Stream($self, 11, \$line);
	    $line = $tmp->scan_lines(0);
	    $self->{'author'} = Parse::Record::strip_english_newlines($line);
	    next;
	} 
...
###########################################################################
package Parse::Format::HSSP::PROTEIN;
...
sub new {
	...
	if ($part1 =~ /^
		...
		$data->{'accnum'} = Parse::Record::strip_trailing_space($11);
...
###########################################################################
package Parse::Format::HSSP::ALIGNMENT;
...
###########################################################################
package Parse::Format::HSSP::PROFILE;

use vars qw(@ISA);

@ISA = qw(Parse::Record);

sub new { Universal::warn(shift, "new() not implemented") }


###########################################################################
package Parse::Format::HSSP::INSERTION;
