# -*- perl -*-
# Copyright (c) 1996-1999  Nigel P. Brown. $Id: Record.pm,v 1.46 1999/09/03 16:33:57 nbrown Exp $

###########################################################################
package Parse::Record;
...
sub free {
    ...
    ##free everything, including self - not necessary as back refs removed
    #foreach $rec (@{$self->{'record_by_posn'}}) {
    #warn "  ", join(", ", @$rec), "\n"    if @$rec > 3;
    #$rec->[3]->free    if @$rec > 3;    #recursively free subrecords
    #splice @$rec, 3;                    #remove parse object
    #}
...
#warn with error string: override Universal::warn() to a) be quiet
#about the classname and b) interpolate Parse::Record->get_absolute_key.
sub warn {
    my $self = shift;
    chomp $_[$#_];
    warn "Warning '$self->{'absolute_key'}' @_\n";
}


###########################################################################
package Parse::Record_Stream;
