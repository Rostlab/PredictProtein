...
package tlprof_htmref;

#===============================================================================
sub htmref {
#-------------------------------------------------------------------------------
#   htmref                  package version of script
#-------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
	...
	undef %rd;
	%rd=
	    &rd_rdb_associativeHere
		($file_in,"not_screen","header","body",@des_rd);
				# ------------------------------
				# correct RDB
				# ------------------------------
	$#tmp=0;
	foreach $des_rd (@des_rd){
	    if    ($Lprof && $des_rd eq "RI_H" && ! defined $rd{$des_rd,1}){
		foreach $it (1..$rd{"NROWS"}){
		    $rd{$des_rd,$it}=$rd{"RI_M",$it}; 
		}}
	    elsif ($Lprof && defined $des_trsl{$des_rd} && 
		   $des_trsl{$des_rd}!~/^\s*$/ &&
		   defined $rd{$des_rd,1}){
		foreach $it (1..$rd{"NROWS"}){
		    $rd{$des_trsl{$des_rd},$it}=$rd{$des_rd,$it};
		}
		$des_rd=$des_trsl{$des_rd};}
	    elsif ($Lprof && defined $kwdProf{$des_rd} && $kwdProf{$des_rd}!~/^\s*$/){
		foreach $it (1..$rd{"NROWS"}){
		    $rd{$des_rd,$it}=$rd{$kwdProf{$des_rd},$it};
		}}
	    elsif (! $Lprof){
	...
	@des_rd=@tmp; $#tmp=0;
				...
				#          refinement of PHD_PROFhtm
				# ------------------------------

				# concetenate @des into one vector (tab separated)
	$desvec=join("\t",@des_rd);

	($nmax,$model_max,$n2nd,$model_2nd,$ri,$ri_diff)=    
	    &htmrefDo($Lmin,$Lmax,$Lloop,$Tminval,$Tgrid,$desvec,$syml,$symh,
...
#	&wrt_rdb_htmref("STDOUT",$desvec,"\t",$nmax,$n2nd,$ri,$ri_diff) if ($Lscreen);
	...
	print "--- htmref_PHD_PROF output in file: \t '$file_out'\n" if ($Lscreen);
...
#-------------------------------------------------------------------------------
    ...
    $scrGoal=    "refine the prediction of PHD_PROFhtm";

    $scrIn=      ".rdb_PHD_PROF file from PHD_PROF";
...
# version 01-96: 18, 25, 4 grid= 50, excl=0.4
    ...
    @des_out=("PRHL","PR2HL");
    %des_trsl=(
	       'RI_S',"RI_H",	# output will be 'RI_H' (input may be RI_S)
	       'RI_M',"RI_H",	# output will be 'RI_H' (input may be RI_S)
	       'OMN', "OHL",	# 
	       'PMN', "PHL",	# 
	       'OtM', "OtH",
	       'OtT', "OtH",
	       'OtN', "OtL",
				...
				# translation for PROF
    %kwdProf=
	(
	 'OHL',  "OMN",		# kwd 'OHL' will be written as 'OMN'
	 'PHL',  "PMN",		# kwd 'PHL' will be written as 'PMN'
	 'RI_H', "RI_M",
	 'PFHL', "PFMN",
	 'PRHL', "PRMN",
	 'PR2HL',"PR2MN",
...
#-------------------------------------------------------------------------------
    ...
    $tmp{"special"}.=        "htmref_Tgrid".","."prof".","; 
        
    $tmp{"title"}=           "title of output file (adds ext .rdb_htmref)";
    ...
    $tmp{"prof"}=            "write different column names";
...
#--------------------------------------------------------------------------------
	...
	foreach $des_in (@des_bodyin) {
		...
		if    ($READNAME[$it] eq $des_in){
		    $ptr_rd2des{$des_in}=$it;
		    push(@des_body,$des_in);
		    $Lfound=1;
		    last;} 
		elsif ($Lprof && defined $kwdProf{$des_in} && 
		       $READNAME[$it] eq $kwdProf{$des_in}) {
		    $ptr_rd2des{$kwdProf{$des_in}}=$it;
		    push(@des_body,$kwdProf{$des_in});
		    $Lfound=1;
		    last;} 
	    }
	    if((!$Lfound) && $Lscreen){
	...
	$des_intmp=$des_in;
	$des_intmp=$kwdProf{$des_in} if ($Lprof && defined $kwdProf{$des_in});
	if (defined $it && defined $READFORMAT[$it] ) {
	    $rdrdb{$des_intmp,"format"}=$READFORMAT[$it];}
	else {
	    $rdrdb{$des_intmp,"format"}="-s";}
    }
...
sub htmrefDo {
    ...
    $[ =1 ;
#--------------------------------------------------------------------------------
#    htmrefDo                     implements the following algorithm:
...
#--------------------------------------------------------------------------------
	...
	$tmp{$_}=1;}
    $Lerr=0;
    foreach $kwd ($oth,$otl,$phd,$ri){
	if (! defined $tmp{$kwd} && 
	    ! defined $tmp{$kwdProf{$kwd}}){
	    print "*** ERROR in htmrefDo: kwd=$kwd or kwdProf=",$kwdProf{$kwd},", not in header!\n";
	    $Lerr=1;}}
    if ($Lerr){
	print "*** ERROR for fileIn=$file_in!\n";
	print "*** desvec passed=$desvec\n";
	exit;}
				# --------------------------------------------------
				# refine
				# --------------------------------------------------
	...
	if ($ct_htm>($numres/($Lmin+2*$Lloop))){
	    print "xx failed in htmrefDo ($scrName)\n";
	    last;}	# x.x
...
sub wrt_rdb_htmref {
    ...
    $[ =1 ;
#--------------------------------------------------------------------------------
#   writes the RDB format written by PHD/PROF
#--------------------------------------------------------------------------------
    @des=split(/\t/,$desvec);
				# header
    @tmp=split(/\#/,$rd{"header"});
	...
	$Lok=1                  if ($_=~/Perl-RDB/);
	next if (! $Lok);
	if ($_=~/(PHD|PROF)htm: prediction of helical transmembrane regions/){
	    $old1="PHDhtm: prediction of helical transmembrane regions";
	    $old2="PROFhtm: prediction of helical transmembrane regions";
	    $new="PROFhtm_ref: refined prediction of transmembrane helices";
	    $_=~s/($old1|$old2)/$new/;}

	if (! $Lnotation && $_=~/NOTATION/){
	    $Lnotation=1;
	    ($Lok,$msg)=
		&wrt_rdb_htmref_notation
		    ();		if (! $Lok){ print "*** ERROR $0:wrt_rdb_htmref after notation\n";
					     print "*** message=",$msg,"\n";
					     exit;}}
	if ($Lnotation) {
	    if ($_=~/\w+/){ 
		...
		$Lnotation=0;
		$Lnotation_done=1;
		$kwd1="PRHL"; $kwd2="PR2HL";
		if ($Lprof){$kwd1=$kwdProf{$kwd1};
			    $kwd2=$kwdProf{$kwd2};}
		print $fhout 
		    "# NOTATION ".$kwd1." : refined prediction (maximum score model)\n",
		    "# NOTATION ".$kwd2. ": refined prediction (second best maximum score model)\n",
		    "# \n";
	    }
	    print $fhout 
		"#",$_;}
				...
				# correct
	    if    (! $Lprof){
		$_=~s/RI_S/RI_H/g if ($_=~ /RI_S/);}
	    else {
		foreach $kwdtmp (keys %kwdProf){
		    if ($_=~/$kwdtmp/){
			$_=~s/$kwdtmp/$kwdProf{$kwdtmp}/;
			last;}}}
	    print $fhout 
		"#",$_;}
    }
				# ------------------------------
				# add local annotations if not
				#    done already
    if (! $Lnotation_done){
	($Lok,$msg)=
	    &wrt_rdb_htmref_notation
...
#	    printf $fhout "%-$tmp$sep",$rd{$des,"format"};}
	...
	foreach $des(@des){
	    if ($des eq $des[$#des]){$sep="\n";}else {$sep="$sep_in";}
	    $tmp="%-s";
	    $tmp=&form_rdb2perl($rd{$des,"format"}) if (defined $rd{$des,"format"} &&
							length($rd{$des,"format"})>1);
...
#-------------------------------------------------------------------------------
    ...
    $sbrName=$tmp."wrt_rdb_htmref_notation";
    ...
    print $fhout 
	"#\n",
	"# MODEL         : iteratively adding transmembrane helices (HTM's)\n";
    printf $fhout 
    ...
    return(1,"ok $sbrName");
}				# end of wrt_rdb_htmref_notation
