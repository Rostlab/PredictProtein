...
#-------------------------------------------------------------------------------
    ...
    exit if ($msg eq "fin");
    				...
    				# ------------------------------
    				# special option for fast clusters hack br 2004-08-29
    				#    write temporary files to local directory
    if (defined $par{"dirTmpCluster"}   && 
	$par{"dirTmpCluster"} !~/^unk$/ &&
	(-d $par{"dirTmpCluster"} || -l $par{"dirTmpCluster"} || -e $par{"dirTmpCluster"}) &&
	(length($par{"dirWork"})<1 || $par{"dirWork"}=~/^unk$/)
       ){
	$par{"dirWork"}=$par{"dirTmpCluster"};
...
#-------------------------------------------------------------------------------
	...
	foreach $kwd (@kwd){
	    next if ($kwd !~ /^file/);
	    next if ($kwd =~ /^file(In|Out|Mat|Help|Def)/i);
            $par{$kwd}=~s/jobid/$par{"jobid"}/ ;
...
sub fastaWrt {
    ...
    local($sbrName,$fhoutLoc,$tmp,$Lok,$idloc);
    $[ =1 ;
#-------------------------------------------------------------------------------
...
#    print "yy into write seq=$seqLoc,\n";
	...
	foreach $it2 (0..4){
	    ...
	    $tmp=substr($seqLoc,($it+10*$it2),10);
	    $tmp=~s/^\s*|\s*$//g;
	    next if ($tmp=~/^\s*$/);
	    $tmpwrt.=sprintf(" %-10s",$tmp);
	}
	$tmpwrt=~s/^\s*|\s*$//g;
	$tmpwrt.="\n";
