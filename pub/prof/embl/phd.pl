...
#	$scr="/home/phd/ut/phd/scr/which_arch.sh" if (! -e $scr);
	$scr="/nfs/data5/users/ppuser/server/pub/prof/pvmgetarch.sh" if (! -e $scr);
...
#-------------------------------------------------------------------------------
	...
	if   ($_ eq "acc")               { $par{"optPhd"}=         "acc"; }
	    ...
	    next if ((! -e $arg || ! -e $arg_nochain) && ! $par{"doSearchFile"});
	    			...
	    			# it appears to be a valid file
	    if (-e $arg_nochain){
				# get format of the sequence/alignment/file
		($Lok,$msg)=
		    &getFileFormatQuick($arg_nochain);
		return(&errSbrMsg("$arg_nochain failed getting format with getFileFormatQuick($arg_nochain)",$msg)) 
		    if (! $Lok);
		$format=$msg;
		if ($format ne "unk"){
...
#-------------------------------------------------------------------------------
	...
	next if (! $par{$des});
	next if ($des=~/fileOut/); # hack br 2004-08-29 (to ascertain that users can choose names for those)
