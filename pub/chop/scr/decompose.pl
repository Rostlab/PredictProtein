...
#===============================================================
				...
				# difference between sequoia and other machines
if ( defined $ENV{'HOSTNAME'} and $ENV{'HOSTNAME'} =~ /sequoia/ ) {
    $dirBio = '/usr/local/bio/';
} else {
    $dirBio = '/usr/pub/molbio/';
}


use lib '/home/ppuser/server/pub/chop/lib/';
use Getopt::Long;
use File::Copy;
use libBlast;
...
#-----------------------------------------------------------
    ...
    $dirTmp = '';
				# specific options
    $dir_chop_home = '/home/ppuser/server/pub/chop/';
    $opt_blastE = 1e-2;
    $exeBlast = $dirBio.'blast/blastpgp';
    $dirBlast = $dir_chop_home.'blast/';
    $opt_hmmerE = 1e-2;
    ...
    $quote_txt = "Result of CHOP prediction (Jinfeng Liu & Burkhard Rost)\n";
    $quote_txt .= "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n";
    $quote_txt .= "Jinfeng Liu & Burkhard Rost\n";
    $quote_txt .= "Proteins. (2004) in press\n";
    $quote_txt .= "________________________________________________________\n";
    ...
    $Lok = GetOptions ('debug!' => \$opt_debug,
		       ...
		       'minDomainLen=i' => \$minDomainLen,
		       'minFragLen2report=i' => \$minFragmentLen,
		       'blastE=f'     => \$opt_blastE,
		       'hmmerE=f'     => \$opt_hmmerE,
...
#    $minDomainCover /= 100;
	...
	print STDERR
	    ...
	    "        -blastE <real>           blast E-value threshold\n",
	    "        -hmmerE <real>           HMMER E-value threshold\n",
	    ...
	    "        -minFragLen2report <int>    min length of fragment to be reported(default=10)\n",
...
#------------------------------------------------------------
	...
	print STDERR "qualified hit: $hsp->{'nameLine'}\n" if ( $opt_debug );
...
sub printSum {
    my ( $table,$fhOut ) = @_;
    my ( $space, $entry,$homo,$eValue,$method,$database,$format,$bar );
    my ( $style_tr,$pdb_id,$region,$srs_url,$homo_link,$pfam_id,$swiss_id );
#-------------------------------------------------------------

    if ( scalar(@$table) == 1 and $table->[0]{'method'} eq 'NULL' ) {
	if ( $opt_html ) {
	    print $fhOut
		"<P>\n",
		"The query has no homology to known domains; no domain is assigned by CHOP.<BR>\n";
	} else {
	    print $fhOut
                "The query has no homology to known domains; no domain is assigned by CHOP.\n";
	}
	return;
    }
	...
	if ( $opt_html ) {
	    $eValue = '&nbsp' if ( $eValue eq 'NULL' );
	    $homo = '&nbsp' if ( $homo eq 'NULL' );
	    $entry->{'method'} =~ s/\s+//g;
	    if ( $entry->{'method'} eq 'NULL' ) {
		$method = '&nbsp';
		$database = '&nbsp';
	    } else {
		($method,$database) = split /\//,$entry->{'method'};
	    }
		...
		$region = substr($homo,4);
		$srs_url = $host_srs.$cgi_srs."?-newId+-e+[PDB-id:'$pdb_id']";
		$homo_link = "<A HREF=\"$srs_url\" TARGET=_blank >$pdb_id</A>$region";
	    } elsif ( $database =~ /pfam/i ) {
		($pfam_id,$region) = split /\(/, $homo;
		$region = '('.$region;
		$srs_url = $host_srs.$cgi_srs."?-newId+-e+[PFAMA-AccNumber:'$pfam_id']";
		$homo_link = "<A HREF=\"$srs_url\" TARGET=_blank>$pfam_id</A>$region";
	    } elsif ( $database =~ /swiss/i ) {
		($swiss_id,$region) = split /\(/, $homo;
                $region = '('.$region;
                $srs_url = $host_srs.$cgi_srs."?-newId+-e+[swissprot:'$swiss_id']";
                $homo_link = "<A HREF=\"$srs_url\" TARGET=_blank>$swiss_id</A>$region";
	    } else {
		$homo_link = $homo;
	    }
