...
$scr{"svmRun"}=                  $scr{"dirHome"}."svmRun_str.pl";
$scr{"findSigp"}=                $scr{"dirHome"}."findSigp.pl";
...
$par{"svm"}=                     "SvmRes";
$par{"fileHssp"}=                "fileHssp.hssp";
$par{"fileProf"}=                "fileProf.rdbProf";
$par{"fileOutSig"}=              "fileOutSig";
...
foreach $var (keys %par){
  ...
  if($var!~ /fileIn|job|dirOut|protId|orgType|email|sendMail|fileHssp|fileProf/){
    $par{$var}.= $par{"jobId"};
  }

}

$par{"fileErr"}=$par{"dirOut"}.$par{"fileErr"};
...
if(-e $par{'fileHssp'} && -e $par{'fileProf'} ){
  ...
  system("$scr{'svmRun'} protId=$par{'protId'} fileFasta=$par{'fileIn'} fileHssp=$par{'fileHssp'} fileProf=$par{'fileProf'} dirMod=$db{'dirMod'} jobId=$par{'jobId'} >>$par{'fileRun'}");
}
else{
  system("$scr{'svmRun'} protId=$par{'protId'} fileFasta=$par{'fileIn'} dirMod=$db{'dirMod'} jobId=$par{'jobId'} >>$par{'fileRun'}");
}
...
$par{"sendMail"}=$par{"dirOut"}.$par{"sendMail"};
...
# now run signalP ...report presence/absence of signal only!
$par{'fileOutSig'}="$par{'protId'}.sig".$par{'jobId'};
system("$scr{'findSigp'} protId=$par{'protId'} fileFasta=$par{'fileIn'} orgType=$par{'orgType'} fileOutSig=$par{'fileOutSig'} >>$par{'fileRun'}");
# now proc signaP predictions!

if(-e $par{'fileOutSig'}){
  open($fhin, $par{'fileOutSig'}) || die "could not open $par{'fileOutSig'}\n";
  while(<$fhin>){
    print $fhmail "$_";
  }
  close($fhin);
  ...
  print $fhmail "SignalP: failed running signalP on sequence\n";
  print $fhmail "SignalP: see http://www.cbs.dtu.dk/services/SignalP/ for latest version of SignalP server\n";
}
...
foreach $val (keys %par){
  ...
  next if($val eq "fileIn" || $val eq "sendMail" || $val eq "dirOut" || $val eq "jobId" || $val eq "emailId" || $val eq "orgType");
...
#system("rm pairs*$par{'jobId'} TSTi*$par{'jobId'} tst-*$par{'jobId'} *Comp*$par{'jobId'} Ma*$par{'jobId'}");
