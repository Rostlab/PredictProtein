...
#-------------------------------------------------------------------------------
#       in:                     (<1|2[_msf|_saf|_fasta]>,@array_with_all_info_from_user)
#                               returntype=1 : single sequence to return
#                               returntype=2 : alignment to return
#                               returntype=2_msf ali in msf format
#                               returntype=2_saf ali in saf format
#                               returntype=2_fasta ali in fasta format
...
#-------------------------------------------------------------------------------
	...
	if (! $Lok);
				# ------------------------------
				# digest input sequence or ali
				# ------------------------------
    $Lok=$id=0;
    if    ($format eq "meta")  { $seq= join("", @rawsequence);
				 $Lok=1; $id=1;}
    elsif ($format eq "dssp")  { ($Lok,$id,$seq)=&dsspRdSeq(\@rawsequence,"*"); }
    elsif ($format =~ /^fasta/){ ($Lok,$id,$seq)= &fastaRdMul(\@rawsequence); }
    elsif ($format eq "gcg")   { ($Lok,$id,$seq)= &gcgRd(\@rawsequence); }
    elsif ($format eq "msf")   { ($Lok,$id,$seq)= &msfRd(\@rawsequence); }
    elsif ($format eq "pdb")   { ($Lok,$id,%tmp)= &pdbExtrSequence(\@rawsequence,"*"); 
				 if ($Lok && defined $tmp{"chains"}) {
				     $seq="";
				     foreach $chain (split(/,/,$tmp{"chains"})) {
					 $seq.=$tmp{$chain};
					 $seq.="!";}$seq=~s/!$//g;}}
    elsif ($format =~ /^pir/)  { ($Lok,$id,$seq)= &pirRdMul(\@rawsequence); }
    elsif ($format eq "saf")   { ($Lok,$id,$seq)= &safRd(\@rawsequence); }
    elsif ($format eq "swiss") { ($Lok,$id,$seq)= &swissRdSeq(\@rawsequence); }
				# ERROR: wrong format
    &abort("ERROR $sbr: format of sequence ($format) not digestable!")
	if (! $Lok && ! $id);
				# ERROR: failed interpreting it 
    &abort("ERROR $sbr: failed reading format ($format) of rawsequence!")
	if (! $Lok);
	...
	return($seq); }
				...
				# ------------------------------
				# process multiple alignment
				# ------------------------------
    if ($returntype=~/^2/) {
	$Lok=0;
				# wrong format!
	&abort("ERROR $sbr: format wanted (returntype=$returntype) not '2_[msf|saf|fasta]'!")
	    if ($returntype !~/^2_(msf|saf|fasta)/i);

	@idLoc= split(/\n/,$id);
	@seqLoc=split(/\n/,$seq);
	$idLoc[1]="meta_by_default"   if (! defined $idLoc[1]);
				# rearrange
	$tmp{"NROWS"}=$#seqLoc;
	foreach $it (1..$#seqLoc){
	    $tmp{$it}=        $idLoc[$it];
	    $tmp{$idLoc[$it]}=$seqLoc[$it];
	}
	$#idLoc=$#seqLoc=0;
	$Lok=$txt=0;
				# write MSF format
	if    ($returntype=~/msf/){
	    $tmp{"FROM"}= "META server";
	    $tmp{"TO"}=   "unk";
	    ($Lok,$txt)=&msfWrt('txt',%tmp); }
				# write SAF format
	elsif ($returntype=~/saf/){
	    $tmp{"HEADER"}= "# SAF format from META server";
	    ($Lok,$txt)=&safWrt('txt',%tmp); }
				# write FASTA format
	elsif ($returntype=~/fasta/){
	    ($Lok,$txt)=&fastaWrt('txt',%tmp); }

				# ERROR: wrong format
	&abort("ERROR $sbr: format of output (in=$format, out=$returntype) not digestable!")
	    if (! $Lok && ! $txt);
				# ERROR: failed digesting
	&abort("ERROR $sbr: formatIn=$format, out=$returntype, failed!".$txt."\n")
	    if (! $Lok);
	
				# everything fine: go home
	&dbglog('$sbr: SINGLE SEQUENCE IN FASTA');
	&dbglog('New Sequence: ',$txt);
	&dbglog('PARSE_SEQ...DONE');
	return($txt); }
				# ------------------------------
				# alignment to return
...
#
#   input: 
#   * formatted alignment (DOES assume formatting)
# 
#   output:
#   1. ALIGNMENT FORMAT
#   2. 0 if not at least 2 sequences!
#
sub parse_seq_ali {
    my($alignmentFormatWanted,@rawsequence)=@_;
    my($seq,$Lok,$format,$sbr);
#-------------------------------------------------------------------------------
#       in:                     (<1|2>,@array_with_all_info_from_user)
#       out:                    (<0|1>,<error_message|@ali)
#                               two possible reasons for error:
#                               1. can NOT write output format wanted
#                               2. internal error
#-------------------------------------------------------------------------------
    $sbr="parse_seq_ali";
    $retseq="";
    ...
    &dbglog('alignmentFormatWanted: ',  $alignmentFormatWanted);
    &dbglog('raw sequence: ',           @rawsequence);
