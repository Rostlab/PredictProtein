...
if (! defined &ctime || ! &ctime || ! &localtime) { 
    if (-e "/home/$ENV{USER}/server/scr/lib/ctime.pl") {
	require("/home/$ENV{USER}/server/scr/lib/ctime.pl");}}
if (! defined &ctime || ! &ctime || ! &localtime) { 
    $date=`date`; }
else {
...
if ($Lscreen) { &myprt_empty; &myprt_txt("file here: \t \t $file_in"); }
				...
				# ------------------------------
$title="unk";
$dir_def= "/home/$ENV{USER}/server/scr/pub/evalsec/";
...
if (&is_ppcol($file_in)){
    ...
    ($tmp,$Lis_rel)= &ppcol_2_dotpred($file_in,$file_dotpred);
...
#--------------------------------------------------------------------------------
    ...
    undef %seq; undef %sec; undef %ri; undef %obs; undef %name;
    $#DESRD=$Lis_name=0;
    $Lfst=1;
				# ------------------------------
				# read file
...
#--------------------------------------------------------------------------------
	...
	if ($Lfin){		# read class prediction
	    $_=~s/^ ---\s+//g; $_=~s/rest /other/;
	    $_=~s/DSSP/ obs/; $_=~s/\%DSSP/\%obs /; $_=~s/PRED/ prd/; $_=~s/\%PRED/\%prd /;
	    if (/^[\+\|]/){
		++$ctfin;
		$rd{"class","$ctfin"}=$_; }}
    }
...
#--------------------------------------------------------------------------------
    $#tmp=0;@tmp=split(/\|/,$line);
				# numbers
    foreach $it (1..$#des_sec) {
	$des2=$des_sec[$it];
	$rd{"num","$des1","$des2"}=$tmp[$it+1]; 
	$rd{"num","$des1","$des2"}=~s/[^0-9\.\-]//g;
    }
...
#--------------------------------------------------------------------------------
	...
	$rd{"qobs","$des1","$des2"}=~s/[^0-9\.\-]//g;
    }
				# percentage of predicted
    foreach $it (1..$#des_sec) {
	++$ct;$des2=$des_sec[$it];
	$rd{"qprd","$des1","$des2"}=$tmp[$ct]; 
	$rd{"qprd","$des1","$des2"}=~s/[^0-9\.\-]//g;
    }
				# segment: obs numb, prd num, obs av, prd av
    foreach $des2 ("nsegobs","nsegprd","nsegavobs","nsegavprd") {
	++$ct;
	$rd{"seg","$des1","$des2"}=$tmp[$ct];
	$rd{"seg","$des1","$des2"}=~s/[^0-9\.\-]//g;
    }
...
#--------------------------------------------------------------------------------
    ...
    $ct=1;
				# SOV %obs
    foreach $it (1..$#des_sec) {
	++$ct;$des2=$des_sec[$it]."obs";
	$rd{"sov","$des2"}=$tmp[$ct]; 
	$rd{"sov","$des2"}=~s/[^0-9\.\-]//g;
    }

    ++$ct;$des2="Sobs";$rd{"sov","$des2"}=$tmp[$ct]; $rd{"sov","$des2"}=~s/[^0-9\.\-]//g;
				# SOV %prd
    foreach $it (1..$#des_sec) {
	++$ct;$des2=$des_sec[$it]."prd";
	$rd{"sov","$des2"}=$tmp[$ct]; 
	$rd{"sov","$des2"}=~s/[^0-9\.\-]//g;
    }
    ++$ct;$des2="Sprd";$rd{"sov","$des2"}=$tmp[$ct];$rd{"sov","$des2"}=~s/[^0-9\.\-]//g;
				# entropy
    foreach $des2 ("obs","Pobs","prd","Pprd") {
	++$ct;
	$rd{"entropy","$des2"}=0;
	$rd{"entropy","$des2"}=$tmp[$ct] if (defined $tmp[$ct]);}
...
#--------------------------------------------------------------------------------
				# watch it:
     foreach $kwd1 ("over","under","q3","bad","entropy","sov") {
 	if    ($kwd1 eq "entropy") {
 	    foreach $kwd2 ("obs","prd") {
 		$rd{$kwd1,$kwd2}=~s/[^0-9\.\-]//g; } }
 	elsif ($kwd1 eq "entropy") {
 	    foreach $kwd2 ("Sobs","Sprd") {
 		$rd{$kwd1,$kwd2}=~s/[^0-9\.\-]//g; } }
 	else {
	    $rd{$kwd1}=0 if (! defined $rd{$kwd1});
 	    $rd{$kwd1}=~s/[^0-9\.\-]//g; } }
    $fh="STDOUT";
				# ------------------------------
				# write
    ...
    print $fh "DAT +-------+--------+-------+--------+ +---------+---------+---------+---------+\n";
				...
				# hack br 99-03
    foreach $kwd ("over","under") {
	$rd{$kwd}=sprintf("%5.1f",$rd{$kwd}) if ($rd{$kwd}=~/^[0-9\.\-]*$/);
	$rd{$kwd}=sprintf("%5s",  $rd{$kwd}) if ($rd{$kwd}!~/^[0-9\.\-]*$/);}

    printf 
	$fh "DAT | OVER  | %5s  | UNDER | %5s  | |                                       |\n",
	$rd{"over"},$rd{"under"};
	    
				# hack br 99-03
    foreach $kwd ("obs","obs") {
	$rd{"entropy",$kwd}=sprintf("%5.2f",$rd{"entropy",$kwd}) 
	    if ($rd{"entropy",$kwd}=~/^\-?[0-9\.]*$/);
	$rd{"entropy",$kwd}=sprintf("%5s",  $rd{"entropy",$kwd}) 
	    if ($rd{"entropy",$kwd}!~/^\-?[0-9\.]*$/);}

    printf 
	$fh "DAT | I obs |  %5s | I prd |  %5s | |                                       |\n",
	$rd{"entropy","obs"},$rd{"entropy","prd"};
    printf 
...
#--------------------------------------------------------------------------------
    ...
    foreach $des(@des){
				# hack br 99-03
	$rd{"contave",$des}=sprintf("%6.2f",$rd{"contave",$des}) 
	    if ($rd{"contave",$des}=~/^\-?[0-9\.]*$/);
	$rd{"contave",$des}=sprintf("%6s",$rd{"contave",$des}) 
	    if ($rd{"contave",$des}!~/^\-?[0-9\.]*$/);
	$rd{"contvar",$des}=sprintf("%6.2f",$rd{"contvar",$des}) 
	    if ($rd{"contvar",$des}=~/^\-?[0-9\.]*$/);
	$rd{"contvar",$des}=sprintf("%6s",$rd{"contvar",$des}) 
	    if ($rd{"contvar",$des}!~/^\-?[0-9\.]*$/);

	printf 
	    $fh "DAT | Dcontent %1s = %6s | one standard deviation = %6s |\n",
...
sub run_program {
    ...
    open (TMP_CMD, "|$cmdtmp") || 
	do { print $log_file "Can't run command: $cmdtmp\n" if ( $log_file );
