...
#    $par{"dirFssp"}=            "/home/rost/data/fssp/,/data/fssp/"; # dir for FSSP files
    ...
    $par{"dirHssp"}=            "/data/hssp/,/data/hssp/"; # dir for HSSP files
    $par{"dirDssp"}=            "/data/dssp/,/data/dssp/"; # dir for DSSP files
    $par{"dirFssp"}=            "/data/fssp/,/data/fssp/"; # dir for FSSP files
...
#    $par{"fileMatGcg"}=         $par{"dirSrcMat"}.    "Maxhom_Blosum.metric";  # MAXHOM-GCG matrix
    ...
    $par{"pdb_maxUnk"}=        20; # maximal percentage of unknown residues in PDB to consider 'protein'
    $par{"pdb_atom"}=           1; # read PDB sequence from ATOM fields, if=0-> SEQUENCE field
    $par{"pdb_seleno"}=         0; # if 1: show selenomethionine as 'X'
...
#----------------------------------------------------------------------
	...
	($Lok,$msg,$rh_pdb)=
	    &pdbExtrSequenceATOM
		($fileInLoc,$chainInLoc,1,$par{"pdb_maxUnk"},$par{"pdb_seleno"});
    }
    else {
	($Lok,$msg,$rh_pdb)=
	    &pdbExtrSequenceSEQRES
		($fileInLoc,$chainInLoc,1,$par{"pdb_maxUnk"});
    }
