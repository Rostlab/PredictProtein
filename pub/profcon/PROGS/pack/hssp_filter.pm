...
#    @ARGV=split(/\s+/,&get_in_keyboard("command line"));	# x.x mac specific
	...
	&ini();                 if (! $Lok) { print "*** ERROR $scrName after ini\n",$msg,"\n";
					      die '*** during initialising $scrName  $msg  ';}
...
#                            "------------------------------------------------------------\n";
	...
	next if ($arg=~/^(dirLib|ARCH|PWD|packName)=/);
        ...
        elsif ($arg=~/^big$/i)              { $LuseBigBinar=         1;}
	    ...
	    return(0,"*** ERROR $sbrName: kwd '$arg' not understood\n");}}
				...
				# replace binary
    if ($LuseBigBinary){
	$par{"exeFilterHssp"}=$par{"exeFilterHsspBig"};
	if (! -e $par{"exeFilterHssp"} && ! -l $par{"exeFilterHssp"}){
	    print 
		"*** you want the BIG for hssp_filter.$ARCH(".
		    $par{"exeFilterHssp"}."), however none there!\n";
	    exit;}}
...
#    $par{"exeConvertSeq"}=      $par{"dirBin"}.  "convert_seq".".".$ARCH;
    ...
    $par{"exeFilterHsspBig"}=   $par{"dirBin"}.  "filter_hssp".".".$ARCH; # FORTRAN excutable
