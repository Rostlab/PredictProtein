...
if (defined $ENV{'PROFCON'}){
    ...
    $root_dir = " /nfs/data5/users/$ENV{'USER'}/server/pub/profcon";
			}

$none11=0;
$none12=0;

if (@ARGV<4){ die "Usage: $0 protein_name file.hssp file.rdbProf file.seg\n";}
...
if (/^\/\//) {last READ_HSSP;}
if (/^SEQLENGTH/) {($none1,$length)=split(/\s+/,$_);}
