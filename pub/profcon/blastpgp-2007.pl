...
($par{"dirScript"} )            =($0=~/^(\S*\/)/);
$par{"dirScript"}               ="" if(! defined $par{"dirScript"});
				# ------------------------------
				# default package asf
$packDefault                    =$par{"dirScript"}."pack/blastpgp.pm";
#$packDefault=                   $par{"dirBlast"}.    "pack/blastpgp.pm";
$ARCH_DEFAULT=                  "SGI64";

$par{"exeBlast"}=               $par{"dirBlastBin"}. "blastpgp.ARCH";

$par{"dbBlastX"}=               $par{"dirDataBlast"}."big_98_X";
$par{"dbBlastB"}=                $par{"dirDataBlast"}."big.00";
				# ------------------------------
				# stringent 
##### MARCO MODIFIES ####
#$par{"argBlastXstrict"}=        "-j 3 -e 1e-12 -h 1e-12 -b 5000 -v 5000 -F T ";
$par{"argBlastXstrict"}=        "-j 2 -e 1e-12 -h 1e-12 -b 5000 -v 5000 -F T ";
########################
$par{"argBlastBstrict"}=        "-e .001 -b 5000 -v 5000 -F T ";
				# ------------------------------
				# permissive
#### MARCO MODIFIES #####
#$par{"argBlastXrelax"}=         "-j 3 -e 0.0005 -h 0.0005 -b 5000 -v 5000 -F T ";
$par{"argBlastXrelax"}=         "-j 2 -e 0.0005 -h 0.0005 -b 5000 -v 5000 -F T ";
$par{"argBlastBrelax"}=         "-e 1 -b 5000 -v 5000 -F T ";
...
#-------------------------------------------------------------------------------
	...
	elsif ($arg=~/^maxAli=(.*)$/i)        { $maxAli=         $1;}
	elsif ($arg=~/^rdb$/)                 { $fileOutRdb=      1;}
	elsif ($arg=~/^rdb=(.*)$/)            { $fileOutRdb=     $1;}
	elsif ($arg=~/^tile$/)                { $tile=            1;}
	elsif ($arg=~/^tile=(.*)$/)           { $tile=           $1;}
	elsif ($arg=~/^eSaf=(.*)$/i)           { $eThresh=        $1;}
...
#-------------------------------------------------------------------------------
	...
	printf "%5s %-15s %-20s %-s\n","","",        "",        "YOU HAVE TO SPECIFY ALL OF THOSE PARAMETERS, IF YOU SPECIFY";
	printf "%5s %-15s %-20s %-s\n","","",        "",        " JUST ONE, THE OTHER ONES WILL RVERT TO NCBI DEFAULTS!!!!!!!!!!!!";
...
#	printf "%5s %-15s=%-20s %-s\n","","",   "x", "";
	...
	printf "%5s %-15s %-20s %-s\n","","",        "",         "and  PSSM building (default $par{dbBlastX}) ";
	printf "%5s %-15s=%-20s %-s\n","","dbFinal",     "x",   "blast database for final iteration (default $par{dbBlastB})";
