...
#===============================================================================
sub blastit {
    local($fileIn,$execBlast,$databaseX,$databaseB,
	  $blastXCmdStr,$blastBCmdStr,
	  $fileOutBlast,$fileOutMat,$fileOutProf,$fileOutSaf,$dirWork,$dirOut,
	  $optNice,$filter,$maxAli,$rdb,
	  $tile,$eThresh,$keepIter,$Ldebug)=@_;
...
#       in:                     $fileOutProf:   <0|1|name of output binary matrix> (0 for none)
...
#       err:                    (1,'ok'), (0,'message')
#-------------------------------------------------------------------------------
				# check input for essential arguments
    return(0,"not def fileInLoc!")                 if (! defined $fileIn);
    return(0,"missing fileIn=$fileIn!")            if (! -e $fileIn && ! -l $fileIn);
    return(0,"missing execBlast=$execBlast!")      if (! -e $execBlast && ! -l $execBlast);
    $tmp=$databaseX.".psq";
    return(0,"missing databaseX=$tmp!") if (! -e $tmp  && ! -l $tmp);
    $tmp=$databaseB.".psq";
    return(0,"missing databaseB=$tmp!") if (! -e $tmp  && ! -l $tmp);
    return(0,"not defined blastXCmdStr")           if (! defined $blastXCmdStr);
    return(0,"not defined blastBCmdStr")           if (! defined $blastBCmdStr);
    				# get arguments 
    $dirOut=     ""             if (! $dirOut);
    ...
    $Ldebug=0                   if (! defined $Ldebug);
				...
				# further correction
    $blastXCmdStr.=" -d ".$databaseX;
    $blastBCmdStr.=" -d ".$databaseB;
				# output files
    ...
    $blastXFile=$idOut.".tmp_$$";
				# BLAST output file
    if (! $fileOutBlast){
	$fileOut=$dirOut.$idOut.$main::par{"extBlastpgp"};}
    else {
	$fileOut=$fileOutBlast;}
                            	# SAF output file 
    if ($fileOutSaf){
	if ($fileOutSaf eq "1"){
	    $saf=$dirOut.$idOut.$main::par{"extSaf"}; }
	else {
	    $saf=$fileOutSaf;}}
    else {
	$saf=0;}
    ...
    if ($fileOutMat){
	if ($fileOutMat eq "1"){
	    $mat=$dirOut.$idOut.$main::par{"extBlastmat"};}
	else {
	    $mat=$fileOutMat;}}
    else {
	$mat=0;}
    ...
    if ($fileOutProf){
	if ($fileOutProf eq "1"){
	    $fileOutProf=$dirOut.$idOut.$main::par{"extBlastcheck"};}
    }
    
      
    if ($rdb){
	if ($rdb eq "1"){
	    $rdb=$dirOut.$idOut.$main::par{"extRdb"};}
	else {
	    $rdb=$rdb;}}
    else {
	$rdb=0;}
...
sub runBlast {
    ...
    $cmdXLoc=$optNice." ".$cmdXLoc            if (defined $optNice && $optNice);

    print "--- runBlastX system '$cmdXLoc'\n" if (defined $Ldebug && $Ldebug);
    $tmp=system($cmdXLoc);
    $tmp == 0  || 
	return(0,"*** ERROR blast on $databaseX returned with exit signal equal to $tmp \n");


    $cmdBLoc="$execBlast $blastBCmdStrLoc";
    $cmdBLoc=$optNice." ".$cmdBLoc            if (defined $optNice && $optNice);

    print "--- runBlastB system '$cmdBLoc'\n" if (defined $Ldebug && $Ldebug);
    $tmp=system($cmdBLoc);
    $tmp == 0  || 
	return(0,"*** ERROR blast on $databaseB returned with exit signal equal to $tmp \n");
    
    if ($keepIter){ 
	$blastIterFile= $fileOut; 
	if (defined $::par{"extBlastIter"}){
	    $blastIterFile.=$::par{"extBlastIter"};}
	else {
	    $blastIterFile.="_tmp";}

	rename ($blastXFile, $blastIterFile)==1 || 
	    return(0,"*** ERROR $sbr: failed to rename blast file with iterations $blastXFile into $blastIterFile \n");
    }
    
    if($fileOutProf){
	rename ($blastCheckFile, $fileOutProf);
    }
    unlink $blastCheckFile               if (-f $blastCheckFile && 
					     (! defined $Ldebug || ! $Ldebug));
    unlink $blastXFile                   if (! $keepIter && 
					     ( ! defined $Ldebug || ! $Ldebug) );
    return (1,"ok");
}				# end of runBlast
