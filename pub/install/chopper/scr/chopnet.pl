...
use File::Copy;
use libGenome;
use libPHD qw(getLenPHD getFieldPHD);
use libChopper;
...
push @tmp_files,$file_splitdomain,$file_sam2pos,$file_vector_in,
    ...
    $file_nn_err,$file_report,$file_postnn,'NNo_tst_err.dat',
    'nn_train.jct_tmpprot','NNo-yeah.tmp';
...
$cmd_process = "$scr_process -i $file_report -l $prot_len -o $file_postnn -of storable ";
...
if ( ! -f $file_postnn ) {
    ...
    exit(1);
				...
				# always output XML
$file_xml = $file_result.'.xml';
$pred_hash = retrieve($file_postnn);
&hash2xml($pred_hash,$file_xml);

if ( $format_out eq 'storable' ) {
    move($file_postnn,$file_result) or die "cannot move $file_postnn to $file_result:$!";
} elsif ( $format_out eq 'xml' ) {
    move ($file_xml,$file_result) or die "cannot move $file_xml to $file_result:$!";
} elsif ( $format_out eq 'casp' ) {
    ($Lok,$seq_hash) = &getFieldPHD($file_in{'prof'},[ 'AA' ]);
    if ( ! $Lok ) {
	print STDERR "*** ERROR getting sequence from rdbProf: $file_in{'prof'}\n";
	die;
    }
    for $seq_no ( sort { $a <=> $b } keys %$seq_hash ) {
	$seq .= $seq_hash->{$seq_no}->{'AA'};
    }
    &xml2casp($file_xml,$seq,$file_result);
}
