...
#-------------------------------------------------------------------------------
	...
	$par{"dirHome"}=            "/usr/pub/molbio/phd/";
    }
    elsif ($0=~/maxhom/){
	...
	$par{"dirHome"}=            "/nfs/data5/users/ppuser/server/pub/phd/";
    }
    else {
	$par{"dirHome"}=            "/usr/pub/molbio/prof/";
    }
    ...
    $par{"dirPerl"}=            $par{"dirHome"}."scr/"; # directory for perl scripts needed
...
#                            "------------------------------------------------------------\n";
	...
	($Lok,$msg)=
	    &brIniHelp(%tmp);   return(0,"*** ERROR $sbrName: after copf.pm:brIniHelp\n".
				       $msg) if (! $Lok);
	exit; }

    ($Lok,$msg)=		# want help?
	&brIniHelpLoop($scrName,
		       %tmp);   return(0,"*** ERROR $sbrName: after copf.pm:brIniHelp\n".
				       $msg) if (! $Lok);
    if ($msg eq "fin") {print 
			    "--- suggest \n".
				"copf.pm help curve\n";
...
#	    if ($par{"verbose"});
	...
	&brIniSet();            return(0,"*** ERROR $sbrName: after copf.pm:brIniSet\n") if (! $Lok);
    ...
    ($Lok,$msg)=		# check errors
        &brIniErr($exclude);    return(0,"*** ERROR $sbrName: after copf.pm:brIniErr\n".$msg) if (! $Lok);  
				# ------------------------------
	...
	($Lok,$msg)=
	    &brIniWrt($exclude,$fhTrace);
	                        return(0,"*** ERROR $sbrName: after copf.pm:brIniWrt\n".
                                       $msg) if (! $Lok);}
...
#    $LscreenTmp=1;
    ...
    return(0,"*** $sbrNamex: not def fhSbr!")                  if (! defined $fhSbr);
				...
				# ------------------------------
				# default settings
    $exeFilterHssp="/nfs/data5/users/ppuser/server/pub/bin/filter_hssp98.".$ARCH   if ($exeFilterHssp eq "def");
#    $fileMatGcg=   "/nfs/data5/users/ppuser/server/pub/lib/mat/Maxhom_GCG.metric"  if ($fileMatGcg eq "def");
    $fileMatGcg=   "/nfs/data5/users/ppuser/server/pub/prof/mat/Maxhom_GCG.metric"  if ($fileMatGcg eq "def");
