...
if (@ARGV<3){ die "Usage: $0 protein_name file.hssp file.rdbProf \n";}
...
if (/^##/) {
if ((defined $mod) and ($mod==2)) {last READ_HSSP;}
		}
...
if (/^No/) {
($none1,$none2,$check_none)=split(/\t/,$_);
if ($check_none eq "PHEL") {$mod=1;}
if ($check_none eq "OHEL") {$mod=2;}
next READ_PROF;}
...
if ($mod==1) {
($resnum,$tmp_type,$tmp_str,$tmp_ri_ss,$none1,$none2,$tmp_ri_acc,
$tmp_ph,$tmp_pe,$tmp_pl,$tmp_twoacc)=split(/\t/,$_);
                } # end of if ($mod==1)
...
$sample_all=$root_dir."/".$input_name.".sample_all";
...
$sample_all=$root_dir."/".$input_name."_TITLE.sample_all";
