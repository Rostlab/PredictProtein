...
$file=$ARGV[0]; #fasta
$file1 = $ARGV[1]; #rdbProf
$hsspfil=$ARGV[2];  #hssp


print "file=$file\tfile1=$file1\thsspfil=$hsspfil\n"x5;
...
open (FOUT, ">$dir/temp/$dataFile") || print "Line 36: can't open file $dir/temp/$dataFile $!";
...
loop666:while ($line=<FILE>) {####################change this loop see file arath_fr13110##############
	if ($line=~/No	AA	OHEL	PHEL	RI_S	OACC	PACC	OREL	PREL	RI_A|No	AA	PHEL	RI_S	PACC	PREL	RI_A	pH	pE	pL/)  {
		last loop666;
		}
#    }
#	if ($line=~/No	AA	PHEL	RI_S	PACC	PREL	RI_A	pH	pE	pL/)  {
#		last loop666;
#		}
	}
