...
void q2pt::fillFromRateMatrix(const vector<MDOUBLE>& freq,
	...
	for (int k=0; k < q_sym.size(); ++k) q_sym[k].resize(matrix_size);
	...
	for (int k=0; k < qMatrix.size(); ++k) v[k].resize(matrix_size);
...
#ifdef VERBOS
	cerr<<"The eigen-vector matrix of the decomposition of the symetric matrix\n";
	...
	for (int k=0; k < left_eig_of_pam.size(); ++k) left_eig_of_pam[k].resize(matrix_size);
	VVdouble right_eig_of_pam; // v is the eigen vectors of the symetrical matrix.
	right_eig_of_pam.resize(matrix_size);
	for (int k=0; k < right_eig_of_pam.size(); ++k) right_eig_of_pam[k].resize(matrix_size);
...
//	cerr<<"left_eig_of_pam"<<endl;
//	for (i=0;i<4;++i) {
//		for (j=0; j<4; ++j) {
//			cerr<<left_eig_of_pam[i][j]<<" ";
//			cerr<<pam1[i][i]<<" ";
//		}
//		cerr<<endl;
//	}
//
//	cerr<<"right eig_of_pam"<<endl;
//	for (i=0;i<4;++i) {
//		for (j=0; j<4; ++j) {
//			cerr<<right_eig_of_pam[i][j]<<" ";
//			cerr<<pam1[i][i]<<" ";
//		}
//		cerr<<endl;
//	}
//
//	cerr<<"press anykey"<<endl;
//	char lll;
//	cin>>lll;
...
#ifdef VERBOS
	cerr<<" priting the 4*4 top-left corner of the 1pam matrix * 10^6 "<<endl;
	for (int a=0; a < 4; ++a) {
		for (int b=0; b < 4; ++b) {
			cerr<<pam1[a][b]*1000000.0<<"   ";
		}
		cerr<<endl;
	}
#endif
