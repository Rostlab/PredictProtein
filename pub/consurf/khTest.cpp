...
void makekhTest(const VVdouble & likelihoodVal, MDOUBLE diffNumOfFreeParam) {
	...
	cerr<<" L1= "<<tmp1a<<" L2= "<<tmp1b<<endl;
	cerr<<" delta L is "<<difL<<endl;
	cerr<<" delta AIC is "<<difL-diffNumOfFreeParam<<endl;
	cerr<<" var is "<<sum_k<<endl;;
	cerr<<" std is "<<sqrt(sum_k)<<endl;
	cerr<<" z is "<<(difL -diffNumOfFreeParam )/sqrt(sum_k)<<endl;
