...
sequenceContainer1G phylipFormat::read(istream &infile, const alphabet* alph){
	...
	for (; currentLinePosition != seqFileData.end() ; ) {
		...
		string name;
		sequence seq;
		seq.setAlphabet(alph);


		
		if (mySeqData.numberOfSequences() < f_numSeq ) {//get from the line a name and a sequence;
			...
			for (; it2 != (currentLinePosition)->end() && 
			       mySeqData[sequenceId].size() <f_seqLength;++it2) {
				if ((*it2)==' ') continue;
				else stringSeq1+=(*it2);
...
void phylipFormat::write(ostream &out, const sequenceContainer1G& sd,
						 const int numOfPositionInLine,
						 const int spaceEvery) {
	sequenceContainer1G::constTaxaIterator it5=sd.constTaxaBegin();
	...
	//	vector<const sequenceContainer1G::sequenceDatum*> vec;
	//	sd.getSequenceDatumPtrVector(vec);
	out<<sd.numberOfSequences()<<"   "<<sd.seqLen();
