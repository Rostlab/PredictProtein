...
tree NJalg::computeNJtree(VVdouble distances,const vector<string>& names){
	...
	resTree.getAllLeaves(currentNodes,resTree.iRoot());
...
tree NJalg::startingTree(const vector<string>& names) {
	...
	for (int k=0 ; k < names.size(); ++k) {
		tree::nodeP tmpNode;
		tmpNode = et.createNode(et.iRoot(),et.iNodes());
...
void NJalg::updateBranchDistance(const VVdouble& distanceTable,
	...
	if (DisI_new<0) DisI_new=0; // no negative..
	if (DisJ_new<0) DisJ_new=0; // no negative..
...
void NJalg::NJiterate(tree& et,
	...
	int minRaw,minCol;
		calc_M_matrix(currentNodes,distanceTable,rMatrix,minRaw,minCol);//CHECK3
...
tree::nodeP NJalg::SeparateNodes(tree& et, tree::nodeP node1,
	...
	tree::nodeP theNewNode = et.createNode(fatherNode,et.iNodes());
	node1->setFather(theNewNode);
	theNewNode->sons.push_back(node1);
	node2->setFather(theNewNode);
	theNewNode->sons.push_back(node2);

	// remove from son list of father node.

	vector<tree::nodeP>::iterator vec_iter1=remove(
		fatherNode->sons.begin(),fatherNode->sons.end(),node1 );

	fatherNode->sons.erase(vec_iter1,fatherNode->sons.end());

	vector<tree::nodeP>::iterator vec_iter2=remove(
		fatherNode->sons.begin(),fatherNode->sons.end(),node2 );

	fatherNode->sons.erase(vec_iter2,fatherNode->sons.end());
...
void NJalg::update3taxaLevel(VVdouble& distanceTable,Vdouble & r_values,
	...
	if (dis0root<0) dis0root=0; // no negative..
	if (dis1root<0) dis1root=0; // no negative..
	if (dis2root<0) dis2root=0; // no negative..
...
void NJalg::GetDisTable(const sequenceContainer1G& sd,const vector<MDOUBLE>  * weights) {
...
//CHECK5
//_myET->iRoot()->SetName("RootOfStar");		
	
//CHECK6
//	et.output(cout,et.iRoot(),tree::ANCESTOR);
