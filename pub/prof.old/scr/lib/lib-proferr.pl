...
sub hsspRdSeqSecAcc {
    ...
    $[ =1 ;
#-------------------------------------------------------------------------------
#   hsspRdSeqSecAcc             reads HSSP file
#                               NOTE: chain breaks are skipped!!
#                               
#       in:                     $fileInLoc:  DSSP file
#       in:                     $chnInLoc:   chain to read (' ' if all)
...
#-------------------------------------------------------------------------------
    $tmp=$0;$tmp=~s/^.*\/|\.pl//g;$tmp.=":";
    $sbrName=$tmp."hsspRdSeqSecAcc";
    $fhinLoc="FHIN_"."hsspRdSeqSecAcc";$fhoutLoc="FHOUT_"."hsspRdSeqSecAcc";
				# ------------------------------
				# check arguments
    return(&errSbr("not def fileInLoc!"))     if (! defined $fileInLoc);
    return(&errSbr("not def chnInLoc!"))      if (! defined $chnInLoc);
	...
	++$ctline;
				# stop header
	last if ($_=~/^\s*NOTATION/);

	$line=$_; $line=~s/\n//g;

	if    ($line=~/^\s*SEQLENGTH\s+(\d+)/){
	    $tmp{"NROWS"}= $1;
	    $tmp{"nres"}=  $tmp{"NROWS"};
	    next; }
	elsif ($line=~/^\s*NCHAIN\s+(\d+)/){
	    $tmp{"nchn"}=  $1;
	    last;}
    }
				# wind forward
	...
	$line=          substr($line,1,40);
	if (length($line)<40){
	    print "xx lineno=$ctline, line=$line\n";
	    print "xx file=$fileInLoc\n";
	    die;}

	$chn=           substr($line,13,1);

				# skip since chain not wanted?
	next if ($chnInLoc ne " " && 
		 $chn ne $chnInLoc);

	$tmp2{"seq"}=   substr($line,15,1);
				# skip chain breaks
	next if ($tmp2{"seq"} eq "!");

	$tmp2{"sec"}=   substr($line,18,1);$tmp2{"sec"}=~s/ /L/;
	$tmp2{"acc"}=   substr($line,37,3);$tmp2{"acc"}=~s/\s//g;
	++$ctres;
	foreach $kwd (@kwdTmp){
	    $tmp{$ctres,$kwd}=$tmp2{$kwd};
	}
