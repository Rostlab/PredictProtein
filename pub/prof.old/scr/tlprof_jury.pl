...
$scrGoal="compiles jury for PHD.rdb files.\n".
    "     \t \n".
    "     \t ";
# 
# 
...
$[ =1 ;				# count from one
				...
				# find library
$dirScr=$0; $dirScr=~s/^(.*\/)[^\/]*$/$1/;
$lib="lib-phd2000.pm"; $Lok=0;
foreach $dir ($dirScr,"","pack/","/nfs/data5/users/ppuser/server/pub/phd/scr/","/nfs/data5/users/ppuser/server/pub/phd/scr/pack/"){
    next if (! -e $dir.$lib);
    $Lok=require($dir.$lib); 
    last if ($Lok);}
die("*** ERROR $scrName: could NOT find lib=$lib!\n".
    "*** NOTE: try to run with full path!\n") if (! $Lok);
				# ------------------------------
				# ini
...
#    print "--- $scrName: working on fileIn=$fileIn!\n";
    ...
    ($Lok,$msg)=
	&convPhd
	    ($fileIn,0,$fileOut,$modeWrt,$par{"nresPerRow"},
	     $par{"riSubAcc"},$par{"riSubHtm"},$par{"riSubSec"},$par{"riSubSym"},
	     $par{"txtPreRow"},$par{"debug"}
	     ); die("*** ERROR $scrName: failed on convPhd for file=$fileInLoc\n".
