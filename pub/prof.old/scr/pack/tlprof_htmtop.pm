...
package tlprof_htmtop;

#===============================================================================
sub htmtop {
#-------------------------------------------------------------------------------
#   htmtop                  package version of script
#-------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
	...
	$file_out=~s/\.rdb.*$/.dat/;
	open($fhout, ">".$file_out) || die("*** ERROR $0 failed to open fileOut=$file_out!\n");
	print $fhout "# statistics for R and K in observed loops\n";
				# set zero
	foreach $desx("odd","even"){
	    foreach $aa (@des_20aa,"nres","npos"){
		$stat_ave{$desx,$aa}=0;}}
	...
	if ($par{"do_profile"}) {
		...
		print 
                    "\n","\n","\n",
		    "-*- WARN htmtop: could not find HSSP file ($file_hssp) -> no profile mode\n",
...
#		exit if (defined $ENV{'USER'} && $ENV{'USER'} eq "rost");
	...
	if ($#file_in>1) {	# new name for output file
	    ...
	    $file_out_rdb=$file_out;}
				# no output written for analysis of statistics
	if ($par{"mode"} ne "obs" || ! $Lprof){
	    open($fhout, ">".$file_out_rdb)  || 
		die ("*** ERROR $0 failed to open fileOutRdb=$file_out_rdb!\n");
...
#--------------------------------------------------------------------------------
    ...
    if ($#ARGV < 1){		# help
	print  "goal: $scrGoal\n";
	print  "use:  '$scrName file_phd_rdb'  (or list, for profiles: HSSP recognised by extension!)\n";
	print  "opt:  \n";
	...
	printf "%5s %-15s  %-10s %-s\n","","prof",   "no value","write different column names";
...
#-------------------------------------------------------------------------------
    $sbrName="$scrName:"."iniDef";
				...
				# --------------------
				# executables
    $par{"exe_htmref"}=         "/nfs/data5/users/ppuser/server/pub/prof/scr/tlprof_htmref.pl";
...
#    $par{"dir_hssp"}=           "/home/rost/data/hsspMembrane/";
    ...
    $par{"ext_rdb_in"}=         "unk";
    $par{"ext_rdb_ref"}=        "unk"; 
    $par{"ext_rdb_top"}=        "_refp.rdb_htmtop"; 
...
#----------------------------------------------------------------------
	...
	if ( $ct >= 3 || ($ct==2 && $_!~/\t\d+[NSF]|\d+[NSF]\t/)) {
		...
		for ($it=1;$it<=$#tmpar;++$it) {
		    $readnum[$it]=$it;}
		for ($it=1;$it<=$#tmpar;++$it) {
		    $READCOL[$it]=""; } }
	    
	    for ($it=1; $it<=$#readnum; ++$it) {
		$tmp_name=$tmpar[$readnum[$it]];
		$tmp_name=~s/\s|\n//g;
		$READNAME[$it]=$tmp_name;} }
				# ------------------------------
				# column format
	elsif ( $ct==2 && $_=~/\t\d+[NSF]|\d+[NSF]\t/) {
	    @tmpar=split(/\t/,$_);
	    for ($it=1; $it<=$#readnum; ++$it) {
		$ipos=$readnum[$it];
		$tmp=$tmpar[$ipos]; 
...
sub htmref_rd {
    ...
    $[ =1 ;
#--------------------------------------------------------------------------------
#   htmref_rd                   reads the PHD_PROFhtm RDB files
#--------------------------------------------------------------------------------
				# merely read files
    $#des_prd=0;
    @des_rd=("AA");
    ...
    %rd=&rd_rdb_associativeHere($file_in,"not_screen","header",@header,"body",@des_rd);
				# compose strings
    @des_out=@des_rd;
    foreach $des (@des_out){
	if ($Lprof && defined $kwdProf{$des}){
	    foreach $it (1..$rd{"NROWS"}){
		$rd{$des,$it}=$rd{$kwdProf{$des},$it};
	    }
	}
	$res{$des}="";
	foreach $it (1..$rd{"NROWS"}){
...
sub wrt_rdb_htmtop {
    ...
    $[ =1 ;
#--------------------------------------------------------------------------------
#   writes the RDB format written by PHD_PROF
#--------------------------------------------------------------------------------
    $fhin="FHIN_WRT";
				# read observed topology
    $top_obs="unk";
    $top_obs=
	&rd_top_obs($par{"file_obstop"},$file_in) if (-e $par{"file_obstop"});
    ...
    open($fhin,$file_in) || die("*** ERROR $0:wrt_rdb_htmtop failed to open file_in=$file_in!\n");
	...
	last if ($_!~/^\#/);
	$Lok=1;
	if ( $ct<5 && ($_=~/^\#\s*(PHD|PROF)/i) ) {
	    $Lok=0;		# change header
	    print $fhout 
		"\# PROFhtm_top: prediction of topology ",
		"for helical transmembrane proteins\n"; }
	elsif ($_=~/\# NOTATION/ && $Lnotation_fst) {
	    $Lnotation_fst= 0;
	    $Lnotation_done=1;
				# write new remaining new header 
	    ($Lok,$msg)=
		&wrt_rdb_htmtop_notation
		    ();		if (! $Lok){ print "*** ERROR wrt_rdb_htmtop: failed on notation!\n";
					     print "*** message=$msg\n";
					     exit;}}
				# add notations
	if    ($Lnotation_done && ! $Lnotation_fst && length($_)<4 && ! $Lprof) {
	    print $fhout 
	...
	elsif ($Lnotation_done && ! $Lnotation_fst && length($_)<4 && $Lprof) {
	    print $fhout 
		"# NOTATION PTMLA: predicted model for maximal charge difference (all)\n",
		"# NOTATION PTMLS: predicted model for maximal charge difference (sep)\n",
		"# NOTATION PiTo : predicted topology of transmembrane regions\n",
		"# NOTATION PiTo :    i=loop inside, T=transmembrane, o=loop outside\n";
	}
				# correct (hacks)
	...
	while (<$fhin>) {
	    ...
	    print $fhout "1S\n";
	    last;}}
    $tmp=$res{"top_fin","string","all"};
    $tmp=$res{"top_fin","string"} if (defined $res{"top_fin","string"});

#    $tmp=~s/T/H/g;		# replace "T" -> "H"
    $tmp=~s/ /i/g;		# replace " " -> "i"
    $#top_fin=0;@top_fin=split(//,$tmp);
    if ($par{"mode"} eq "top"){
	$tmp=       $res{"top_fin","mod_top","all"};
	$#prd_topa= 0;
	@prd_topa=  split(//,$tmp);
	$tmp=       $res{"top_fin","mod_top","sep"};
	$#prd_tops= 0;
	@prd_tops=  split(//,$tmp);
    }
	...
	printf $fhout 
	    "%1s$sep_in%1s$sep_in",
	    $prd_topa[$ct],$prd_tops[$ct] if ($par{"mode"} eq "top");
	$tmp="?";
	$tmp=$top_fin[$ct] if (defined $top_fin[$ct]);
	printf $fhout 
	    "%1s\n",$tmp;
    }
}				# end wrt_rdb_htmtop
...
#===============================================================================
sub wrt_rdb_htmtop_notation {
#    local($fhout) = @_ ;
    local($sbrName,$fhinLoc,$fhoutLoc,$tmp);
    $[ =1 ;
#-------------------------------------------------------------------------------
#   wrt_rdb_htmtop_notation                       
#       in:                     $fileInLoc
...
#-------------------------------------------------------------------------------
    ...
    $sbrName=$tmp."wrt_rdb_htmtop_notation";
    ...
    return(1,"ok $sbrName");
}				# end of wrt_rdb_htmtop_notation
