...
#-------------------------------------------------------------------------------
    $sbrName="$scrName".":ini";
    $par{"dirPerl"}=            "/home/$ENV{USER}/server/pub/phd/scr/"; # directory for perl scripts needed
				# ------------------------------
...
# 	$dir="//home/$ENV{USER}/server/pub/perl/" if (! defined $dir || ! -d $dir);
# 	$dir.="/"               if ($dir !~/\/$/);
# 	$dir=""                 if (! -d $dir);}
#     else {
# 	$dir=$par{"dirPerl"};}
...
#  	die("*** $scrName: failed to require perl library '$lib' (dir=$dir)\n") if (! $Lok);}
    ...
    $tmp{"scrNameFull"}=$0;
				# special help
    $tmp{"scrAddHelp"}= "";
    $tmp{"special"}=    "table,list,red,curve,exe,minLen,len,";
...
#-------------------------------------------------------------------------------
				...
				# --------------------
				# directories
    $par{"dirHome"}=            "/home/$ENV{USER}/server/pub/";
    ...
    $par{"dirBin"}=             $par{"dirHome"}. "bin/";   # FORTRAN binaries of programs needed
				...
				# <<<<<<<<<<<<<<<<<<<<
				# for porting PHD asf
    $par{"dirSrcMat"}=          "/home/$ENV{USER}/server/pub/phd/". "mat/";   # general material
    $par{"dirPerl"}=            "/home/$ENV{USER}/server/pub/phd/". "scr/";   # perl libraries
    $par{"dirPerlScr"}=         "/home/$ENV{USER}/server/pub/phd/". "scr/";   # perl scripts needed
    $par{"dirBin"}=             "/home/$ENV{USER}/server/pub/phd/". "bin/";   # FORTRAN binaries of programs needed
    ...
    $par{"dirHssp"}=            "//home/$ENV{USER}/server/pub/data/hssp/,/data/hssp/";
...
#    $par{"exeFilterHssp"}=      "//home/$ENV{USER}/server/pub/lion/src-lib/filter_hssp".".".$ARCH; # FORTRAN excutable
}				# end of iniDef
...
#-------------------------------------------------------------------------------
    ...
    return($archFound)          if ($archFound);
				...
				# ------------------------------
    foreach $possible ("/home/$ENV{USER}/server/pub/phd/scr/pvmgetarch.sh",
		       "//home/$ENV{USER}/server/pub/etc/pvmgetarch.sh") {
...
#-------------------------------------------------------------------------------
    @tmp=("//home/$ENV{USER}/server/pub/perl/ctime.pl","/home/$ENV{USER}/server/pub/perl/ctime.pl");
				# ------------------------------
				# get function
...
#-------------------------------------------------------------------------------
    ...
    return(0,"*** $sbrNamex: not def fhSbr!")                  if (! defined $fhSbr);
				...
				# ------------------------------
				# default settings
    $exeFilterHssp="/home/$ENV{USER}/server/pub/bin/filter_hssp98.".$ARCH   if ($exeFilterHssp eq "def");
    $fileMatGcg=   "/home/$ENV{USER}/server/pub/lib/mat/Maxhom_GCG.metric"  if ($fileMatGcg eq "def");
