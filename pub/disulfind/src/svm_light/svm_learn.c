...
void svm_learn_classification(DOC *docs, double *class, long int
     ...
     /* class:       Training labels (y-part, zero if test example for transduction) */
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  {  /* default value for C */
    ...
    if(verbosity>=1) printf("Setting default regularization parameter C=%.4f\n",learn_parm->svm_c);
  }
  ...
  for(i=0;i<totdoc;i++)
  {
    ...
    if(class[i] == 0) /* neither positive or negative example */
	{
      unlabeled[i]=1;
      label[i]=0;
      transduction=1;
    ...
    if(class[i] > 0) /* positive example */
	{
	  /* individual upper bounds for each var = upper bound C on alphas * factor to multiply C for positive examples * cost factor */
      learn_parm->svm_cost[i] = learn_parm->svm_c * learn_parm->svm_costratio * docs[i].costfactor;
      label[i]=1;
      trainpos++;
    }
    else if(class[i] < 0) /* negative example */
	{
	  /* individual upper bounds for each var = upper bound C on alphas * cost factor */
  ...
  if(verbosity==1) { printf("Optimizing"); fflush(stdout); }
  ...
  if(verbosity>=1)
  {
    ...
    for(i=0;(i<totdoc);i++)
	{ /* get final statistic */
      if((lin[i]-model->b)*(double)label[i] <= 0.0)	misclassified++;
    }
    ...
    if(learn_parm->remove_inconsistent) {
      inconsistentnum=0;
      for(i=0;i<totdoc;i++)
		if(inconsistent[i]) inconsistentnum++;
      printf("Number of SV: %ld (plus %ld inconsistent examples)\n", model->sv_num-1,inconsistentnum);
    }
    else {
      upsupvecnum=0;
      for(i=1;i<model->sv_num;i++) {
		if(fabs(model->alpha[i]) >= (learn_parm->svm_cost[(model->supvec[i])->docnum]-learn_parm->epsilon_a)) upsupvecnum++;
      }
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  docs_org=docs; /* original documents */
  ...
  for(i=0;i<totdoc;i++) {
    ...
    j=2*totdoc-1-i; /* go from 2*totdoc-1 to totdoc */
  ...
  totdoc*=2; /* Number of examples in docs/label is twice the old one */
  ...
  if(learn_parm->alphafile[0]) write_alphas(learn_parm->alphafile,a,label,totdoc);

  /* this makes sure the model we return does not contain pointers to the temporary documents */
  for(i=1;i<model->sv_num;i++) {
...
void svm_learn_ranking(DOC *docs, double *rankvalue, long int totdoc,
		       ...
		       MODEL *model)
     /* docs:        Training vectors (x-part) */
     /* rankvalue:   Training target values (y-part) that determine the ranking */
     /* totdoc:      Number of examples in docs/label */
     /* totwords:    Number of features (i.e. highest feature index) */
     /* learn_parm:  Learning paramenters */
     /* kernel_parm: Kernel paramenters */
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  DOC *docdiff; 	/* training set of pairs generated by original training set */
  long totpair; 	/* number of pairs generated by training set */
  double *target;	/* target of pairs generated by training set: -1 or +1 */
  double *alpha;
  long *greater,*lesser;
  MODEL pairmodel;
  long i,j,k,z;
  ...
  totpair=0; /* number of pairs generated from training set */
  for(i=0;i<totdoc;i++)
  {
  ...
  docdiff=(DOC *)my_malloc(sizeof(DOC)*totpair); /* training set of pairs generated by original training set */
  target=(double *)my_malloc(sizeof(double)*totpair); /* target of pairs generated by training set: -1 or +1 */
  greater=(long *)my_malloc(sizeof(long)*totpair);
  lesser=(long *)my_malloc(sizeof(long)*totpair);

  k=0; /* index on the pairs */
  for(i=0;i<totdoc;i++)
  {
    for(j=i+1;j<totdoc;j++)
		{
      if(docs[i].queryid == docs[j].queryid)
	  	{
				if(rankvalue[i] > rankvalue[j]) /* positive pair */
				{
	  			docdiff[k].words=sub_ss(docs[i].words,docs[j].words); /* compute the difference a-b of two sparse vectors */
	  			docdiff[k].twonorm_sq=sprod_ss(docdiff[k].words,docdiff[k].words); /* compute the inner product of two sparse vectors */
	  			docdiff[k].docnum=k; /* Document ID. This has to be the position of the document in the training set array. */
					if (learn_parm->scored_pairs) /* Sauro */
					{
	  				docdiff[k].costfactor=fabs(docs[i].costfactor-docs[j].costfactor);
					}
					else
					{
	  				docdiff[k].costfactor=1;
					}
	  			if (learn_parm->ranking_type) target[k]=1; /* target of pairs generated by training set for ranking classification: -1 or +1 */ /* Sauro */
	  			else target[k]=docs[i].costfactor-docs[j].costfactor; /* target of pairs generated by training set for ranking regression */ /* Sauro */
	  			greater[k]=i;
	  			lesser[k]=j;
	  			k++;

					if(verbosity>=4) /* Sauro */
					{
						printf("\npair[%d] target:%f",k-1,target[k-1]);
						printf(" cost:%f <feature>:<value>",docdiff[k-1].costfactor);
						for (z=0;z<totwords;++z) printf(" %d:%f",(docdiff[k-1].words)[z].wnum,(docdiff[k-1].words)[z].weight);
					}
				}
				else if(rankvalue[i] < rankvalue[j]) /* negative pair */
				{
	  			docdiff[k].words=sub_ss(docs[i].words,docs[j].words); /* compute the difference a-b of two sparse vectors */
	  			docdiff[k].twonorm_sq=sprod_ss(docdiff[k].words,docdiff[k].words); /* compute the inner product of two sparse vectors */
	  			docdiff[k].docnum=k; /* Document ID. This has to be the position of the document in the training set array. */
				...
				else if(rankvalue[i] == rankvalue[j])  /* Sauro */
				{
					/* DO NOTHING: no pairs between examples with the same rank value */
				}
      }
    }
  }
  printf("done.\n"); fflush(stdout);
	...
	/* Sauro */
	if (learn_parm->ranking_type)
	{
		printf("Ranking Classification (the target of the pairs is in {-1,+1})\n");
  	/* must use unbiased hyperplane on difference vectors */
  	learn_parm->biased_hyperplane=0;
  	svm_learn_classification(docdiff,target,totpair,totwords,learn_parm,kernel_parm,NULL,&pairmodel);
	}
	else /* Sauro */
	{
		printf("Ranking Regression (the target of the pairs is a real number computed by the cost of the examples)\n");
  	/* must use unbiased hyperplane on difference vectors */
  	learn_parm->biased_hyperplane=0;
  	svm_learn_regression(docdiff,target,totpair,totwords,learn_parm,kernel_parm,NULL,&pairmodel);
	}

  /* Transfer the result into a more compact model. If you would like to output the original model on pairs of documents, see below. */
     ...
     /* retrain: selects training mode (1=regular / 2=holdout) */
{
  ...
  qp.opt_g = (double *)my_malloc(sizeof(double)*learn_parm->svm_maxqpsize*learn_parm->svm_maxqpsize);
  ...
  for(;retrain;iteration++)
  {
    ...
    if(kernel_cache) kernel_cache->time=iteration;  /* for lru cache */
    if(verbosity>=2)
	{
    ...
    for(jj=0;(j=working2dnum[jj])>=0;jj++) { /* clear working set */
      if((chosen[j]>=(learn_parm->svm_maxqpsize/minl(learn_parm->svm_maxqpsize,learn_parm->svm_newvarsinqp)))
    ...
    if(retrain == 2) {
      ...
      if(iteration % 101) {
        already_chosen=0;
		if((minl(learn_parm->svm_newvarsinqp,learn_parm->svm_maxqpsize-choosenum)>=4) && (kernel_parm->kernel_type != LINEAR)) {
	  		/* select part of the working set from cache */
	  		already_chosen=select_next_qp_subproblem_grad_cache(
			      label,unlabeled,a,lin,c,totdoc,
			      (long)(minl(learn_parm->svm_maxqpsize-choosenum,learn_parm->svm_newvarsinqp)/2),
		...
		choosenum+=select_next_qp_subproblem_grad(
                            ...
                            learn_parm,inconsistent,active2dnum,
			      			working2dnum,selcrit,selexam,kernel_cache,key,chosen);
      }
    ...
    if(verbosity>=2) { printf(" %ld vectors chosen\n",choosenum); fflush(stdout); }

    if(verbosity>=2) t1=get_runtime();

    if(kernel_cache) cache_multiple_kernel_rows(kernel_cache,docs,working2dnum,choosenum,kernel_parm);

    if(verbosity>=2) t2=get_runtime();
    if(retrain != 2) {
      optimize_svm(docs,label,unlabeled,chosen,active2dnum,model,totdoc,
		   working2dnum,choosenum,a,lin,c,learn_parm,aicache,kernel_parm,&qp,&epsilon_crit_org);
    }
    ...
    if((!retrain) && (transduction)) {
      ...
      activenum=shrink_problem(learn_parm,shrink_state,kernel_parm,active2dnum,
			       ...
			       a,inconsistent);
      inactivenum=totdoc-activenum;
      if((kernel_cache) && (supvecnum>kernel_cache->max_elems) && ((kernel_cache->activenum-activenum)>maxl((long)(activenum/10),500))) {
		kernel_cache_shrink(kernel_cache,totdoc,minl((kernel_cache->activenum-activenum),(kernel_cache->activenum-supvecnum)),shrink_state->active);
      }
    }
    ...
    if((!retrain) && learn_parm->remove_inconsistent) {
      ...
      if(learn_parm->remove_inconsistent == 1) {
		retrain=identify_inconsistent(a,label,unlabeled,totdoc,learn_parm,&inconsistentnum,inconsistent);
      }
      else if(learn_parm->remove_inconsistent == 2) {
		retrain=identify_misclassified(lin,label,unlabeled,totdoc,model,&inconsistentnum,inconsistent);
      }
      else if(learn_parm->remove_inconsistent == 3) {
		retrain=identify_one_misclassified(lin,label,unlabeled,totdoc,model,&inconsistentnum,inconsistent);
      }
      if(retrain) {
...
double estimate_r_delta_average(DOC *docs, long int totdoc, KERNEL_PARM *kernel_parm)
{
  long i;
  double avgxlen;
  DOC nulldoc;            /* assumes that the center of the ball is at the */
