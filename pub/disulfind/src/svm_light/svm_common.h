...
typedef struct doc {
  long    docnum;              /* Document ID. This has to be the position of the document in the training set array. */
  long    queryid;             /* for learning rankings, constraints are generated for documents with the same queryID. */
  double  costfactor;          /* Scales the cost of misclassifying this document by this factor. The effect of this
				  				value is, that the upper bound on the alpha for this example is scaled by this factor.
				  				The factors are set by the feature 'cost:<val>' in the training data. Default 1 */
  double  twonorm_sq;          /* The squared euclidian length of the document vector. */
  WORD    *words;              /* The words/values in the document by increasing word-number. */
} DOC;

typedef struct learn_parm {
  long   type;                 /* selects between regression, classification and ranking */
  double svm_c;                /* upper bound C on alphas */
  double eps;                  /* regression epsilon (eps=1.0 for classification */
  double svm_costratio;        /* factor to multiply C for positive examples. Default 1. It must be > 0 */
  ...
  long   skip_final_opt_check; /* do not check KT-Conditions at the end of optimization for examples removed by shrinking. WARNING: This might lead to sub-optimal solutions! */
  ...
  char alphafile[200];         /* file to store optimal alphas in. use empty string if alphas should not be output */
  ...
  long   totwords;             /* number of features */
	...
	/* Sauro */
	int		 scored_pairs;				 /* boolean value; in a ranking model,
																  true if we assign a cost to pairs using cost:<value> of 2 examples which create the pair,
																  false if we assign 1 as a score for each pair
																	Default: false
																	*/

	/* Sauro */
	int		 ranking_type;				 /* boolean value; in a ranking model,
																	the training with the pairs can be made by classification (true) or by regression (false);
																	the target for regression is the difference between the cost:<value> feature 
																	of the examples that build the pair
