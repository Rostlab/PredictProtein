		    ...
		    long int *totwords, long int *totdoc)
{
  ...
  if ((docfl = fopen (docfile, "r")) == NULL) { perror (docfile); exit (1); }
  ...
  while((!feof(docfl)) && fgets(line,(int)ll,docfl)) {
    ...
    if((wpos>1) && ((doc.words[wpos-2]).wnum>(*totwords))) (*totwords)=(doc.words[wpos-2]).wnum;
  ...
  free(line);
  free(doc.words);
  if(verbosity>=1) { fprintf(stdout, "OK. (%ld examples read)\n", dnum); }
  (*totdoc)=dnum;
}

int parse_document(char *line, DOC *doc, double *label, long int *numwords, long int max_words_doc)
{
  ...
  while(((numread=sscanf(line+pos,"%s",featurepair)) != EOF) &&	(wpos<max_words_doc)) {
    /* printf("%s\n",featurepair); */
    while(isspace((int)line[pos])) pos++;
    while((!isspace((int)line[pos])) && line[pos]) pos++;
...
void nol_ll(char *file, long int *nol, long int *wol, long int *ll)
{
  ...
  int ic;
  char c;
  long current_length,current_wol; /* longest line and maximum number of spaces per line */

  if ((fl = fopen (file, "r")) == NULL) { perror (file); exit (1); }

  current_length=0; /* longest line */
  current_wol=0;    /* maximum number of spaces per line */

  (*ll)=0;  /* longest line */
  (*nol)=1; /* number of lines */
  (*wol)=0; /* maximum number of spaces per line */
