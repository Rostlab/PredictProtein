...
void read_input_parameters(int, char **, char *, char *,long *, long *, LEARN_PARM *, KERNEL_PARM *);
void wait_any_key();
void print_help();
...
int main (int argc, char* argv[])
{
  ...
  read_input_parameters(argc,argv,docfile,modelfile,&verbosity,&kernel_cache_size,&learn_parm,&kernel_parm);
  ...
  /* Grep through file 'docfile' and count number of lines, maximum number of spaces per line, and longest line. */
  ...
  { /* don't need the cache */
	...
	{
      svm_learn_classification(docs,target,totdoc,totwords,&learn_parm,&kernel_parm,NULL,&model);
    }
    else if(learn_parm.type == REGRESSION)
	{
      svm_learn_regression(docs,target,totdoc,totwords,&learn_parm,&kernel_parm,NULL,&model);
    }
    else if(learn_parm.type == RANKING)
	{
      svm_learn_ranking(docs,target,totdoc,totwords,&learn_parm,&kernel_parm,NULL,&model);
    }
  }
  else
  {
    if(learn_parm.type == CLASSIFICATION)
	{
      ...
      svm_learn_classification(docs,target,totdoc,totwords,&learn_parm,&kernel_parm,&kernel_cache,&model);
			   ...
			   LEARN_PARM *learn_parm,KERNEL_PARM *kernel_parm)
{
  ...
  learn_parm->scored_pairs=0; /* Sauro */
  learn_parm->ranking_type=1; /* Sauro */
  ...
  for(i=1; (i<argc) && ((argv[i])[0] == '-'); i++)
  {
    switch ((argv[i])[1])
      ...
      case 'y': i++; learn_parm->scored_pairs=atoi(argv[i]); break; /* Sauro */
      case 'w': i++; learn_parm->eps=atof(argv[i]); break;
      case 'z': i++; strcpy(type,argv[i]); break;
      case '@': i++; learn_parm->ranking_type=atoi(argv[i]); break; /* Sauro */
      case '?': print_help(); exit(0);
      default: printf("\nUnrecognized option %s!\n\n",argv[i]); print_help(); exit(0);
      }
  }
  if(i>=argc)
  {
...
void print_help()
{
  ...
  printf("                        of unbiased hyperplane (i.e. x*w>0) (default 1=biased)\n");
  printf("         -i [0,1]    -> remove inconsistent training examples\n");
  printf("                        and retrain (default 0)\n");
  printf("         -y {0,1}    -> in a ranking model, assign a cost to the pairs using the the cost of 2 examples\n"); /* Sauro */
  printf("                        which create the pairs (default 0 => all the pairs have the same cost 1)\n");
  printf("         -@ {0,1}    -> in a ranking model, use classification or regression to learn a model\n"); /* Sauro */
  printf("                        (default 1 => ranking with classification)\n");
