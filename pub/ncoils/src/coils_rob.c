...
/* Program COILS version 2.1 */
  ...
  /* Adapted by Rob Russell (SB) to make it less user friendly
   *  (UNIX command line options replace command line-driven commands) 
   * Also a FASTA output option: */
...
(weightchar!='N')  && (weightchar!=(char) 13))
  ...
  if(verbose) fprintf(pickup,"COILS version 2.1\n");
  if (chartchar=='2')
  {
    if(verbose) fprintf(pickup,"using MTIDK matrix.\n");
    Init_newchart();
  } else {
   if(verbose)  fprintf(pickup,"using MTK matrix.\n");
    Init_oldchart();
  }
  if ((weightchar=='y') || (weightchar=='Y'))
  {
    if(verbose) fprintf(pickup,"weights: a,d=2.5 and b,c,e,f,g=1.0\n");
    hept_weight=10;
    ad_weight=2.5;
  } else {
    if(verbose) fprintf(pickup,"no weights\n");
    hept_weight=7;
    ad_weight=1.0;
  }
  if(verbose) {
	  if(inputfile[0]!='-') fprintf(pickup,"Input file is %s\n",inputfile);
	  else fprintf(pickup,"Reading from standard input\n");
  }
}
void  Select_option()
{
...
void exit_error() {
	fprintf(stderr,"format: coils [options] <  [fasta file]  > [output file]\n");
	fprintf(stderr,"      -mtk (use MTK matrix) \n");
	fprintf(stderr,"      -mtidk (use MTIDK matrix) \n");
	fprintf(stderr,"      -ad2.5  (a weight of 2.5 for positions a & d)\n");
	fprintf(stderr,"      -w <window size> [default is 21]\n");
	fprintf(stderr,"      -pm <minimum probabilty> [default is 0.0]\n");
	fprintf(stderr,"      -v (verbose mode)\n");
	fprintf(stderr,"      -out <char>\n");
	fprintf(stderr,"         p = propabilities in columns \n");
	fprintf(stderr,"         a = probabilities in rows\n");
	fprintf(stderr,"         b = scores (size of the scanning window defined by the user)\n");
	exit(-1);
}
...
void  Get_info(int argc, char *argv[]) 
{
  ...
  /* Modified version of Get_info() 
   * Rob Russell, March 1999 makes commands command line driven */
   ...
   for(i=1; i<argc; ++i) {
      if(argv[i][0]!='-') {
	strcpy(&inputfile[0],argv[i]); 
      } else if(strcmp(&argv[i][1],"v")==0) { 
	  verbose = 1;
      } else if(strcmp(&argv[i][1],"mtk")==0) { 
	  chartchar = '1';
    	  Init_oldchart();
      } else if(strcmp(&argv[i][1],"mtidk")==0) {
	  chartchar = '2';
    	  Init_newchart();
      } else if(strcmp(&argv[i][1],"ad2.5")==0) {
	  weightchar = 'y';
    	  hept_weight=10;
	  ad_weight=2.5;
      } else if(strcmp(&argv[i][1],"pm")==0) {
	  if((i+1)>=argc) { exit_error(); }
	  sscanf(argv[i+1],"%f",&peakmin);
	  i++;
	  ...
	  if((i+1)>=argc) { exit_error(); }
	  option = argv[i+1][0];
	  if((option!='p') && (option!='a') && (option!='b') && (option!='c')) {
		exit_error();
	  }
	  i++;
      } else {
  ...
  if(inputfile[0]!='-') {
     ...
     if(verbose) printf("Input file is: %s\n",inputfile);
  } else {
     protein = stdin;
     if(verbose) printf("Reading from standard input\n");
  }
  pickup = stdout;
  if(verbose) fprintf(pickup,"COILS version 2.1\n");
  if (chartchar=='2')
