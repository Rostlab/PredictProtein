...
C     zscore
      REAL            SDEV,CURT,SKEW,VAR,ADEV,AVE,STMP,SUM
      REAL            HIGHZ,LOWZ
...
C if format is 'UNK' assume its a list of filenames
     ...
     +              (INDEX(TEMPNAME,'LIST') .NE. 0) ) THEN
               ...
               IF ( INDEX(TEMPNAME,'PROFILE') .NE. 0) THEN
                  ...
                  LPROFILE_2=.TRUE.
               ENDIF
               IF ( INDEX(TEMPNAME,'SS-SA') .NE.0 .OR.
     +              INDEX(TEMPNAME,'SSSA') .NE.0 ) THEN
                  LPROF_SSSA_2=.TRUE.
C     profile is sssa type and not regular maxhom profile, thus (D.P.) 
                  LPROFILE_2=.FALSE.
                  LDSSP_2=.FALSE.
     ...
     +              (LPROF_2 .EQV. .TRUE.)) THEN
            ...
            IF (INDEX(SEQFORMAT,'PROFILE') .NE. 0)THEN
               LPROFILE_2=.TRUE.
            ENDIF
            IF ( INDEX(SEQFORMAT,'SS-SA') .NE.0 .OR.
     +           INDEX(SEQFORMAT,'SSSA') .NE.0 ) THEN
               LPROF_SSSA_2=.TRUE.
C     profile is sssa type and not regular maxhom profile, thus (D.P.) 
               LPROFILE_2=.FALSE.
               LDSSP_2=.FALSE.
     ...
     +           INDEX(SEQFORMAT,'PROFILE-DSSP') .NE.0) THEN
            ...
            ENDIF
         ENDIF
         WRITE(6,*)' : LDSSP_1 ', LDSSP_1
         WRITE(6,*)' : LISTOFSEQ_1 ', LISTOFSEQ_1
         WRITE(6,*)' : LPROFILE_1 ', LPROFILE_1
         WRITE(6,*)' : LPROF_1 ', LPROF_1
         WRITE(6,*)' : LPROF_SSSA_1 ', LPROF_SSSA_1

         WRITE(6,*)' : LDSSP_2 ', LDSSP_2
         WRITE(6,*)' : LISTOFSEQ_2 ', LISTOFSEQ_2
         WRITE(6,*)' : LPROFILE_2 ', LPROFILE_2
         WRITE(6,*)' : LPROF_2 ', LPROF_2
         WRITE(6,*)' : LPROF_SSSA_2 ', LPROF_SSSA_2
...
c	  WRITE(logstring,'(a)')'second profile will be ignored '
         ELSE IF (.NOT.LPROFILE_1 .AND. (LPROFILE_2) ) THEN
            PROFILEMODE=2
         ELSE IF ( LPROFILE_1 .AND. .NOT. LPROFILE_2) THEN
            PROFILEMODE=1
         ELSE IF ( LPROF_SSSA_1 .AND. LDSSP_2 .OR. 
     +             LPROF_SSSA_1 .AND. LISTOFSEQ_2 .AND.
     +           .NOT. LPROF_SSSA_2 ) THEN 
            WRITE(*,*)' info: profilemode equals 7: '
            PROFILEMODE=7
         ELSE IF ( LPROF_SSSA_2 .AND. LDSSP_1 ) THEN 
            WRITE(*,*)' info: profilemode equals 8: '
            PROFILEMODE=8
         ELSE IF ( LPROF_SSSA_1 .AND. LPROF_SSSA_2 ) THEN 
            WRITE(*,*)' info: profilemode equals 9: '
            PROFILEMODE=9
         ELSE
            PROFILEMODE=0
         ENDIF
...
C--------------------------------------------------------------------
     ...
     +              .OR. (LPROFILE_2 .EQV. .TRUE.) .OR.
     +              (LPROF_2.EQV. .TRUE.) .OR.
     +              (LPROF_SSSA_1 .EQV. .TRUE.) .OR.
     +              (LPROF_SSSA_2 .EQV. .TRUE.) ) THEN
                  METRICFILE='PROFILE'
               ELSE
                  WRITE(LOGSTRING,'(A)')
...
C--------------------------------------------------------------------
     ...
     +               ' 0.0     : NO scaling'
            ...
            ENDIF
         ENDIF
         IF(LSCALE_PROFILE)WRITE(6,*)' SCALING PROFILE ENABLED'
...
C--------------------------------------------------------------------
     ...
     +	  '              rescale Profile-metric '
         ...
         IF( INDEX(SMAX_ANSWER,'IGNORE') .EQ. 1 .OR.
     +       INDEX(SMIN_ANSWER,'IGNORE') .EQ. 1 )THEN
            LSCALE_PROFILE=.FALSE.
            WRITE(6,*)'INFO: PROFILESCALING DISABLED'
         ENDIF
C--------------------------------------------------------------------
...
C--------------------------------------------------------------------
     ...
     +        FILTER_VAL)
         CALL STRPOS(HIGHZ_ANSWER,IBEG,IEND)
         CALL READ_REAL_FROM_STRING(HIGHZ_ANSWER(IBEG:IEND),
     +        HIGHZ)
         CALL STRPOS(LOWZ_ANSWER,IBEG,IEND)
         CALL READ_REAL_FROM_STRING(LOWZ_ANSWER(IBEG:IEND),
     +        LOWZ)
C--------------------------------------------------------------------
...
CAUTION LSTRUC IS NOT JUST 3 STATES
     ...
     +           N1,LSQ_1,LSTRCLASS_1,NSURF_1,LACC_1)
         ...
         ELSE
            ...
            ENDIF
         ENDIF
         WRITE(6,*)'INFO: LPROFILE_1= ',LPROFILE_1
c=======================================================================
...
C scale profile_1
            ELSE
C               WRITE(6,*)' call scale_profile enabled 1'
             IF(LSCALE_PROFILE .EQV. .TRUE. )THEN 
                IF(LPROFILE_1)THEN  
                   WRITE(6,'(a,4(2x,f5.2)))')' CALL SCALE_PROFILE 1',
     +                  smin,smax,maplow,maphigh
                   call scale_profile_metric(maxsq,ntrans,trans,
     +                  simmetric_1,smin,smax,maplow,maphigh)
                ELSEIF(LPROF_SSSA_1)THEN
                   WRITE(6,*)'INFO: call scale_sssa_profile_metric',
     +                  SMIN,SMAX,MAPLOW,MAPHIGH
                   CALL SCALE_SSSA_PROFILE_METRIC(MAXSQ,NTRANS,
     +                  NSTRUCTRANS,NACCTRANS,TRANS,SIM_SSSA_METRIC_1,
     +                  SMIN,SMAX,MAPLOW,MAPHIGH)
                ELSE
                   WRITE(6,*)'ERROR not know how to scale this profile'
                   STOP
                ENDIF
             ENDIF
            ENDIF
...
C     profile is sssa type and not regular maxhom profile, thus (D.P.) 
     ...
     +                    LDSSP_2=.TRUE.
                     IF (LPROFILE_2 .OR. LPROF_SSSA_2 ) THEN
     ...
     +                          SIMMETRIC_2,MAXBOX,NBOX_2,PROFILEBOX_2)
                        ELSEIF(LPROF_SSSA_2)THEN
                           CALL READ_SSSA_PROFILE(KPROF,NAME_2,MAXSQ,
     +                          NTRANS,TRANS,NSTRUCTRANS,NACCTRANS,
     +                          LDSSP_2,N2IN,NCHAIN,HSSPID_2,HEADER_2,
     +                          COMPND_2,
     +                          SOURCE_2,AUTHOR_2,XSMIN,XSMAX,XMAPLOW,
     +                          XMAPHIGH,PROFILEMETRIC,PDBNO_2,
     +                          CHAINID_2,
     +                          CSQ_2_ARRAY,STRUC_2,NSURF_2,COLS_2,
     +                          SHEETLABEL_2,BP1_2,BP2_2,NOCC_2,
     +                          GAPOPEN_2,
     +                          GAPELONG_2,CONSWEIGHT_2,
     +                          SIM_SSSA_METRIC_2,
     +                          MAXBOX,NBOX_2,PROFILEBOX_2) 
                        ENDIF
...
C reset conservation weights for sequence 2 if not wanted
     ...
     +                             MAPHIGH,GAPOPEN_2,GAPELONG_2,SDEV)
                           ELSEIF(LPROF_SSSA_2)THEN
                              WRITE(6,*)'ERROR: norming SSSA profile',
     +                             'not ready yet'
                           ENDIF
                        ELSEIF(LSCALE_PROFILE)THEN
                           IF(LPROFILE_2)THEN  
                              WRITE(6,'(a,4(2x,f5.2)))')
     +                             'CALL SCALE_PROFILE_METRIC ',
     +                             smin,smax,maplow,maphigh
                              CALL SCALE_PROFILE_METRIC(maxsq,ntrans,
     +                             trans,simmetric_2,smin,smax,
     +                             maplow,maphigh)
                           ELSEIF(LPROF_SSSA_2)THEN
                              WRITE(6,*)'',
     +                             'INFO: SCALE_SSSA_PROFILE_METRIC',
     +                             SMIN,SMAX,MAPLOW,MAPHIGH
                              CALL SCALE_SSSA_PROFILE_METRIC(MAXSQ,
     +                             NTRANS,NSTRUCTRANS,NACCTRANS,TRANS,
     +                             SIM_SSSA_METRIC_2,SMIN,SMAX,
     +                             MAPLOW,MAPHIGH)
                           ENDIF
C                           WRITE(6,*)' call scale_profile disabled 2'
...
c	  close(kdeb)
     ...
     +           CURT)
            ...
            ENDDO
c recalculate zscore without too high and too low values
            DO I=1,3
               SUM=0.0
               PNALIGN=0
               VAR=0.0
               DO J=1,NALIGN
                  IF( (ZSCORE_TEMP(J) .LT. HIGHZ) .AND.
     +                 (ZSCORE_TEMP(J) .GT. LOWZ) ) THEN
                     SUM = SUM + ALISORTKEY(J)
                     PNALIGN = PNALIGN + 1
                  ENDIF
               ENDDO
               AVE = SUM / PNALIGN
               DO J=1,NALIGN
                  IF( (ZSCORE_TEMP(J) .LT. HIGHZ) .AND.
     +                 (ZSCORE_TEMP(J) .GT. LOWZ) ) THEN
                     STMP= (ALISORTKEY(J) - AVE) * (ALISORTKEY(J) - AVE)
                     VAR = VAR + STMP
                  ENDIF
               ENDDO
               VAR=VAR/(PNALIGN -1)
               SDEV=SQRT(VAR)
               DO J=1,NALIGN
                  ZSCORE_TEMP(J)=(ALISORTKEY(J) - AVE) / SDEV
               ENDDO
            ENDDO
...
c	          CALL LOWTOUP(CFIELD(ID+1),ILEN)
     ...
     +                 INDEX(CFIELD(ID),'OUT')  .NE.0 ) THEN
	          ...
	          CALL COPY_FIELD(CFIELD,FILTER_ANSWER,ID,NFIELD)
               ...
               ELSE IF (INDEX(CFIELD(ID),'HIGHZ').NE.0) THEN
	          CALL COPY_FIELD(CFIELD,HIGHZ_ANSWER,ID,NFIELD)
               ELSE IF (INDEX(CFIELD(ID),'LOWZ').NE.0) THEN
	          CALL COPY_FIELD(CFIELD,LOWZ_ANSWER,ID,NFIELD)   
     ...
     +                 FILTER_ANSWER(IBEG:IEND),' [number]'
                  ...
                  CALL STRPOS(LOWZ_ANSWER,IBEG,IEND)
                  WRITE(6,*)' LOWZ       : ',
     +                 LOWZ_ANSWER(IBEG:IEND),' [number]'
                  CALL STRPOS(HIGHZ_ANSWER,IBEG,IEND)
                  WRITE(6,*)' HIGHZ       : ',
     +                 HIGHZ_ANSWER(IBEG:IEND),' [number]'
...
C call MAXHOM 
     ...
     +     FILTER_ANSWER(I:J)//LINE
      ...
      CALL STRPOS(LOWZ_ANSWER,I,J)
      WRITE(KCOM,'(A)')'   echo "LOWZ '//
     +     LOWZ_ANSWER(I:J)//LINE
      CALL STRPOS(HIGHZ_ANSWER,I,J)
      WRITE(KCOM,'(A)')'   echo "HIGHZ '//
     +     HIGHZ_ANSWER(I:J)//LINE
