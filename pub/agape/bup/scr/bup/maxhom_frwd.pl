...
($fileIn,$fileDb,$fileHsspOut,$fileStripOut,$dirWork,$ZeroShift,$GO,$GE,$maxhom_exe,$maxhomDefault,$dbg)=@ARGV;

%h_args=( "fileIn" => $fileIn,
	  "fileDb" => $fileDb,
	  "fileHsspOut" => $fileHsspOut,
	  "fileStripOut" => $fileStripOut,
	  "dirWork" => $dirWork,
	  "ZeroShift" => $ZeroShift,
	  "GO" => $GO,
	  "GE" => $GE,
	  "maxhom_exe"   => $maxhom_exe,
	  "maxhomDefault" => $maxhomDefault
	  );


foreach $it (sort keys %h_args){
    die "argument $it not defined, stopped" if(! defined $h_args{$it} );
}
...
($Lok,$msg)=&run_maxhom($fileIn,$fileDb,$fileHsspOut,$fileStripOut,$ZeroShift,$GO,$GE,$maxhom_exe,$maxhomDefault,$dbSize,$dbg);
...
#	if($error);
	...
	if(! $dbg);
#    system("gzip $StripFile $HsspFile");
    return(1,"ok");
}
#============================================================================
