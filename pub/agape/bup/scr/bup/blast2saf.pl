...
foreach $arg (@ARGV){
	...
	elsif ($arg=~/^fastadir=(.*)$/i)         { $fastaDir    =        $1;} 
...
foreach $fileIn (@fileIn){
    ...
    if(! defined $fastaDir){
	if($fileIn =~/\//){
	    $tmp=$fileIn; ($fastaDir)=($tmp=~/^(.*\/)/);
	}else{ $fastaDir=""; } 
    }
				# BLAST output
    if ($ctfile > 1 ){
	$fileOutSaf=$id.$par{"extSaf"}; 
        $fileInQuery=$fastaDir.$id.$par{"extFasta"}; 
	...
	if (! defined $fileInQuery)  {$fileInQuery=$fastaDir.$id.$par{"extFasta"}; }
...
close FHERROR;
if (-z $fileErr){ system("rm -f $fileErr");} 
...
sub blastp_to_saf {
    ...
    my (@query, @l_tmp,%sequences,@alignedids,@namesSort,%rdb_lines);
	...
	$queryLength=$#query+1; 
	    ...
	    print "INFO: fastaFile=$queryfile not found\n";
	}
    ...
    if($blastfile=~/\.gz$/){
	$cmd="gunzip -c $blastfile";
	open($fhin,"$cmd |") ||
	    return(0,"ERROR failed to open $cmd, stopped");
    }else{
	open($fhin,$blastfile) ||
	    return(0,"ERROR failed to open file=$blastfile, stopped");
    }
	...
	return(2,"*** WARNING $sbr : no hits found in Blastpgp search of $blastfile");  
    }

    return(0,"*** ERROR $sbr: blast file=$blastfile  format not recognized")              if ($iter == 0);
                                #............................................................
...
#+++++++++++=
    ...
    @l_tmp=();
    for $i (0 .. $#alignedNames){
	push @l_tmp, $alignedNames[$i];
	last if($i == $maxAli-1);
    }
    @alignedNames=@l_tmp;
...
if($short eq "1"){
	...
	die "sequence for id=$key not defined, stopped"
	    if(! defined $sequences{$key});
	$tmp_sequences{$short}=$sequences{$key};
