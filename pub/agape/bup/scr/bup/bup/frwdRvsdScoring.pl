...
print FHOUT "queryID\trank\thomID\tcombPval\tbetter\tcombEval\tscore\tmiu\tlambda\n";
foreach $id1 (sort keys %h_combPval){
	...
	undef $score; undef $miu; undef $lambda;
	if($h_frwd{$id1}{$id2}{'pval'} <= $h_rvsd{$id1}{$id2}{'pval'}){ 
	    $better ="frwd"; 
	    $score  =$h_frwd{$id1}{$id2}{'score'};
	    $miu    =$h_frwd{$id1}{$id2}{'miu'};
	    $lambda =$h_frwd{$id1}{$id2}{'lambda'};
	}else{ 
	    $better ="rvsd"; 
	    $score  =$h_rvsd{$id1}{$id2}{'score'};
	    $miu    =$h_rvsd{$id1}{$id2}{'miu'};
	    $lambda =$h_rvsd{$id1}{$id2}{'lambda'};
	}
	die "$this: values not defined: score=$score, miu=$miu, lambda=$lambda, stopped"
	    if(! defined $score || ! defined $miu || ! defined $lambda);
	@l_line=($id1,$rank,$id2,$combPval,$better,$combEval,$score,$miu,$lambda);
	$line=join "\t", @l_line;
	print FHOUT $line,"\n";
    }
}
