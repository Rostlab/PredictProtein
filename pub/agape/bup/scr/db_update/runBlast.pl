...
foreach $id (sort keys %h_id2fasta){
    ...
    if($doBlast){
	if($useFixedDbLen){
	    $cmd="$blastall_exe -p blastp -i $fasta -d $mfasta -e 100 -v $ctFasta -b $ctFasta -o $blastOut -m 9 -F F -z $DbLen2use -a 1";
	}else{ $cmd="$blastall_exe -p blastp -i $fasta -d $mfasta -e 100 -v $ctFasta -b $ctFasta -o $blastOut -m 9 -F F -a 1"; }
	print "$cmd\n" if($dbg);
