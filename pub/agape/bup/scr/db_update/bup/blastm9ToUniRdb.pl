...
foreach $fileIn (sort keys %h_filesIn){
	...
	die "alignment lengths in file=$fileIn $id1 $id2 calculated in independent ways not eqal: $aliLen vs $aliLenCheck in line:\n$_\n, stopped" 
    ...
    close $fh;
    if($aliCt ==0){
	die "ERROR: did not find any alignments in fileIn=$fileIn, stopped";
    }
