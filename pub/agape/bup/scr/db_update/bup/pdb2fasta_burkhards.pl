...
$scrGoal="reads PDB files and writes out the sequences (chains into split files) \n".
    "     \t note 1: chains with strange symbols to (i) numbers (ii) small cap letters\n".
    "     \t note 2: file.pdb_A for chain A\n";
#  
# 
#------------------------------------------------------------------------------#
#	Copyright				        	1999	       #
...
%par=(
      ...
      'minAcids',       90,	# minimal number of amino acids (excluding '!|X')
      'maxUnk',         20,	# maximal percentage of unknown residues
      '', "",			# 
      '', "",			# 
      'extOut',         ".f",			# 
      '', "",			# 
      );
$databaseId="pdb|pdb|";		# will be added to identifier by default (if none other given)
@kwd=sort (keys %par);
$Ldebug=  0;
$Lverb=   1;
...
if ($#ARGV<1){			# help
    ...
    printf "%5s %-15s=%-20s %-s\n","","fileOutProt", "x",   "name of output file with protein list";
    printf "%5s %-15s=%-20s %-s\n","","fileOutId",   "x",   "name of output file with all ids";
...
#    printf "%5s %-15s %-20s %-s\n","","",   "no value","";
    ...
    printf "%5s %-15s=%-20s %-s\n","","unk",      "p",       "max percentage of unknown acids";

    printf "%5s %-15s %-20s %-s\n","","split",    "no value","write out one fasta file per chain";
    printf "%5s %-15s %-20s %-s\n","","nomerge",  "no value","do not write big FASTA with ALL sequences";
    printf "%5s %-15s %-20s %-s\n","","nochn",    "no value","do not split chains (split ok but not chains)";
    printf "%5s %-15s=%-20s %-s\n","","chain",    "X",       "reads chain X, only";

    printf "%5s %-15s %-20s %-s\n","","uni",      "no value","kicks out identical chains (takes first!)";
    printf "%5s %-15s %-20s %-s\n",""," ",        "",        "NOTE: only check within ONE protein!!";
    

    printf "%5s %-15s %-20s %-s\n","","list",     "no value","if input file is list of FASTA files".
	                                                     " note: automatic if extension *.list!!";
    printf "%5s %-15s %-20s %-s\n","","all",      "no value","show all residues (strange='X')";
...
$fhout="FHOUT"; $fhoutMerge="FHOUT_MERGE";$fhoutRedundant="FHOUT_REDUNDANT";
$fhoutProt="FHOUT_PROTEIN";$fhoutId="FHOUT_ID";$fhoutError="FHOUT_ERROR";
...
#    next if ($arg eq $ARGV[1]);
    ...
    elsif ($arg=~/^fileOutId=(.*)$/i)     { $fileOutId=      $1;}
    elsif ($arg=~/^fileOutProt=(.*)$/i)   { $fileOutProt=    $1;}
    elsif ($arg=~/^fileOutErr.*=(.*)$/i)  { $fileOutError=   $1;}
    ...
    elsif ($arg=~/^maxUnk=(.*)$/i)        { $par{"maxUnk"}=  $1;}
    elsif ($arg=~/^unk=(.*)$/i)           { $par{"maxUnk"}=  $1;}
    ...
    elsif ($arg=~/^split$/)               { $LwrtSplit=      1;}
    elsif ($arg=~/^nomerge$/i)            { $LwrtMerge=      0;}
    elsif ($arg=~/^nochain$/i)            { $LwrtNochn=      1;}
    elsif ($arg=~/^nochn$/i)              { $LwrtNochn=      1;}
    elsif ($arg=~/^uni.*$/i)              { $Lunique=        1;}
    elsif ($arg=~/^all$/i)                { $LshowAll=       1;}
...
if (! defined $fileOutFasta){
    $tmp=$fileIn;
    $tmp=~s/^..*\///g;
    $tmp=~s/\.list//g;
    $tmp=~s/\.(brk|pdb)//g;
    if ($fileIn=~/list$/ || $LisList){
	$fileOutFasta=$tmp.$par{"extOut"};}
    else {
	$fileOutFasta="Out-pdb".$par{"extOut"};}}
	

if (! defined $fileOutId){
    $tmp=$fileIn;
    $tmp=~s/^..*\///g;
    $tmp=~s/\..*$//g;
    $fileOutId="Outid-".$tmp.".list";}
if (! defined $fileOutProt){
    $tmp=$fileIn;
    $tmp=~s/^..*\///g;
    $tmp=~s/\..*$//g;
    $fileOutProt="Outprot-".$tmp.".list";}
...
if ($LisList || $fileIn =~/\.list/) {
    ...
    $#fileTmp=$#chainTmp=0;	# slim-is-in
				...
				# ------------------------------
				# open DB file for all sequences
$ct=0;
if ($LwrtMerge) {
    open("$fhoutMerge",">".$fileOutFasta) || die "*** $scrName ERROR creating file $fileOutFasta!";
}
				# ------------------------------
				# write list of PDB with protein
				#     no chains!
if (! defined $fileOutProt){
    $fileOutProt=$fileOut; 
    if ($fileOut=~/^Out/){
	$fileOutProt=~s/Out/Outprot/;}
    else {
	$fileOutProt.="_protein";}}

open($fhoutProt,">".$fileOutProt)   || die "*** $scrName ERROR creating fileOutProt=$fileOutProt!";
open($fhoutError,">".$fileOutError) || die "*** $scrName ERROR creating fileOutError=$fileOutError!";
				# ------------------------------
				# write list of ids:
				#     incl chains!
if (! defined $fileOutId){
    $fileOutId=$fileOut; 
    if ($fileOut=~/^Out/){
	$fileOutId=~s/Out/Outid/;}
    else {
	$fileOutId.="_id";}}

open($fhoutId,">".$fileOutId) || die "*** $scrName ERROR creating fileOutId=$fileOutId!";
				# --------------------------------------------------
				# read file(s)
				# --------------------------------------------------
undef %taken;
...
$nfileIn=$#fileIn;
if ($Lunique){
    $fileOutRedundant="Outredundant-pair.list";
    open($fhoutRedundant,">".$fileOutRedundant)||
	die("*** ERROR $scrName: failed to open fileout=$fileOutRedundant!\n");
    print $fhoutRedundant
	"# the following chains are not listed since identical to other chains\n",
	"# NOTATION: idRed=identifier of identical chain\n",
	"# NOTATION: idRep=identifier of representative chain (to which idRed is identical)\n";
}
...
foreach $fileIn (@fileIn){
    ...
    if (! -e $fileIn){print "-*- WARN $scrName: no fileIn=$fileIn\n";
		      next;}
				# get chain
    $chainIn="*";
    $chainIn=$chainIn[$ct]        if (defined $chainIn[$ct]);
    $id=$fileIn; $id=~s/^.*\/|\..*$//g;
	...
	$id,$chainIn,$ctfile,(100*$ctfile/$#fileIn) if ($Lverb);
				# ------------------------------
				# read sequence for PDB
				# ------------------------------
#    ($Lok,$msg,$rh_pdb)=
#	&pdbExtrSequence($fileIn,$chainIn,1,$par{"maxUnk"});
    if (1){
	($Lok,$msg,$rh_pdb)=
	    &pdbExtrSequenceATOM($fileIn,$chainIn,1,$par{"maxUnk"},$LshowAll);
    }
    else {
	($Lok,$msg,$rh_pdb)=
	    &pdbExtrSequenceSEQRES($fileIn,$chainIn,1,$par{"maxUnk"});
    }
...
#-------------------------------------------------------------------------------
	...
	(
	 ...
	 'DPN',"F",		# PHE D-CONFIGURATION
	 'FTY',"Y",		# DIFLUROMETHELENEPHOSPHATE DERIVATIVE
	 'MSE',"M",		# selenoMethionin
	 'NPH',"C",		# CHEMICAL MODIFICATION AT SG 
	 'PTH',"Y",		# 
