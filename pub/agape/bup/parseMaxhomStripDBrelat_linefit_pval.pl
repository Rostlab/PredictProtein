...
($StripListFile,$dbRelatFile,$idFile)=@ARGV;
...
undef %h_ids;
open(FHIN,$idsFile) || die "failed to open idsFile=$idsFile, stopped";
while(<FHIN>){
    next if(/^\#|^\s*$/);
    s/\s//g;
    s/.*\///; s/\..*$//;
...
#$statParaFile=$StripListFile;
#$statParaFile=~s/^.*\///; $statParaFile=~s/\./-/;
#$statParaFile.=".stat-para-dat";
#open(FHOUTSTAT,">".$statParaFile) || 
#    die "failed to open $statParaFile for wrting, stopped";
...
sub parse_strip{
	...
	$Eval,$Pval,$EvalLoc,$z,$size,$scoreTmp,$coef,$disp);
...
#    print FHOUTSTAT $Query."\t".$coef."\t".$disp."\t".$miu."\t".$lambda."\t".$h_classDistbn{"miu_evd"}."\t".$h_classDistbn{"lamda_evd"}."\t".$h_classDistbn{"miu_evd_censd"}."\t".$h_classDistbn{"lamda_evd_censd"}."\n";
	...
	foreach $HomID (sort { $h_Results{$a}{$key} <=> $h_Results{$b}{$key} } keys %h_Results ){
	    ...
	    last if($Rank > $maxRankEval);

	    $Score =$h_Results{$HomID}{'VAL'};
	    
	    print $fh $Query."\t".$Rank."\t".$HomID."\t".$Pval."\t".$Score."\n";
