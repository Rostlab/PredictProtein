...
foreach $StripFile (@StripList){
    ...
    ($Lok,$msg)=&parse_strip($StripFile,{%h_ids},$K_evd,$lambda_evd);
...
sub parse_strip{
    my $sbr='parse_strip';
    my ($StripFile,$hr_ids,$K_evd,$lambda_evd)=@_;
    die "$0: $sbr: ERROR arguments not defined, stopped"
	if(! defined $StripFile || ! defined $hr_ids ||
	   ! defined $K_evd || ! defined $lambda_evd);
