...
#parses hssp file into single file with extension "mpearson", since alignments are presented in "pearson" format.

($hsspFile,$queryFasta,$mfasta,$list2consider,$fileOut)=@ARGV;
die "args mfasta,hsspFile,dirOut not defined, stopped" 
    if(! defined $mfasta || ! defined $queryFasta || ! defined $hsspFile ||
       ! defined $list2consider || ! defined $fileOut);

if(defined $list2consider){
    open(FHIN,$list2consider) || 
	die "failed to open list2consider=$list2consider, stopped";
    while(<FHIN>){
	next if(/^\#|^\s*$/);
	$_=~s/\s//g;
	$h_ids2consider{$_}=1;
...
$querySeq="";
open(FHIN,$queryFasta) || die "failed to open $mfasta, stopped";
while(<FHIN>){
    if(/>(\S+)/){ $id=$1; $id=~s/.*\|//; next;}
    $querySeq.=$_;
}
...
while(<FHIN>){
    if(/>(\S+)/){ $id=$1; $id=~s/.*\|//; next;}
    if(defined $list2consider){
	next if(! defined $h_ids2consider{$id});
    }
    $h_fastas{$id}.=$_;
}
close FHIN;
...
if($hsspFile =~/\.list/){
    open(FHIN,$hsspFile) || 
	die "failed to open hsspFile=$hsspFile, stopped";
    while(<FHIN>){
	next if(/^\#|^\s*$/);
	$_=~s/\s//g;
	push @l_hssps, $_;
    ...
    close FHIN;
}else{ push @l_hssps, $hsspFile; }

foreach $hsspFile (@l_hssps){
    undef %h_aliId2pearson;
    ($Lok,$msg,$query)=&parse_hssp($hsspFile,'10000');
    return(0,"ERROR $msg") if (!$Lok);
    
    #$fileOut=$hsspFile;
    #$fileOut=~s/^.*\///; $fileOut=~s/\..*$/\.mpearson/;
	...
	if(defined $list2consider){
	    next if(! defined $h_ids2consider{$subject});
	}
	$totqali=$h_aliId2pearson{$subject}{'query'};
	$totsali=$h_aliId2pearson{$subject}{'subject'};

	#get rid of overhanging gaps (keep alignment region only, hssp 'knows only entire query')
	($sFrontGap) =($totsali=~/^(-*)/); $sFrontGapLen=length($sFrontGap);
	($sEndGap)   =($totsali=~/(-*)$/); $sEndGapLen  =length($sEndGap); 
	substr($totqali,0,$sFrontGapLen)="";
	substr($totsali,0,$sFrontGapLen)="";

	substr($totqali,-$sEndGapLen,$sEndGapLen)="";
	substr($totsali,-$sEndGapLen,$sEndGapLen)="";
