...
($mpearson,$fileOut,$maxAli,$dbRelatFile)=@ARGV;

die "$0: arguments not defined mpearson=$mpearson fileOut=$fileOut, stopped"
    if(! defined $mpearson || ! defined $fileOut || ! defined $dbRelatFile );
...
undef %h_pearsons;
&read_mpearson($mpearson,\%h_pearsons);
...
sub read_mpearson{
    ...
    my ($ct,$qID,$qAli,$qFasta,$sID,$subjectID,$sAli,$sFasta,$homCt,$info,$Pvalue,$Evalue);
    my $fh="FH".$sbr;
