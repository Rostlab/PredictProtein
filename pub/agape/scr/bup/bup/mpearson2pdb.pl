...
foreach $modelNo (1 .. $fragNo){
	...
	#first find most significant fragments
	foreach $fragLocCt ( sort {$h_number2data{$a}{"pvalLoc"} <=> $h_number2data{$b}{"pvalLoc"}} 
			     keys %h_number2data ){
	    #check if fits
	    print "parent candidate: $h_number2data{$fragLocCt}{parent}\n";
	    $overlapCt=0;
		...
		print "parent chosen: $h_number2data{$fragLocCt}{parent}\n";
