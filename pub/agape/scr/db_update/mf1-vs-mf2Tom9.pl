...
($mfasta,$mf_db,@l_args)=@ARGV;

die "arguments not defined: mfasta=$mfasta, mf_db=$mf_db, stopped"
    if(! defined $mfasta || ! defined $mf_db);
...
#$cmd="$formatdb_exe -i $mf_db -p T";
#system($cmd)==0 || 
#    die "failed on $cmd, stopped";
@l_db_files=($mf_db.".phr",$mf_db.".pin",$mf_db.".psq");

foreach $it (@l_db_files){
    die "$it not found, stopped" if(! -e $it);
}
...
foreach $id (sort keys %h_id2fasta){
    $fasta=$h_id2fasta{$id};
    $blastOut=$id.".blastm9";
    $blastOutgz=$blastOut.".gz";
    if(-e $blastOutgz || -e $blastOut){
	print "$blastOut already exists, skipping\n";
	next;
    }
    if($doBlast){
	if($useFixedDbLen){
	    $cmd="$blastall_exe -p blastp -i $fasta -d $mf_db -e 100 -v $ctFasta -b $ctFasta -o $blastOut -m 9 -F F -z $DbLen2use -a 1";
	}else{ $cmd="$blastall_exe -p blastp -i $fasta -d $mf_db -e 100 -v $ctFasta -b $ctFasta -o $blastOut -m 9 -F F -a 1"; }
	print "$cmd\n" if($dbg);
	...
	$cmd2="$blastpgp_exe -i $fasta -d $mf_db -R $check -e 100 -v $ctFasta -b $ctFasta -o $blastOut -m 9 -F F";
	if($usePsiSpace){
