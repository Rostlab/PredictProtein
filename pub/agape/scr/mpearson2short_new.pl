...
($mpearson,$fileOutShort,$fileOutLong,$maxAli,$dbRelatFile,$QueryName)=@ARGV;

die "$0: arguments not defined mpearson=$mpearson, fileOutShort=$fileOutShort, fileOutLong=$fileOutLong, maxAli=$maxAli, dbRelatFile=$dbRelatFile, QueryName=$QueryName, stopped"
    if(! defined $mpearson || ! defined $fileOutShort || ! defined $fileOutLong || ! defined $maxAli || ! defined $dbRelatFile || ! defined $QueryName);
...
$fileOutShortHtml=$fileOutShort.".html";
open(FHOUTHTML,">".$fileOutShortHtml) ||
    die "failed to open fileOutShortHtml=$fileOutShortHtml, stopped";
print FHOUTHTML "<html lang=\"en\">"."\n";
...
print FHOUT "\# query sequence   : $QueryName\n$qSeqOutForm\n\#\n";

print FHOUTHTML "\# \"short\" alignment format (showing only aligned regions)\n";
print FHOUTHTML "\# P-value, E-value : statistical significance of \"bi-directional\" scores (caution: too optimistic for highly significant values)\n";
print FHOUTHTML "\# P-frwd, E-frwd   : statistical significance of \"forward\" scores (generalized profile of a query against a database of generalized sequences)\n";
print FHOUTHTML "\# P-frwd, E-frwd   : statistical significance of \"reversed\" scores (generalized sequence of a query against a database of generalized profiles)\n";
print FHOUTHTML "\# query sequence   : $QueryName\n$qSeqOutForm\n\#\n";
...
foreach $homCt (sort {$a <=> $b} keys %h_aliForm){
	...
	print FHOUTHTML $qLine;
	print FHOUTHTML $sLine;
	print FHOUTHTML "\n";
	
    }
}
print FHOUTHTML "</html>"."\n";
close FHOUT; close FHOUTHTML;
...
print FHOUTLONG "\# query sequence   : $QueryName\n$qSeqOutForm\n\#\n";
foreach $homCt (sort {$a <=> $b} keys %h_pearsons){
    last if($homCt > $maxAli);
    $qID  =$h_pearsons{$homCt}{'qid'};
    $sID  =$h_pearsons{$homCt}{'sid'};
