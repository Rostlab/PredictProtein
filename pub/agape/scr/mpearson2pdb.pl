...
$configFile     ="/nfs/data5/users/ppuser/server/pub/agape/scr/agape_config.pm";
...
while(<FHINCOMBLOC>){
    ...
    next if(/^queryID/);
    s/\s*$//;
    ($queryID,$rank,$HomID,$Pval,$better,$Eval,$score,$miuBest,$lambdaBest,$frwdPval,$frwdEval,$rvsdPval,$rvsdEval)=split(/\t/,$_);
    return(0,"ERROR $sbr: data not defined in $fileParsedCombined: queryID=$queryID, rank=$rank, HomID=$HomID, Pval=$Pval, better=$better, Eval=$Eval, score=$score, miuBest=$miuBest, lambdaBest=$lambdaBest, frwdPval=$frwdPval, frwdEval=$frwdEval, rvsdPval=$rvsdPval, rvsdEval=$rvsdEval")
	if(! defined $queryID || ! defined $rank || ! defined $HomID || ! defined $Pval  || ! defined $better || ! defined $Eval || ! defined $score || ! defined $miuBest || ! defined $lambdaBest || ! defined $frwdPval || ! defined $frwdEval || ! defined $rvsdPval || ! defined $rvsdEval);
    ...
    $h_homID2aliDat{$HomID}{'miuBest'}     =$miuBest;
    $h_homID2aliDat{$HomID}{'lambdaBest'}  =$lambdaBest;
    $h_homID2aliDat{$HomID}{'frwdPval'}    =$frwdPval;
    $h_homID2aliDat{$HomID}{'frwdEval'}    =$frwdEval;
    $h_homID2aliDat{$HomID}{'rvsdPval'}    =$rvsdPval;
    $h_homID2aliDat{$HomID}{'rvsdEval'}    =$rvsdEval;
}
close FHINCOMBLOC; #----------------------------------------------------------
...
foreach $modelNo (1 .. $fragNo){
	...
	foreach $homCt ( sort {$a <=> $b} keys %h_alignments ){
	    ...
	    $miuLoc    =$h_homID2aliDat{$parent}{'miuBest'};
	    $lambdaLoc =$h_homID2aliDat{$parent}{'lambdaBest'};
