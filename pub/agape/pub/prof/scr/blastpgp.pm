...
$packDefault=       "/usr/pub/molbio/maxhom/scr/blastpgp.pm";
#$packDefault=       "/usr/pub/molbio/perl/pack/blastpgp.pm";
#$packDefault=       "/home/rost/pub/prof/scr/blastpgp.pm";
#$packDefault=       "/home/dudek/perl/blastpgp.pl";

# default settings
$ARCH_DEFAULT=      "SGI64";
$ARCH_DEFAULT=                  "LINUX";

$par{"exeBlast"}=   "/home/rost/pub/prof/bin/blastpgp.ARCH";
#$par{"exeBlast"}=   "/usr/pub/molbio/blast/blastpgp";
$par{"dbBlastX"}=    "/data/blast/bigx_coil_seg";
$par{"dbBlastB"}=    "/data/blast/big";
$par{"argBlastX"}=   "-j 3 -e 1 -h 1e-4 -b 3000 -F T -d";
$par{"argBlastB"}=   "-e 1 -b 3000 -F T -d";
$par{"optNice"}=    "nice -15";
...
foreach $fileIn (@fileIn){
	...
	if (defined $fileOut) {$fileOutBlast=$fileOut;}
	else { $fileOutBlast=$dirOut.$id.$par{"extBlastpgp"}; }
	if ($fileOutMat ne "0" ) {$fileOutMat="1"; }
	if ($fileOutSaf ne "0")  {$fileOutSaf="1" ; }
	
	if ($fileOutRdb ne "0")  {$fileOutRdb="1";}
    }
				# BLAST matrix
    ($Lok,$msg)=
	&blastp::blastit($fileIn,$par{"exeBlast"},$par{"dbBlastX"},$par{"dbBlastB"},$par{"argBlastX"},$par{"argBlastB"},
			 $fileOutBlast,$fileOutMat,$fileOutSaf,$dirWork,$dirOut,$par{"optNice"},$filter,$maxAli,$fileOutRdb,$tile,$eThresh,$keepIter,$Ldebug); 
...
#	next if ($arg eq $ARGV[1]);blastpgp.pm.~1~
	...
	elsif ($arg=~/^(arg|argBlast)=(.*)$/i){ $par{"argBlastX"}=$2;}
...
sub helpBlast{
    $btex= "\n----------------------------------------\n";
    $btex.="SOME OF PSI-BLAST OPTIONS:\n";
...
#-------------------------------------------------------------------------------
	...
	printf "%5s %-15s %-20s %-s\n","","",        "",        "after filtering (def: 3000)";
	printf "%5s %-15s=%-20s %-s\n","","tile",     "x",      "either just 'tile' -> will tile blast prediction in saf";
	printf "%5s %-15s %-20s %-s\n","","",        "",        "or 'tile=(0|1)' to enable|disable tiling (def 0)";
	printf "%5s %-15s=%-20s %-s\n","","eSaf",   "x",           "maximum blast expect value to be included in 'saf' file";
	printf "%5s %-15s%-20s %-s\n","","keepIter", "no value",   "to keep blast file with results of iterative run (extension is '._Iter'";
	...
	printf "%5s %-15s=%-20s %s %-s %-s\n","","dbTrain",           "database_name", "Psi-Blast  database for training  (def=",$par{"dbBlastX"},")";
	printf "%5s %-15s=%-20s %s %-s %-s\n","","dbFinal",           "database_name", "Psi-Blast  database for final search  (def=",$par{"dbBlastB"},")";
