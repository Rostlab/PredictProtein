...
foreach $arg (@ARGV){		# optional input keys
    $arg=~s/\s|\n//g;
    foreach $des (@desOpt){if ($arg=~/^$des=/){$arg=~s/^$des=//g;$par{"$des"}=$arg;}}}
...
foreach $fileIn(@fileIn){
	...
	$fileX=$fileIn;$fileX=~s/^.*\///g;
	$fileDaf=$fileX;$fileDaf=~s/\.fssp/\.daf_fssp/g;}
				# security: same name?
    if ($fileDaf eq $fileIn){$fileDaf.=".tmp"; print"*** ERROR daf same name as FSSP\n";}
    $fileDafTmp="x".$$.""."_".$fileDaf;push(@fileTmp,$fileDafTmp);
...
if (! $par{"debug"}){
	...
	if (-e $file){print "system 'rm $file'\n";}
	system("\\rm $file");}}
...
#--------------------------------------------------------------------------------
				# run converter
    system("$exeConv $fileFssp >> $fileDafTmp");
