...
#-------------------------------------------------------------------------------
    ...
    $parSbr{"optSeg"}=    "x";	# pass the output print options as comma separated list
				#    NO '-' needed, see below
    $parSbr{"exeSeg"}=    "/home/rost/pub/molbio/bin/seg".$ARCH; # executable of SEG
...
#-------------------------------------------------------------------------------
	...
	if    ($Lboth){		# PHDsec+acc
	    $pos=9; 
	    $pos=12             if (! $parSbr{"isPred"});} # correct for pred+obs
	else          {		# PHDacc
	    $pos=4; 
	    $pos=6              if (! $parSbr{"isPred"});} # correct for pred+obs
	$rel=$tmp[$pos];
