...
#    elsif ($arg=~/^=(.*)$/){ $=$1;}
	...
	if (! $Lok){ print "*** wrong command line arg '$arg'\n";
		     exit;}}}
				...
				# (1) read file(s)
				# ------------------------------
@tmp=`ps -ealf|grep $togrep`;
foreach $tmp (@tmp){
    $tmp=~s/\n//g;
    @tmp2=split(/\s+/,$tmp);
    next if ($tmp=~/csh |emacs | bash | sh |$0 |ps |grep |\-bash/);
    next if ($#tmp2 < 4);
    $cmd="kill -9 $tmp2[4]";
    print "system '$cmd'  ($tmp)\n";
    system("$cmd");
}
exit;
