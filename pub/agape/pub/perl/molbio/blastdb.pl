...
$scrGoal="translates DB in fasta format to BLAST DB version (OLD AND NEW)\n".
    "     \t \n".
    "     \t ";
#  
# 
...
$par{"exeBlast2db"}=        "/home/rost/molbio/bin/setdb."   .   "ARCH"; # format DB for old BLAST
$par{"exeBlast3db"}=        "/home/rost/molbio/bin/formatdb.".   "ARCH"; # format DB for new BLAST (PSI)

$par{"fileOutScreen"}=      "BLASTDB-screen.tmp";
$par{"fileOutLog"}=         "BLASTDB-log.tmp";
...
if ($#ARGV<1 ||			# help
    ...
    print  "goal: $scrGoal\n";
    print  "use:  '$scrName db (in fasta format)'\n";
    print  "opt:  \n";
...
foreach $fileIn (@fileIn){
    ...
    printf "--- $scrName: working on %-25s %4d (%4.1f perc of job)\n",$fileIn,$ctfile,(100*$ctfile/$#fileIn);
				# ------------------------------
				# now create BLAST binaries:
				#  - old blast (WashU): 
				#    'setdb.ARCH -t pdb pdb.fasta'
    $titleIn=$fileIn; $titleIn=~s/\.*$//g;
    $cmd=$par{"exeBlast2db"}." -t ".$titleIn." ".$fileIn;
    ($Lok,$msg)=
	&sysRunProg($cmd,$par{"fileOutScreen"},
		    $FHTRACE);  die("ERROR $scrName: system call failed ($cmd)\n".$msg."\n") if (! $Lok);
				#  - new blast (PSI-BLAST): 
				#    'formatdb.ARCH -t pdb -i pdb.fasta -p T -l logfile'
    $cmd=$par{"exeBlast3db"}." -t ".$titleIn." -i ".$fileIn." -p T -l ".$par{"fileOutLog"};
    ($Lok,$msg)=
	&sysRunProg($cmd,$par{"fileOutScreen"},
		    $FHTRACE);  die("ERROR $scrName: system call failed ($cmd)\n".$msg."\n") if (! $Lok);
}
