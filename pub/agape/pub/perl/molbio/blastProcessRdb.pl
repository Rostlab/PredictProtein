...
$scrHelpTxt.="    or *.blast\n";
$scrHelpTxt.="    NOTE: if RDB   -> NO list\n";
$scrHelpTxt.="          if BLAST -> give list of *.blast files as file'.list'!!\n";
$scrHelpTxt.="    \n";
...
if ($par{"doPairs"} && defined $fileRes) {
	...
	&fileResRd($fileRes);   &errScrMsg("*** ERROR $scrName: after fileResRd=$fileRes:",
				...
				# --------------------------------------------------
				# mode: input = original blast file
				# --->  WATCH it changes input files @fileIn
				# --------------------------------------------------
$#fileRm=0;
if ($par{"isBlastOriginal"}){
				# convert BLAST 2 rdb
    $fileOutTmp=$par{"dirWork"}.$par{"titleTmp"}."original.rdb";
    push(@fileRm,$fileOutTmp)   if (! $par{"debug"});
    ($Lok,$msg)=
	&conv_blast2rdb_here(\@fileIn,$fileOutTmp);
    &errScrMsg("*** ERROR $scrName: after conv_blast2rdb_here:",
	       $msg,$scrName) if (! $Lok);
				# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
foreach $fileIn (@fileIn){
    ...
    print "--- $scrName: working on filein=$fileIn!\n" if ($par{"verbose"});
				...
				#             $ncol, $nrow
    ($Lok,$msg)=
	&fileInRd($fileIn);     &errScrMsg("*** ERROR $scrName: failed reading filein=$fileIn",
					   $msg,$scrName) if (! $Lok);
				# ------------------------------
				# get lists of unique identifiers
    				#     to take (write)
				# in  GLOBAL: $rd[itrow][itcol], $ncol, $nrow
				# out GLOBAL: $id1[1..$nrow], $id2[1..$nrow], 
    ($Lok,$msg)=
	&uniqIdLoc();		&errScrMsg("*** ERROR $scrName: failed getting list of ids filein=$fileIn",
					   $msg,$scrName) if (! $Lok);
...
#    $wrt2.=    "# $fileIn\n";
    ...
    next if (! $par{"doPairs"});
				# ------------------------------
				# do pairs
				# in  GLOBAL:             @rd, @dist, $nrow, %ptr, $resMax
    ...
    ($Lok,$msg)=
	&getPairsLoc();		&errScrMsg("*** ERROR $scrName: failed getting pairs for filein=$fileIn",
					   $msg,$scrName) if (! $Lok);
...
#    $par{""}=                   "";
    ...
    $par{"dirDataOrigin"}=      "/data/genome/ORGANISM/seq/";
    $par{"dirDataOrigin"}=      "/home/rost/current/cafasp/fasta/";
    $tmp=join("|",@ARGV);
    foreach $org ("caeel","yeast","metja"){
	$par{"dirDataOrigin"}=~s/ORGANISM/$org/ if ($tmp=~/$org/);
    }
...
#-------------------------------------------------------------------------------
    ...
    $tmp{"special"}.=        "pairs".",". "pdb".",". "mirror".",". "fileRes".",";
...
#-------------------------------------------------------------------------------
	...
	if (! defined $pairs{$id1}){
	    ...
	    $fileFasta=$par{"dirDataOrigin"}.$id1.$par{"extOrigin"};
				# a la swissprot: /data/swissprot/current/x/id
	    if (! -e $fileFasta) {
		$tmp=$id1; $tmp=~s/^[^\_]+\_(.).*$/$1/g;
		$fileFasta=$par{"dirDataOrigin"}.$tmp."/".$id1.$par{"extOrigin"};}
				# a la genome /data/genome/org/seq/id
	    if (! -e $fileFasta) {
		$fileFasta=$par{"dirDataOrigin"}.$id1.$par{"extOrigin"};}
				# a la genome /data/genome/org/seq/ID (change case)
	    if (! -e $fileFasta) {
		$tmp=$id1;$tmp=~tr/[a-z]/[A-Z]/;
		$fileFasta=$par{"dirDataOrigin"}.$tmp.$par{"extOrigin"};}
