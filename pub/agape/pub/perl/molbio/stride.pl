...
$scrGoal="perl wrapper around STRIDE\n".
    "     \t \n".
    "     \t ";
#  
# 
...
%par=(
      ...
      'extStride',           ".stride",
      'exeStrideBin',        "/home/rost/molbio/bin/stride.ARCH",
      'parStrideCheck',      1,	# check STRIDE for completeness
      'parStrideDelete',     0,	# delete empty STRIDE files
      'optNice',           "nice -15",
...
if ($#ARGV<1 ||			# help
    ...
    printf "%5s %-15s=%-20s %-s\n","","fileErr", "x",       "name of file with list of faulty STRIDE files";
    printf "%5s %-15s=%-20s %-s\n","","dirOut",  "x",       "output dir (only for many input files)";
    printf "%5s %-15s=%-20s %-s\n","","extOut",  "x",       "extension for STRIDE (only for many input files)";
    printf "%5s %-15s %-20s %-s\n","","check",   "no value","if set: check STRIDE file";
    printf "%5s %-15s %-20s %-s\n","","del",     "no value","delete empty STRIDE files";
    printf "%5s %-15s=%-20s %-s\n","","exe",     "x",       "STRIDE binary";
    printf "%5s %-15s=%-20s %-s\n","","ARCH",    "x",       "system architecture (SGI32|SGI64|ALPHA|LINUX)";
    printf "%5s %-15s %-20s %-s\n","","new",     "no value","existing STRIDE files will be overwritten";
    printf "%5s %-15s %-20s %-s\n","","skip",    "no value","existing STRIDE files will NOT be overwritten";
...
#    next if ($arg eq $ARGV[1]);
    ...
    elsif ($arg=~/^check$/)               { $par{"parStrideCheck"}= 1;}
    elsif ($arg=~/^del\S*$/)              { $par{"parStrideDelete"}=1;
					    $par{"parStrideCheck"}= 1;}
    elsif ($arg=~/^exe.*=(.*)$/i)         { $par{"exeStrideBin"}=  $1;}
...
die ("*** ERROR $scrName: missing input $fileIn!\n") 
    if (! -e $fileIn);
$fileErr="STRIDE_errors.list"     if (! defined $fileErr);
...
if ($par{"exeStrideBin"}=~/ARCH/){
    ...
    $par{"exeStrideBin"}=~s/ARCH/$ARCH/;}

die ("*** ERROR $scrName: missing executable for STRIDE=".$par{"exeStrideBin"}."!\n")
    if (! -e $par{"exeStrideBin"} && ! -l $par{"exeStrideBin"});
...
foreach $fileIn (@fileIn){
    ...
    ($Lok,$msg)=
	&strideRun
	    ($fileIn,$fileOut,$par{"exeStrideBin"},$par{"optNice"}
	     );			&errScrMsg("failed strideRun on $fileIn->$fileOut",$msg) if (! $Lok);
...
if ($par{"parStrideCheck"}){
	...
	($Lok,$msg,$len)=
	    &strideCheck
		($fileOut);	&errScrMsg("failed strideCheck on out=$fileOut",$msg) if (! $Lok);
...
if ($Lverb){
    ...
    print "--- ids of $ctErr files with STRIDE problems: $fileErr\n" if (-e $fileErr);
}
exit;
