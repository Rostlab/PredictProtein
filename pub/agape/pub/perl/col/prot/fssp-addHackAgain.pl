...
if ($#ARGV<1){print"goal:   replaces new-representative set pdb by all similar ones (TABLE2)\n";
...
while (<$fhin>) {$_=substr($_,1,12);$_=~s/^\s+|\s+$//g;$_=~s/-//g;
		 @tmp=split(/\s+/,$_);
		 if (! defined $fssp2{$tmp[1]}){$fssp2{$tmp[1]}=$tmp[2];}
		 else {$fssp2{$tmp[1]}.=",$tmp[2]";}
...
while (<$fhin>) {$_=~s/\n//g;$_=~s/^\s+|\s+$//g;
		 ...
		 $new=~s/^,+|,+$//g;
				# now add new representative set
		 if (defined $fssp2{$id}){$newFssp=$fssp2{$id};
					  @tmp=split(/,/,$newFssp);
					  foreach $tmp(@tmp){
					      if (defined $fssp{$tmp}){
						  $new.=",".$fssp{$tmp}}}}
