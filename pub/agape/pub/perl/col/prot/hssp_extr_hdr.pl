...
$lowDisI=-25;			# minimal distance from HSSP threshold
...
if ($#ARGV<1){
    ...
    print"   or:  'script list fileTrue=truePairs849Hand.dat'\n";
    print"option: low=30      (default=$lowPide)\n";
    print"        len=50      (default=$lowLali)\n";
    print"        up=95       (default=$upPide, upper cut-off for PIDE)\n";
    print"        lowDisI=-5  (minimal distance from HSSP, def=$lowDisI)\n";
    print"        lowR12>x    (min for len1/len2 ratio, i.e., 0.5, def=$lowR12)\n";
    ...
    print"        verbose,verb2,verb3 (why excluded)\n";
...
foreach $_ (@ARGV){
    ...
    elsif(/^lowDisI=(.+)$/)      {$lowDisI=$1;    $lowDisI=~s/\s//g;}
    ...
    elsif(/^verb2/)   {$Lscreen2=1;}
    elsif(/^verb3/)   {$Lscreen3=1;}
    else {
	$tmp=$_;$tmp=~s/\n//g;
	if    (&is_hssp($tmp)){	# external lib-prot.pl
	    ...
	    while(<$fhin>){$_=~s/\n//g;$tmp=$_;$tmp=~s/_.$//g;
			   if   (-e $_|| -e $tmp)  {push(@fileHssp,$_);}}close($fhin);}
	else {
...
$fileOutId2=      "OutId2-".   "$title"."-p"."$lowPide"."-l"."$lowLali".".list";
...
$ratioResCovered=$ctAll=0;$#idSwiss=$#idHssp=$ctExcl=0;
foreach $fileIn (@fileHssp){
	...
	$idStr=$rd{"$itSeq","ID"};
	$disI=&getDistanceHsspCurve($rd{"$itSeq","LALI"});
	if    ($rd{"$itSeq","IDE"}==1){ # exclude self
	    $flag{"$idSeq"}=1; $idMemory=$idSeq; 
	    if ($LexclSelf){
		if ($Lscreen3){print "--- excl $itSeq $idStr\t self\n";}
		++$ctExcl;
		next;}}
	...
	if ((100*$rd{"$itSeq","IDE"})<$lowPide){ # minimal percentage seq identity
	    if ($Lscreen3){
		print "--- excl $itSeq $idStr\t ",(100*$rd{"$itSeq","IDE"})," < $lowPide\n";}
	    ++$ctExcl;
	    next;}
	if ((100*$rd{"$itSeq","IDE"})>$upPide){ # maximal percentage seq identity
	    if ($Lscreen3){
		print "--- excl $itSeq $idStr\t ",(100*$rd{"$itSeq","IDE"})," > $upPide\n";}
	    ++$ctExcl;
	    next;}
	if ($rd{"$itSeq","LALI"}<=$lowLali){ # minimal length
	    if ($Lscreen3){
		print "--- excl $itSeq $idStr\t lali:",$rd{"$itSeq","LALI"}," < $lowLali\n";}
	    ++$ctExcl;
	    next;}
	if (($disI+$lowDisI)>(100*$rd{"$itSeq","IDE"})){ # minimal distance from HSSP
	    if ($Lscreen3){
		print "--- excl $itSeq $idStr\t hsspT ($disI,$lowDisI) < ",
		      $rd{"$itSeq","IDE"},"\n";}
	    ++$ctExcl;
	    next;}
	if (($rd{"$itSeq","LEN2"}==0)||($rd{"$itSeq","LALI"}==0)){ # error?
	    if ($Lscreen3){
		print "--- excl $itSeq $idStr\t lali or len2 =0\n";}
	    ++$ctExcl;
	    next;}
				# ratio cut off?
	if (($lowR12>0)&&(($rd{"len1"}/$rd{"$itSeq","LEN2"})<$lowR12)){
	    if ($Lscreen3){
		print "--- excl $itSeq $idStr\t lowR12=$lowR12\n";}
	    ++$ctExcl;
	    next;}
	if (($lowR1A>0)&&(($rd{"len1"}/$rd{"$itSeq","LALI"})<$lowR1A)){
	    if ($Lscreen3){
		print "--- excl $itSeq $idStr\t lowR1A=$lowR1A\n";}
	    ++$ctExcl;
	    next;}
	if (($lowR2A>0)&&(($rd{"$itSeq","LALI"}/$rd{"$itSeq","LEN2"})<$lowR2A)){
	    if ($Lscreen3){
		print "--- excl $itSeq $idStr\t lowR2A=$lowR2A\n";}
	    ++$ctExcl;
	    next;}
				# ------------------------------
	++$ct;			# statistics
...
close($fhout);
				...
				# write output files
				# ------------------------------
				# file with id-list  swiss1,swiss2, ..
&open_file("$fhout", ">$fileOutId2");
printf $fhout "%-6s\t%-s\n","id1","id2";
$tmp="";%ok=%ok2=0;
foreach $it (1..$#idSwiss){
    if (! defined $ok{$idHssp[$it]}){
	%ok2=0;
	if ($it>1){$tmp=~s/,$//g;
		   printf $fhout "$id1\t$tmp\n";}
	$id1=$idHssp[$it];$ok{$id1}=1;}
    if (defined $ok2{$idSwiss[$it]}){ # avoid writing twice
	next;}
    $tmpx=$idSwiss[$it];$tmpx=~s/\s//g;$tmp.="$tmpx,";}
$tmp=~s/,$//g;
printf $fhout "$id1\t$tmp\n";
close($fhout);push(@fileOut,$fileOutId2);

if ($#idFound>0){		# only if Lstat
				# ------------------------------
				# file with id's
...
if ($Lscreen){ 
    if ($Lstat){print"found: $pideAll, excluded=$ctExcl\n";}
...
#					   print "xx no,,,-id=$id, true\n";
    ...
    printf $fhout "%-8s\t","dist"; # header
    @des=("dis","disCum");
    if (defined $fileTrue){push(@des,"disT","disF","disCumT","disCumF");}
    foreach $des(@des){printf $fhout "%-8s\t",$des;}
    print $fhout "\n";
    $distDis=0;			# x.x
    foreach $des(@des){
	$sum{"$des"}=0;}
	...
	if ($his{"dis","$dist"}==0){ # ignore 0 counts
	    next;}
	printf $fhout "%-8s\t",$dist;
	foreach $des(@des){
	    if    ($des eq "dis")     {$sum{"$des"}+=$his{"$des","$dist"};
				       printf $fhout "%-8d\t",$his{"$des","$dist"};}
	    elsif ($des eq "disT")    {$sum{"$des"}+=$his{"$des","$dist"};
				       printf $fhout "%-8d\t",$his{"$des","$dist"};}
	    elsif ($des eq "disF")    {$sum{"$des"}+=$his{"$des","$dist"};
				       printf $fhout "%-8d\t",$his{"$des","$dist"};}
	    elsif ($des eq "disCum")  {printf $fhout "%-8d\t",$sum{"dis"};}
	    elsif ($des eq "disCumT") {printf $fhout "%-8d\t",$sum{"disT"};}
	    elsif ($des eq "disCumF") {printf $fhout "%-8d\t",$sum{"disF"};}
	    else                      {$sum{"$des"}+=$his{"$des","$dist"};
				       printf $fhout "%-8d\t",$sum{"$des"};}}
...
#-------------------------------------------------------------------------------
	...
	if ($his{"all","$pide"}==0){
	    next;}
	printf $fhout "%-8s\t",$pide;
	foreach $des(@des){
	    if    ($des eq "all")     {$sum{"$des"}+=$his{"$des","$pide"};
				       printf $fhout "%-8d\t",$his{"$des","$pide"};}
	    elsif ($des eq "allT")    {$sum{"$des"}+=$his{"$des","$pide"};
				       printf $fhout "%-8d\t",$his{"$des","$pide"};}
	    elsif ($des eq "allF")    {$sum{"$des"}+=$his{"$des","$pide"};
				       printf $fhout "%-8d\t",$his{"$des","$pide"};}
	    elsif ($des eq "allCum")  {printf $fhout "%-8d\t",$sum{"all"};}
	    elsif ($des eq "allCumT") {printf $fhout "%-8d\t",$sum{"allT"};}
	    elsif ($des eq "allCumF") {printf $fhout "%-8d\t",$sum{"allF"};}
