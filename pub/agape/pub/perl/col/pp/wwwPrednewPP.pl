...
$usr.="\@embl-heidelberg.de" if ($usr eq "rost" || $usr eq "vriend");
                                # ----------------------------------------
if (!$usr) {			# address is mandatory 
    ...
    exit(1);}
                                # ---------------------------------------- 
                                # make a basic address syntax control (one "@" and one ".")
if (($usr !~ /\S+\@\S+\.\S+/) && ($usr ne "rost") && ($usr ne "vriend")) {
    print &PrintHeader; 
    print "Please check your e-mail address: format appears invalid (given=$usr).";
    exit(1);}
                                # ---------------------------------------- 
if ($usr =~ /\S*\.(\S+)/) {	# last part of the address must not be more than 4 char
    if (length($1) > 4) {
	print &PrintHeader; 
	print "Please check your e-mail address: format appears invalid (given=$usr).";
	exit(1);}}
