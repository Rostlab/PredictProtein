...
$scrHelpTxt.="   - exclude mutually too similar pairs (too redundant alignment, redRed=80, see help red)\n";
$scrHelpTxt.=" \n";
$scrHelpTxt.=" \n";
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
foreach $it (1..$#fileIn){	# loop over all input files
	...
	&hsspFilter($fileIn[$it],$fileOut[$it],$par{"exeFilterHssp"},$par{"fileMatGcg"},
                    $par{"inclPos"},$par{"exclPos"},$par{"minIde"},$par{"maxIde"},$par{"minSim"},$par{"maxSim"},
                    $par{"thresh"},$par{"threshSgi"},$par{"mode"},$fileOutTable,$par{"redRed"},
...
#                       "------------------------------------------------------------\n";
    ...
    if ($par{"verbose"}){
	($Lok,$msg)=
	    &brIniWrt;		return(0,"*** ERROR $sbrName: after lib-ut:brIniWrt\n".$msg) if (! $Lok); }
