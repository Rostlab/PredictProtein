...
if ($#ARGV<1){print "goal:    removes all files from input file or by pattern\n";
	      print "usage:   'script list-of-files OR patterns (not both)'\n";
...
foreach $tmp (@list){
    ...
    push(@file,$tmp[9]);push(@usr,$tmp[3]);push(@size,$tmp[5]);
    push(@month,$tmp[6]);push(@day,$tmp[7]);push(@date,$tmp[8]);}
