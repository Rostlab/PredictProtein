...
#    $rdb{"NALIGN"}=$rdb{"PROT_NALI"} if (! defined $rdb{"NALIGN"} && defined $rdb{"PROT_NALI"});
	...
	$rdb{$itres,"AA"}=$rdb{1,$itres,"AA"};
				# reliability index
	($Lok,$msg,$rdb{$itres,$par{"sec_kwdri"}})=
	    &get_ri($par{"modepred"},100,
		    @tmp);      &errScrMsg("problem with RI par(sec_states)=".
					   $par{"sec_states"}."out=".join(' ',@tmp).
					   "line=".__LINE__ ."\n",
					   $msg) if (! $Lok);
    }
				# ----------------------------------------
    ...
    $fileOut=$id.$par{"extOutRdb"};
				# open file
    open($fhout,">".$fileOut) || &errScrMsg("failed to open fileout=$fileOut","line=".__LINE__);
    print $fhout
	"\# Perl-RDB\n",
	"\# \n",
	"\# jury sec (njury=".$nprd.")\n",
	...
	print $fhout $sep,$rdb{$itres,$par{"sec_kwdri"}};
