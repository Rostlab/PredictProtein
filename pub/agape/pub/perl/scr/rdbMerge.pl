...
$scrGoal="merges RDB files (if different number of rows: fill in empty)\n".
    "     \t \n".
    "     \t input:  <*.rdb|file_rdb.list>\n".
    "     \t output: merged_rdb_file\n".
    "     \t \n".
    "     \t ";
#  
# 
...
#	$fileIn,$ctfile,(100*$ctfile/$#fileIn);
	...
	foreach $tmpx (@tmpx){
	    if ($tmpx=~/\t/){
		push(@tmprd,split(/\t/,$tmpx));}
	    else {
		push(@tmprd,$tmpx);}
	}
	if (! defined $rd{$ctfile,"NCOL"}){
	    $rd{$ctfile,"NCOL"}=$#tmprd;
	}
	
	$rd{$ctfile,$ctrow}=join("\t",@tmprd);
	$rd{$ctfile}=$ctrow;
	$rd{$ctfile,"NROWS"}=$ctrow;
	$res{"NROWS","max"}=$ctrow if ($ctrow > $res{"NROWS","max"});
    }
    close($fhin);
}
$rd{"NROWS"}=$ctfile;
...
foreach $itfile (1..$rd{"NROWS"}){
    next if (! defined $rd{$itfile});
    if (length($rd{$itfile,"header"})>5){
	print $fhout
	    "# \n",
	    "# file $itfile\n",
	    $rd{$itfile,"header"},
...
foreach $itrow (1..$res{"NROWS","max"}){
    $#tmp=0;
    foreach $itfile (1..$rd{"NROWS"}){
	next if (! defined $rd{$itfile});
	if (! defined $rd{$itfile,$itrow}){
	    foreach $tmp (1..$rd{$itfile,"NCOL"}){
		push(@tmp,"");
	    }
	}
	else {
	    push(@tmp,split(/\t/,$rd{$itfile,$itrow}));
	}
    }
    print $fhout
	join("\t",@tmp),"\n";
