...
$scrGoal="removes strange characters (non AA) from FASTA files\n".
    "     \t file.fasta (or list)\n".
    "     \t new";
#  
# 
...
#		next;}
    ...
    close($fhin);
				# ------------------------------
				# now correct mistakes
    foreach $id (@id){
				# all capital
	$seq{$id}=~tr/[a-z]/[A-Z]/;
	$seq{$id}=~s/[^ABCDEFGHIKLMNPQRSTUVWXYZ]//g;
    }
				# ------------------------------
				# move old file
    $old= $fileIn;
	...
	foreach $it (1..$#tmp){
	    print $fhout $tmp[$it];
				# new line
	    if    (int($it/$nchar_perline) == ($it/$nchar_perline)){
		print $fhout "\n";
	    }
				# space every 
	    elsif (int($it/$nchar_space) == ($it/$nchar_space)){
		print $fhout " ";
	    }
	}
	print $fhout "\n";
