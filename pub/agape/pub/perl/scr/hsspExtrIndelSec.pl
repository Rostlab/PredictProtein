...
$scrGoal="extracts sec str segment based statistics about insertions and deletions\n".
    "     \t input:  HSSP file(s)\n".
    "     \t output: statistics\n".
    "     \t \n".
    "     \t ";
...
%par=(
      'extOut',                 ".dat",
      'extHssp',                ".hssp",
      'minLenNot',              10,           # compiling info (log) for background
				              #    non-seg segments must be longer than this!
      '', "",			# 
      );
@kwd=sort (keys %par);
$Ldebug=0;
...
if ($#ARGV<1 ||			# help
    ...
    printf "%5s %-15s=%-20s %-s\n","","title",     "x",       "file out= 'Out-'TITLE (all columns with TITLE";

#    printf "%5s %-15s=%-20s %-s\n","","",   "x", "";
#    printf "%5s %-15s %-20s %-s\n","","",        "",         "continued";
#    printf "%5s %-15s %-20s %-s\n","","",   "no value","";

    printf "%5s %-15s=%-20s %-s\n","","sec",       "HELTSBG", "analyse respective sec str";
...
$kwd2skipHssp="noacc,noali,noalifill,noins,nopair";
@kwd2readHssp=
    (
     "HYDRO",
     "CONS",
...
#    next if ($arg eq $ARGV[1]);
    ...
    elsif ($arg=~/^sec=([HGIEBLST]*)$/)   { $secGet=         $1;}
...
#    elsif ($arg=~/^=(.*)$/){ $=$1;}
					    ...
					    push(@chainIn,"*") if ($arg !~ /\.list/);}
    elsif ($arg =~ /$par{"extHssp"}/)     { $file= $arg;
					    $chain=$arg; 
					    $chain=~s/^(.*)[_:](.)$/$2/;
					    $file= $1;
					    if (! -e $file){
						print "*** problem arg=$arg, file=$file!\n";
						exit;}
					    push(@fileIn, $arg); 
					    push(@chainIn,$chain);}
    else {
...
undef %res;
				# ------------------------------
				# xx beg helix special head
$fileOuttmp=$fileOut."_tm";
open($fhout,">".$fileOuttmp) || warn "*** $scrName ERROR creating fileOuttmp=$fileOuttmp";
$wrt= "id";
foreach $kwd (@kwd2readHssp){
#    foreach $kwd2 ("H","not"){
#	$wrt.=$sep."num".$kwd."_".$kwd2.$title;
#    }
    foreach $kwd2 ("H","not","notseg"){
	$wrt.=$sep."ave".$kwd."_".$kwd2.$title;
    }
    foreach $kwd2 ("H"){
	$wrt.=$sep.$kwd."_infseg".$kwd2.$title;
    }
    foreach $kwd2 ("H"){
	$wrt.=$sep.$kwd."_infall".$kwd2.$title;
    }
}
$wrt.=$sep."numali".$title;
$wrt.=$sep."numres".$title;
$wrt.=$sep."numTMH".$title;
print $fhout 
    $wrt,"\n";
print "xx:",$wrt,"\n"           if ($Ldebug);

foreach $kwd (@kwd2readHssp){
    foreach $kwd2 (@secGet,"not"){
	$tmpsum{$kwd2,$kwd}=      0;
	$tmpsum{$kwd2,$kwd,"ave"}=0;
    }
    foreach $kwd2 (@secGet){
	$tmpsum{$kwd2,$kwd,"inf","seg"}=0;
	$tmpsum{$kwd2,$kwd,"inf","ave"}=0;
    }
}
	
				# xx end helix special head
				# ------------------------------
...
foreach $fileIn (@fileIn){
    ...
    $id=~s/^.*\///g;
    $id=~s/\..*$//g;
    $chain=$chainIn[$ctfile];
    $idc=$id;
    $idc.="_".$chain            if ($chain ne "*");

				# ------------------------------
				# read HSSP
    $kwdtmp=$kwd2skipHssp;
				# read particular chain?
    $kwdtmp.=",".$chainIn[$ctfile] if (defined $chainIn[$ctfile] &&
				       $chainIn[$ctfile] ne "*");

#    print "--- $scrName: working on fileIn=$fileIn!\n";
    printf "--- $scrName: working on %-10s %4d (%4.1f perc of job)\n",$idc,$ctfilein,(100*$ctfilein/$#fileIn);
    ...
    &errScrMsg("failed hsspRd on file=$fileIn, kwd=$kwd",$msg,$scrName)
	if (! $Lok);

    if (0){
	print "xx file=$fileIn kwd=$kwdtmp\n";
	foreach $kwd (sort keys %hssp){
	    next if ($kwd=~/[23456789]/);
	    next if ($kwd!~/[NDEL|NOCC|NINS|SEC]/i);
	    print "xx kwd=",$kwd,", val=",$hssp{$kwd},"\n";
	}
	die;
    }
				# ------------------------------
	...
	foreach $itres (1..$hssp{"NUMRES"}){
	    $hssp{"prof",$kwdhssp,$itres}=0;
	    foreach $aa (@hydrophobic){
		$hssp{"prof",$kwdhssp,$itres}+=$hssp{"prof",$aa,$itres};
	    }
	    $hssp{"prof",$kwdhssp,$itres}=int($hssp{"prof",$kwdhssp,$itres}/$#hydrophobic);
	}
    }
	...
	$res{$idc}=1;}
				# keep general stuff
    $res{$idc,"numali"}=$hssp{"NALIGN"};
    $res{$idc,"numres"}=$hssp{"NUMRES"};
				# ini
    $res{$idc,"not","numres"}=0;
    foreach $sec (@secGet){
	$res{$idc,$sec,"numseg"}=0;
	$res{$idc,$sec,"numres"}=0;
    }
    foreach $kwd (@kwd2readHssp){
	foreach $kwdtmp ("not",@secGet){
	    $res{$idc,$kwdtmp,$kwd}=0;
	    $res{$idc,$kwdtmp,$kwd}=0;
	}}
	...
	if ($secHere !~ /$secGetOr/){
	    ...
	    $kwdtmp= $secHere;
				# new segment?
	    if ($itres > 1 && $hssp{"sec",$itres} ne $hssp{"sec",($itres-1)}){
				# averages over previous
		++$res{$idc,$secHere,"numseg"};
		$ctsegHere=$res{$idc,$secHere,"numseg"};
		$res{$idc,$secHere,$ctsegHere,"numres"}=0;
		foreach $kwd (@kwd2readHssp){
		    $res{$idc,$secHere,$ctsegHere,$kwd}=0;
		}
	    }
	    ++$res{$idc,$secHere,"numres"};
	    ++$res{$idc,$secHere,$ctsegHere,"numres"};
	}
				# take hssp for one residue
	foreach $kwd (@kwd2readHssp){
	    $kwdhssp=$kwd;
	    $kwdhssp="WEIGHT"   if ($kwd=~/CONS/);
	    die if (! defined $kwdtmp);
	    if ($kwdtmp eq "not"){
#		print "xx kwdtmp=$kwdtmp, val bef=",$res{$idc,$kwdtmp,$kwd},"\n";
		if (! defined $idc || ! defined $kwdtmp || ! defined $kwd ||
		    ! defined $res{$idc,$kwdtmp,$kwd}
		    ){
		    print "xx not ini $idc,$kwdtmp,$kwd!\n";
		    die;}
		$res{$idc,$kwdtmp,$kwd}+=$hssp{"prof",$kwdhssp,$itres};
		if (! defined $hssp{"prof",$kwdhssp,$itres}){
		    print "xx missing HSSP(prof,$kwdhssp($kwd),$itres),\n";
		    die;}
	    }
	    else {
#		print "xx sec=$secHere, ct=$ctsegHere, $kwd, val bef=",$res{$idc,$secHere,$ctsegHere,$kwd},"\n";
		
		$res{$idc,$secHere,$ctsegHere,$kwd}+=$hssp{"prof",$kwdhssp,$itres};
	    }
	}
    }
				# ------------------------------
				# find all similar lengths non-segments
    foreach $sec (@secGet){
	$res{$idc,$sec,"not","beg"}="";
	$res{$idc,$sec,"not","end"}="";
				# loop over all residues
	$itres2=1;
	while ($itres2 < $hssp{"NUMRES"}){
	    ++$itres2;
	    next if ($hssp{"sec",$itres2} eq $sec);
	    $beg=$itres2;
	    while ($itres2 < $hssp{"NUMRES"} &&
		   $hssp{"sec",$itres2} ne $sec){
		++$itres2;
	    }
	    $len=(1+$itres2-$beg);
				# only those longer
	    next if ($len < $par{"minLenNot"});
				# seems ok, keep!
	    $res{$idc,$sec,"not","beg"}.=$beg.",";
	    $res{$idc,$sec,"not","end"}.=$itres2.",";
	}
	$res{$idc,$sec,"not","beg"}=~s/,*$//g;
	$res{$idc,$sec,"not","end"}=~s/,*$//g;
    }

				# ------------------------------
				# now average over all segments asf
				# all segments
    foreach $kwd (@kwd2readHssp){
	foreach $kwd2 ("not"){
	    $tmp3=sprintf("%8.4f",$res{$idc,$kwd2,$kwd}/$res{$idc,$kwd2,"numres"}); 
	    $tmp3=~s/\s//g;
	    $res{$idc,$kwd2,$kwd,"ave"}=$tmp3;
	}
	$kwdhssp=$kwd;
	$kwdhssp="WEIGHT"   if ($kwd=~/CONS/);
	foreach $sec (@secGet){
				# get background averages
	    $res{$idc,"not",$kwd,"aveseg"}=0;
	    if (length($res{$idc,$sec,"not","beg"}) > 0){
		@beg=split(/,/,$res{$idc,$sec,"not","beg"});
		@end=split(/,/,$res{$idc,$sec,"not","end"});
		$not=0;
		foreach $ittmp (1..$#beg){
		    $tmp=$cttmp=0;
		    foreach $itres2 ($beg[$ittmp] .. $end[$ittmp]){
			$tmp+=$hssp{"prof",$kwdhssp,$itres2};
			++$cttmp;
		    }
		    $not+=$tmp/$cttmp;
		}
		$res{$idc,"not",$kwd,"aveseg"}=$not/$#beg;
	    }
				# for each segment
	    foreach $itseg (1..$res{$idc,$sec,"numseg"}){
				# average
		$tmp3=sprintf("%8.4f",$res{$idc,$sec,$itseg,$kwd}/$res{$idc,$sec,$itseg,"numres"}); 
		$tmp3=~s/\s//g;
		$res{$idc,$sec,$itseg,$kwd,"ave"}=$tmp3;
				# info
		$res{$idc,$sec,$itseg,$kwd,"inf","seg"}=$info_shoot;
		$res{$idc,$sec,$itseg,$kwd,"inf","ave"}=$info_shoot;
		if ($res{$idc,$sec,$itseg,$kwd,"ave"} > 0 &&
		    $res{$idc,"not",$kwd,"ave"}> 0){
		    $tmp3=sprintf("%8.4f",
				  log($res{$idc,$sec,$itseg,$kwd,"ave"}/$res{$idc,"not",$kwd,"ave"}));
		    $tmp3=~s/\s//g;
		    $res{$idc,$sec,$itseg,$kwd,"inf","ave"}=$tmp3;}
				# compile length specific non-average
		if ($res{$idc,$sec,$itseg,$kwd} > 0 && 
		    $res{$idc,"not",$kwd,"aveseg"} > 0){
		    $tmp3=sprintf("%8.4f",
				  log($res{$idc,$sec,$itseg,$kwd}/$res{$idc,"not",$kwd,"aveseg"}));
		    $tmp3=~s/\s//g;
		    $res{$idc,$sec,$itseg,$kwd,"inf","seg"}=$tmp3;}
	    }
	}
    }
				# --------------------------------------------------
				# xx beg per helix output body
				# --------------------------------------------------
    $sec="H";
    $Lerr=0;
    foreach $itseg (1..$res{$idc,$sec,"numseg"}){
	$wrt=$idc."_n=".$itseg;
				# numbers
	foreach $kwd (@kwd2readHssp){
#	    $wrt.=$sep.sprintf("%6.2f",$res{$idc,$sec,$itseg,$kwd});
#	    $wrt.=$sep.sprintf("%6.2f",$res{$idc,"not",$kwd});
				# averages
	    $wrt.=$sep.sprintf("%7.3f",$res{$idc,$sec,$itseg,$kwd,"ave"});
	    $wrt.=$sep.sprintf("%7.3f",$res{$idc,"not",$kwd,"ave"});
	    $wrt.=$sep.sprintf("%7.3f",$res{$idc,"not",$kwd,"aveseg"});
				# log info
	    $wrt.=$sep.sprintf("%8.4f",$res{$idc,$sec,$itseg,$kwd,"inf","seg"});
	    $wrt.=$sep.sprintf("%8.4f",$res{$idc,$sec,$itseg,$kwd,"inf","ave"});
				# error check
	    if ($kwd=~/CONS/ && 
		($res{$idc,$sec,$itseg,$kwd,"ave"}>2 || $res{$idc,$sec,$itseg,$kwd,"ave"}<0.5)){
		print "xx id=$idc $kwd itseg=$itseg sec=$sec ave=",$res{$idc,$sec,$itseg,$kwd,"ave"},"\n";
		$Lerr=1;}
	    if ($kwd=~/CONS/ && 
		($res{$idc,"not",$kwd,"aveseg"}>2 || $res{$idc,"not",$kwd,"aveseg"}<0.5)){
		print "xx id=$idc $kwd itseg=$itseg not_ave=",$res{$idc,"not",$kwd,"aveseg"},"\n";
		$Lerr=1;}
	    if ($kwd=~/CONS/ && 
		($res{$idc,"not",$kwd,"ave"}>2 || $res{$idc,"not",$kwd,"ave"}<0.5)){
		print "xx id=$idc $kwd itseg=$itseg not_ave=",$res{$idc,"not",$kwd,"ave"},"\n";
		$Lerr=1;}
	    if ($kwd=~/NINS|NDEL|NOCC/ && 
		$res{$idc,$sec,$itseg,$kwd,"ave"}<0){
		print "xx id=$idc $kwd itseg=$itseg sec=$sec ave=",$res{$idc,$sec,$itseg,$kwd,"ave"},"\n";
		$Lerr=1;}
	    if ($kwd=~/NINS|NDEL|NOCC/ && 
		$res{$idc,"not",$kwd,"ave"}<0){
	    ...
	    if ($kwd=~/NINS|NDEL|NOCC/ && 
		$res{$idc,"not",$kwd,"aveseg"}<0){
		print "xx id=$idc $kwd itseg=$itseg not_aveseg=",$res{$idc,"not",$kwd,"aveseg"},"\n";
		$Lerr=1;}

				# sum up
	    $tmpsum{$sec,$kwd}+=             $res{$idc,$sec,$itseg,$kwd};
	    $tmpsum{"not",$kwd}+=            $res{$idc,"not",$kwd};
	    $tmpsum{$sec,$kwd,"ave"}+=       $res{$idc,$sec,$itseg,$kwd,"ave"};
	    $tmpsum{"not",$kwd,"ave"}+=      $res{$idc,"not",$kwd,"ave"};
	    $tmpsum{"not",$kwd,"aveseg"}+=   $res{$idc,"not",$kwd,"aveseg"};
	    if ($res{$idc,$sec,$itseg,$kwd,"inf","seg"}       < $info_shoot){
		$tmpsum{$sec,$kwd,"inf","seg"}+= $res{$idc,$sec,$itseg,$kwd,"inf","seg"};
	    }
	    if ($res{$idc,$sec,$itseg,$kwd,"inf","ave"} < $info_shoot){
		$tmpsum{$sec,$kwd,"inf","ave"}+= $res{$idc,$sec,$itseg,$kwd,"inf","ave"};
	    }
	}
				# general stuff
	$wrt.=$sep.$res{$idc,"numali"};        $tmpsum{"numali"}+=$res{$idc,"numali"}; 
	$wrt.=$sep.$res{$idc,"numres"};        $tmpsum{"numres"}+=$res{$idc,"numres"}; 
	$wrt.=$sep.$res{$idc,$sec,"numseg"};   

	print $fhout $wrt,"\n";
	print "xx:",$wrt,"\n"           if ($Ldebug);
    }
    $tmpsum{"numseg"}+=$res{$idc,$sec,"numseg"};
    if ($Lerr){
	die"*** ERROR in $scrName\n";
    }
				# --------------------------------------------------
				# xx end per helix output body
				# --------------------------------------------------
}
				# --------------------------------------------------
				# xx beg per helix output
				# --------------------------------------------------
				# averages
$sec="H";
$wrt="sumall";
$wrt.="_".$title                if (length($title)>1);
foreach $kwd (@kwd2readHssp){
#    $wrt.=$sep.sprintf("%6.2f",$tmpsum{$sec,$kwd}/$tmpsum{"numseg"});
#    $wrt.=$sep.sprintf("%6.2f",$tmpsum{"not",$kwd}/$tmpsum{"numseg"});
    $wrt.=$sep.sprintf("%7.3f",$tmpsum{$sec,$kwd,"ave"}/$tmpsum{"numseg"});
    $wrt.=$sep.sprintf("%7.3f",$tmpsum{"not",$kwd,"ave"}/$tmpsum{"numseg"});
    $wrt.=$sep.sprintf("%7.3f",$tmpsum{"not",$kwd,"aveseg"}/$tmpsum{"numseg"});
    $wrt.=$sep.sprintf("%8.4f",$tmpsum{$sec,$kwd,"inf","seg"}/$tmpsum{"numseg"});
    $wrt.=$sep.sprintf("%8.4f",$tmpsum{$sec,$kwd,"inf","ave"}/$tmpsum{"numseg"});
}
$wrt.=$sep.sprintf("%5.1f",$tmpsum{"numali"}/$tmpsum{"numseg"});       
$wrt.=$sep.sprintf("%5.1f",$tmpsum{"numres"}/$tmpsum{"numseg"});       
$wrt.=$sep.sprintf("%5.1f",$tmpsum{"numseg"}/$tmpsum{"numseg"});  
print $fhout $wrt,"\n";
print "xx:",$wrt,"\n"           if ($Ldebug);
close($fhout);
				...
				# --------------------------------------------------
				# xx end per helix output
				# --------------------------------------------------

print "--- note: all helices in $fileOuttmp\n";

				# --------------------------------------------------
				# (2) averages over all segments in each protein
				#     and average over data set
				# --------------------------------------------------
$Lerrend=0;
				# ini
foreach $kwd (@kwd2readHssp){
    foreach $kwd2 ("not",@secGet){
#	$res{"sum",$kwd2,$kwd}=              0;
	$res{"sum",$kwd2,$kwd,"ave"}=        0;
    }
    foreach $kwd2 (@secGet){
	$res{"sum",$kwd2,$kwd,"inf","seg"}=  0;
	$res{"sum",$kwd2,$kwd,"inf","ave"}=  0;
    }
}
foreach $sec (@secGet){
    $res{"sum",$sec,"numseg"}=               0;
}
$res{"sum","numali"}=                        0;
$res{"sum","numres"}=                        0;
...
foreach $idc (@idc){
				# sums
    $res{"sum","numali"}+=                  $res{$idc,"numali"};
    $res{"sum","numres"}+=                  $res{$idc,"numres"};
    foreach $kwd (@kwd2readHssp){
#	$res{"sum","not",$kwd}+=            $res{$idc,"not",$kwd};
	$res{"sum","not",$kwd,"ave"}+=      $res{$idc,"not",$kwd,"ave"};
    }
    foreach $sec (@secGet){
	$res{"sum",$sec,"numseg"}+=         $res{$idc,$sec,"numseg"};

	foreach $kwd (@kwd2readHssp){
#	    $res{$idc,$sec,$kwd}=            0;
	    $res{$idc,$sec,$kwd,"ave"}=      0;
	    $res{$idc,$sec,$kwd,"inf","seg"}=0;
	    $res{$idc,$sec,$kwd,"inf","ave"}=0;
				# averages over all segments 'SEC' in that protein
	    foreach $itseg (1..$res{$idc,$sec,"numseg"}){
#		$res{$idc,$sec,$kwd}+=            $res{$idc,$sec,$itseg,$kwd};
		$res{$idc,$sec,$kwd,"ave"}+=      $res{$idc,$sec,$itseg,$kwd,"ave"};
		if ($res{$idc,$sec,$itseg,$kwd,"inf","seg"}<$info_shoot){
		    $res{$idc,$sec,$kwd,"inf","seg"}+=$res{$idc,$sec,$itseg,$kwd,"inf","seg"};}
		if ($res{$idc,$sec,$itseg,$kwd,"inf","ave"}<$info_shoot){
		    $res{$idc,$sec,$kwd,"inf","ave"}+=$res{$idc,$sec,$itseg,$kwd,"inf","ave"};}
	    }
	    $nseg=$res{$idc,$sec,"numseg"};
	    if ($res{$idc,$sec,"numseg"} > 0){
#		$res{$idc,$sec,$kwd}=            $res{$idc,$sec,$kwd}            /$nseg;
		$res{$idc,$sec,$kwd,"ave"}=      $res{$idc,$sec,$kwd,"ave"}      /$nseg;
		$res{$idc,$sec,$kwd,"inf","seg"}=$res{$idc,$sec,$kwd,"inf","seg"}/$nseg;
		$res{$idc,$sec,$kwd,"inf","ave"}=$res{$idc,$sec,$kwd,"inf","ave"}/$nseg;
				# sums
#		$res{"sum",$sec,$kwd}+=          $res{$idc,$sec,$kwd};
		$res{"sum",$sec,$kwd,"ave"}+=    $res{$idc,$sec,$kwd,"ave"};
		if ($res{$idc,$sec,$kwd,"inf","seg"} < $info_shoot){
		    $res{"sum",$sec,$kwd,"inf","seg"}+=      $res{$idc,$sec,$kwd,"inf","seg"};}
		if ($res{$idc,$sec,$kwd,"inf","ave"} < $info_shoot){
		    $res{"sum",$sec,$kwd,"inf","ave"}+=$res{$idc,$sec,$kwd,"inf","ave"};}
	    }
	}
...
open($fhout,">".$fileOut) || warn "*** $scrName ERROR creating fileOut=$fileOut";
				...
				# ------------------------------
				# header
$wrt= "id";                                  $wrt.="_".$title if (length($title));
foreach $kwd (@kwd2readHssp){
#    foreach $kwd2 (@secGet,"not"){
#	$wrt.=$sep."num".$kwd."_".$kwd2.$title;
#    }
    ...
    foreach $kwd2 (@secGet){
	$wrt.=$sep.$kwd."_infave_".$kwd2.$title;
	$wrt.=$sep.$kwd."_infseg_".$kwd2.$title;
    }
}
$wrt.=$sep."numali".$title;
$wrt.=$sep."numres".$title;
foreach $sec (@secGet){
    $wrt.=$sep."numSeg_".$sec.$title;
}
...
foreach $idc (@idc){
    $wrt=$idc;
#    $wrt=$idc.$sep.$res{$idc,"numali"}.$sep.$res{$idc,"numres"};

    foreach $kwd (@kwd2readHssp){
#	foreach $kwd2 (@secGet,"not"){
#	    $wrt.=$sep.sprintf("%6.2f",  $res{$idc,$kwd2,$kwd});
#	    if (! defined $res{$idc,$kwd2,$kwd}){print "xx missing $idc,$kwd2,$kwd, res\n";die;}
#	}
	foreach $kwd2 (@secGet,"not"){
	    $wrt.=$sep.sprintf("%7.3f",$res{$idc,$kwd2,$kwd,"ave"});
	}
	foreach $kwd2 (@secGet){
	    $wrt.=$sep.sprintf("%8.4f",$res{$idc,$kwd2,$kwd,"inf","ave"});
	    $wrt.=$sep.sprintf("%8.4f",$res{$idc,$kwd2,$kwd,"inf","seg"});
	}
    }
    $wrt.=$sep.sprintf("%5d", $res{$idc,"numali"});
    $wrt.=$sep.sprintf("%5d", $res{$idc,"numres"});
    foreach $sec (@secGet){
	$wrt.=$sep.sprintf("%5d", $res{$idc,$sec,"numseg"});
    }
    ...
    print $wrt,"\n"              if ($Ldebug || $Lverb);
				...
				# averages
$wrt="sumall"; 
$wrt.="_".$title if (length($title));

foreach $kwd (@kwd2readHssp){
#    foreach $kwd2 (@secGet,"not"){
#	$wrt.=$sep.sprintf("%6.2f",  $res{"sum",$kwd2,$kwd}/$#idc);
#    }
    foreach $kwd2 (@secGet,"not"){
	$wrt.=$sep.sprintf("%7.3f",$res{"sum",$kwd2,$kwd,"ave"}/$#idc);
    }
    foreach $kwd2 (@secGet){
	$wrt.=$sep.sprintf("%8.4f",$res{"sum",$kwd2,$kwd,"inf","ave"}/$#idc);
	$wrt.=$sep.sprintf("%8.4f",$res{"sum",$kwd2,$kwd,"inf","seg"}/$#idc);
    }
}
$wrt.=$sep.sprintf("%5.1f", $res{"sum","numali"}/$#idc);
$wrt.=$sep.sprintf("%5.1f", $res{"sum","numres"}/$#idc);
foreach $sec (@secGet){
    $wrt.=$sep.sprintf("%5.1f", $res{"sum",$sec,"numseg"}/$#idc);
}
...
#-------------------------------------------------------------------------------
	...
	foreach $kwd (@hsspRd_iniKwdPair){
	    ...
	    if (defined $ptr && defined $tmp[$ptr]){
		$val=$tmp[$ptr]; 
