...
if ($#ARGV<1){print "goal:   $scrGoal\n";
	      ...
	      print "        \n";
	      print "note:   need full path of SWISS-IDs\n";
	      print "        OR:\n";
	      print "        assume: hmaa_drome to be found in:\n";
	      print "                /data/swissprot/current/d/hmaa_drome\n";
	      print "        \n";
...
foreach $it (1..$#ARGV){
	...
	$tmp=$ARGV[$it];$tmp=~s/^dir=//g;
	$par{"dir"}=&complete_dir($tmp);} # external lib-ut.pl
...
foreach $file (@file){
    ...
    if (! $Lok){ print "*** ERROR $scrName: swissGetLocation returned msg=\n$msg\n";
		 die; }
