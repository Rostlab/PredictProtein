...
$scrIn=  "<file.hssp*|file.hssp_C|file.list>";
$scrGoal="extracts conservation weight and sequence\n".
    "     \t input:  ".$scrIn."\n".
    "     \t output: \n".
    "     \t need:   \n".
    "     \t \n".
...
$kwd2call="nopair,noali,noseq,nosec,noacc,noins,nalifill";
...
if (! defined $fileOut){
	...
	$fileOut="Out-".$fileOut;
				...
				# ------------------------------
				# read list
if ($#fileIn==1 && $fileIn[1]=~/list$/){
    open($fhin,$fileIn[1])||warn "*** $scrName ERROR reading filein=$fileIn[1]\n";
    $#tmp=0;
    $#chainIn=0;
    while(<$fhin>){
	chop;
...
$ctfile=0;
foreach $it (1..10){
    $fileOut.="$it"             if (-e $fileOut);
}
    
open($fhout,">".$fileOut) || warn "*** $scrName ERROR creating fileOut=$fileOut";

printf $fhout
    "%5s$sep%3s$sep%5s$sep%3s$sep%6s\n",
    "id","chn","itres","seq","cw";

$#id=$#cwall=$#cwrange=0;
foreach $fileIn (@fileIn){
    ++$ctfile;
    $chainIn=$chainIn[$ctfile];
...
#    printf "--- $scrName: working on %-25s %4d (%4.1f perc of job)\n",$fileIn,$ctfile,(100*$ctfile/$#fileIn);
    ...
    $kwdlocal=$kwd2call;
    $kwdlocal.=",chn=".$chainIn if ($chainIn ne "*");

    ($Lok,$msg)=
	&hsspRd($fileIn,$kwdlocal);
    if (! $Lok){
	print "*** ERROR $scrName filein=$fileIn, chain=$chainIn, error message:\n";
	print "$msg\n";
	exit;
    }
...
#    $id.="_".$chainIn if ($chainIn ne "*");
    ...
    foreach $it (1..$nres){
	if (! defined $hssp{"prof","WEIGHT",$it} ||
	    ! defined $hssp{"seq",$it}){
	    if (! defined $hssp{"prof","WEIGHT",$it}){
		print "*** missing weight for res $it, file=$fileIn, chain=$chainIn\n";
	    }
	    if (! defined $hssp{"seq",$it}){
		print "*** missing seq for res $it, file=$fileIn, chain=$chainIn\n";
	    }
	    exit;
	}

	$res{$id,"cw",$it}= $hssp{"prof","WEIGHT",$it};
	$res{$id,"seq",$it}=$hssp{"seq",$it};
	$cw=$res{$id,"cw",$it};
	push(@cwall,$cw);
	if (! defined $cw{$cw}){
	    push(@cwrange,$cw);
    ...
    foreach $it (1..$nres){
	printf $fhout
	    "%5s$sep%3s$sep%5d$sep%3s$sep%6.2f\n",
	    $id,$chainIn,$it,$res{$id,"seq",$it},$res{$id,"cw",$it};
	print $it, " " ,$res{$id,"seq",$it},", ",$res{$id,"cw",$it},"\n"
	    if ($Ldebug);
    }
}
close($fhout);
				...
				# (3) write output
				# ------------------------------
$wrt=sprintf("%5s$sep%10s$sep%8s$sep%10s$sep%8s\n",
	     "cw","Nocc","Pocc","NCocc","PCocc");
@cwrange=sort bynumber (@cwrange);
$ctall=$#cwall;
$sum=0;
foreach $cw (@cwrange){
    $sum+=$cw{$cw};
    $wrt.=sprintf("%6.2f$sep%10d$sep%6.2f$sep%10d$sep%6.2f\n",
		  $cw,
		  $cw{$cw},100*($cw{$cw}/$ctall),
		  $sum,100*($sum/$ctall));
		  
}
print $wrt,"\n";
    
$fileOut2=$fileOut."_dis";
