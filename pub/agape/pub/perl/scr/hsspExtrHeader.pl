...
$#fileHssp=$LisList=0;
foreach $_ (@ARGV){
    if   ($_=~/^(low|lowPide)=(.+)$/){$lowPide=$2;    $lowPide=~s/\s//g;}
    elsif($_=~/^(up|upPide)=(.+)$/)  {$upPide=$2;     $upPide=~s/\s//g;}
    elsif($_=~/^file.*Rdb=(.+)$/)    {$fileOutRdb=$1; $fileOutRdb=~s/\s//g;}
    elsif($_=~/^fileTrue=(.+)$/)     {$fileTrue=$1; $fileTrue=~s/\s//g;}
    elsif($_=~/^(lowLen|len)=/)      {$_=~s/^.*[Ll]en=//g;$_=~s/\s//g;$lowLali=$_;}
    elsif($_=~/^lowLali=/)           {$_=~s/^.*[Ll]en=//g;$_=~s/\s//g;$lowLali=$_;}
    elsif($_=~/^lowR12=/)            {$_=~s/^lowR12=//g;$_=~s/\s//g;$lowR12=$_;}
    elsif($_=~/^lowR1A=/)            {$_=~s/^lowR1A=//g;$_=~s/\s//g;$lowR1A=$_;}
    elsif($_=~/^lowR2A=/)            {$_=~s/^lowR2A=//g;$_=~s/\s//g;$lowR2A=$_;}
    elsif($_=~/^title=/)             {$_=~s/^title=//g;$_=~s/\s//g;$title=$_;}
    elsif($_=~/^chain=/)             {$_=~s/^chain=//g;$_=~s/\s//g;$chain=$_;}
    elsif($_=~/^wrtRdb/)             {$LwrtRdb=1;}
    elsif($_=~/^notExcl/)            {$LexclSelf=0;}
    elsif($_=~/^notStat/)            {$Lstat=0;}
    elsif($_=~/^not(_?[sS]creen|_?[vV]erbose)/) {$Lscreen=0;}
    else {
	$tmp=$_;$tmp=~s/\n//g;
...
foreach $fileIn (@fileHssp){
	...
	if ($Lstat){
		...
		($dist,$msg)=
		    &getDistanceHsspNewCurve($rd{"$itSeq","LALI"});
		$res{"$idStr","distance"}=($res{"$idStr","pide"} - $dist);
...
if ($Lscreen){ 
    print "--- number of ids (unique) found=",$#idFound,"\n";
    print "--- write output into $fileOut\n";
    if ($Lstat){
	if ($ct==0){
	    print "--- ratio covered=NN (ct=0)\n";}
	else {
...
if ($Lscreen){ 
    ...
    foreach $file (@fileOut){print "$file,";}print"\n";}
