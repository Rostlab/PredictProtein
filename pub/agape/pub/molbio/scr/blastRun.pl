...
$scrIn=      "fasta-file(s), or list-thereof, or FASTAmul (kwd=<big|swiss>)";
...
#-------------------------------------------------------------------------------
    ...
    $SBR="$tmp"."blastRunLoc";$fhinLoc="FHIN_"."blastRunLoc";
    ...
    if ($par{"doFormat"}){      # format db
        if ($par{"blastVersion"} =~ /3$/ ||
	    $par{"blastVersion"} =~ /psi/){  # new blast -> formatdb
            ($Lok,$msg,$par{"dbBlast"})=
    ...
    return(&errSbrMsg("failed setting env for BLAST",$msg,$SBR)) if (! $Lok);
	...
	return(&errSbrMsg("failed reading list=$fileIn (blastRunRdList)",$msg,$SBR)) if (! $Lok);
        ...
        if ( $isFastaMul && ! $par{"debug"}){
            foreach $file (@fileIn){
				...
				# --------------------------------------------------
    if (! &isFasta($fileIn[1])){
	$fileOutScreenCopfTmp=$par{"fileOutScreen"};
	$fileOutScreenCopfTmp="STDOUT" if ($par{"debug"});
        ($Lok,$msg,@fileTmp)=
	    &copf2fasta($par{"exeCopf"},$par{"exeConvertSeq"},1,$par{"titleTmp"},$par{"extFasta"},
                        $par{"dirWork"},$fileOutScreenCopfTmp,$fhTrace,@fileIn);
        return(&errSbrMsg("failed converting to FASTA",$msg,$SBR)) if (! $Lok);
        return(&errSbrMsg("N files converted=".$#fileTmp.", N files in=".$#fileIn,$msg,$SBR)) 
            if ($#fileTmp != $#fileIn);
        $Lchange=0;
	...
	$fileBlast=$par{"dirOut"}.$idIn.$par{"extBlast"}; 
				# br 99-09: change file name: remove 'TMP-BLAST- = titleTmp'
	$fileBlast=~s/$par{"titleTmp"}//g;
	$Lskip=0;
	$Lskip=1                if (! $par{"doNew"} && -e $fileBlast);
	if (! $Lskip && ! $par{"doNew"} && $par{"doGzip"}){
	    $fileBlastGzip=$fileBlast.".gz";
	    $Lskip=1            if (! $par{"doNew"} && -e $fileBlastGzip);
	    if ($Lskip){
		$cmd=$par{"exeGunzip"}." $fileBlastGzip";
		eval "\$cmdSys=\"$cmd\"";
		($Lok,$msg)=
		    &sysRunProg($cmdSys,$fileOutScreenLoc,$fhTrace);
		print $fhTrace "-*- WARN $scrName: failed to gunzip ($cmd=>$msg)\n" if (! $Lok); }}

				# simple BLASTP (blast2)   <<< BLASTP
	if (! $Lskip && $par{"blastVersion"} !~ /3$/ && $par{"blastVersion"} !~ /psi/ ){
	    ...
	    return(&errSbrMsg("failed blastp (".$par{"exeBlastp"}.
			      ": $fileIn->$fileBlast)",$msg,$SBR)) if (! $Lok); }
				# blast3 (blast3)           <<< NEW BLAST3
	elsif (! $Lskip&& $par{"blastVersion"} =~ /3$/){ # 
	    ...
	    return(&errSbrMsg("failed blast3 (".$par{"exeBlast3"}.
			      ": $fileIn->$fileBlast)",$msg,$SBR)) if (! $Lok); }
				# psi-blast (blastpsi)      <<< NEW PSI-blast (dariusz)
	elsif (! $Lskip&& $par{"blastVersion"} =~ /psi/){ # 
	    print $fhTrace "--- run blastpsi $fileIn -> $fileBlast (db=",$par{"dbBlast"},")\n";
	    ($Lok,$msg)= 
		&blastRunPsi($fileIn,$fileBlast,$par{"exeBlastpsi"},$par{"exeBlastpsiperl"},
			     $par{"dbBlast"},$par{"parPsi"},$par{"parE"},$par{"parB"},
			     $par{"fileOutScreen"},$fhTrace); 
	    return(&errSbrMsg("failed blastpsi (".$par{"exeBlastpsi"}.
			      ": $fileIn->$fileBlast)",$msg,$SBR)) if (! $Lok); }
                                # END of blast for 1 file  
                                # ##############################


				# ERROR: unclear what it is !
	elsif (! $Lskip){	# simple blast (blast2)     <<< old BLAST
	    print $fhTrace 
		"*** WARN blast version=",$par{"blastVersion"}," not recognised!\n",
		"--- run blastsimple $fileIn -> $fileBlast (db=",$par{"dbBlast"},")\n";
	    ($Lok,$msg)= 
		&blastpRunSimple($fileIn,$fileBlast,$par{"exeBlastp"},
				 $par{"dbBlast"},$par{"parE"},$par{"parB"},
				 $par{"fileOutScreen"},$fhTrace); 
				   
	    return(&errSbrMsg("failed blastsimple (".$par{"exeBlastp"}.
			      ": $fileIn->$fileBlast)",$msg,$SBR)) if (! $Lok); }
                                # END of blast for 1 file  
                                # ##############################
	...
	&summaryFileTmpWrt1($fileBlast) if ($par{"doThresh"});
				# compress output file
	if ($par{"doGzip"}){
	    $cmd=$par{"exeGzip"}." $fileBlast";
	    eval "\$cmdSys=\"$cmd\"";
	    ($Lok,$msg)=
		&sysRunProg($cmdSys,$fileOutScreenLoc,$fhTrace);
    ...
    return(1,"ok $SBR");}				# end of blastRunLoc
...
#==============================================================================
sub blastRunPsi {
    local($fileInLoc,$fileOutLoc,$exeBlastpsiLoc,$exeBlastpsiperlLoc,
	  $dbBlastLoc,$parPsiLoc,$parELoc,$parBLoc,$fileOutScreenLoc,$fhSbrErr) = @_ ;
    local($sbrName,$fhinLoc,$tmp,$Lok);
    $[ =1 ;
#-------------------------------------------------------------------------------
#   blastRunPsi           simply runs BLASTPGP from dariusz (blastpsi)
#                               NOTE: for security call &blastSetenv before running
#       in:                     $fileInLoc     : FASTA formatted input file
#       in:                     $fileOutLoc    : BLASTP output                if = 0: onto screen
#       in:                     $exeBlastLoc   : FORTRAN exe BLASTP           if = 0: default
#       in:                     $exeBlastpsiLoc: Dariusz exe                  if = 0: default
#       in:                     $dbBlastLoc    : BLASTP db to run             if = 0: swiss
#       in:                     $parPsiLoc     : explicit command to run PSI-blast
#       in:                     $parELoc       : BLASTP para E                if = 0: default
#       in:                     $parBLoc       : BLASTP para B                if = 0: default
#       in:                     $fileOutScreen : screen dumpb of system call  if = 0: 0 -> STDOUT
#       in:                     $fhSbrErr      : error file handle
...
#       err:                    (1,'ok'), (0,'message')
#-------------------------------------------------------------------------------
    $sbrName="lib-br:"."blastRunPsi";$fhinLoc="FHIN_"."blastRunPsi";
				# check arguments
    return(&errSbr("not def fileInLoc!"))          if (! defined $fileInLoc);
    return(&errSbr("miss in file '$fileInLoc'!"))  if (! -e $fileInLoc);
                                # ------------------------------
				# local defaults
    $exeBlastpsiLocDef=         "/home/rost/pub/molbio/blastpgp.".$ARCH;
#    $exeBlastpsiperlLocDef=     "/home/rost/pub/blast/blastpgp.pl";
    $dbBlastLocDef=             "big";
#    $databaseXDef=              "/data/blast/bigx_coil_seg";
#    $databaseBDef=              "/data/blast/big";
    $blastXargDef=              "-j 3 -e .0001 -h 1e-12 -F T -b 3000 -d";
    $blastXargDef=              "-j 3 -e 10000 -h 1e-12 -F T -b 3000 -d";
#    $blastBargDef=              "-e .001 -b 3000 -F T -d";
    ...
    $parBLocDef=             2000;
				...
				# check arguments
    $fileOutLoc=0               if (! defined $fileOutLoc);
    $exeBlastpsiLoc=$exeBlastpsiLocDef  
	                        if (! defined $exeBlastpsiLoc || ! $exeBlastpsiLoc);
    $dbBlastLoc=$dbBlastLocDef  if (! defined $dbBlastLoc || ! $dbBlastLoc);
    $parELoc=$parELocDef        if (! defined $parELoc || ! $parELoc);
    $parBLoc=$parBLocDef        if (! defined $parBLoc || ! $parBLoc);
    ...
    $fileOutScreenLoc=0         if (! defined $fileOutScreenLoc || ! $fileOutScreenLoc);
    $cmdBlast="";
    $cmdBlast=$parPsiLoc        if (defined $parPsiLoc && length($parPsiLoc) > 1);
    $cmdBlast.=$blastXargDef    if (length($cmdBlast) < 1);
    ...
    $cmd=  $exeBlastpsiperlLoc." ".$fileInLoc." exe=".$exeBlastpsiLoc;


				# xx hack to force
    $cmd.= " argBlast='".$cmdBlast."'"
	if (length($cmdBlast)>1);
				# xx hack to force

    $cmd.=" fileOut=$fileOutLoc" 
	if ($fileOutLoc);
    ...
    ($Lok,$msg)=
	&sysRunProg($cmdSys,$fileOutScreenLoc,$fhSbrErr);
    return(&errSbrMsg("failed to run BLASTPSI on ($fileInLoc)",$msg)) 
        if (! $Lok || ($fileOutLoc && ! -e $fileOutLoc));
...
#    system("setenv BLASTDB $BLASTDBloc"); # system call
    ...
    print "setenv BLASTMAT $BLASTMATloc\; setenv BLASTDB $BLASTDBloc\n";
...
#-------------------------------------------------------------------------------
    ...
    $cmd.=" V=".$par{"parV"}    if (defined $par{"parV"} && $par{"parV"} ne "500");
...
#-------------------------------------------------------------------------------
    ...
    $tmp{"special"}.=        "cmd,saf,mat,red=n,";
    $tmp{"special"}.=        "blast3".","."E".",". "B".",". "db"."," ;
    $tmp{"special"}.=        "big,pdb,swiss,trembl,human,mouse,";
    $tmp{"special"}.=        "minDist,gzip,low,";
    ...
    $tmp{"V"}=               "short for parV=x     -> set BLAST V parameter";
    ...
    $tmp{"saf"}=             "create SAF output      (note: only for PSI-blast)";
    $tmp{"mat"}=             "create BLAST_mat       (note: only for PSI-blast)";
    $tmp{"red=n"}=           "reduce redundancy      (note: only for PSI-blast)";
    $tmp{"cmd"}=             "command for PSI-blast  example";
    $tmp{"cmd"}.=    "---                   '-j 3 -e .001 -h 1e-12 -F T -b 3000 -d'";
    $tmp{"cmd"}.=    "---                   j: number of iterations            (here 3)";
    $tmp{"cmd"}.=    "---                   e: cut-off in Evalue               (here 10-3)";
    $tmp{"cmd"}.=    "---                   h: cut-off in Evalue for iteration (here 10-12)";

    $tmp{"blast3"}=          "will run Blast3        (note: default = BLASTP, i.e. blast2)";
    $tmp{"big"}=             "will run against PDB + SWISS-PROT + SWISS-NEW + TREMBL";
    $tmp{"swiss"}=           "will run against SWISS-PROT";
    $tmp{"trembl"}=          "will run against TREMBL";
    $tmp{"pdb"}=             "will run against PDB";
    $tmp{"human"}=           "will run against human (not up-to-date!)";
    $tmp{"mouse"}=           "will run against mouse (not up-to-date!)";

    $tmp{"ec"}=              "will run against enzymes only (version summer 2001)";
...
#                            "------------------------------------------------------------\n";
	...
	elsif ($arg=~/^big$/)                 { $par{"dbBlast"}=     $par{"dbBig"}; }
	elsif ($arg=~/^swiss$/)               { $par{"dbBlast"}=     $par{"dbSwiss"}; }
	elsif ($arg=~/^trembl$/)              { $par{"dbBlast"}=     $par{"dbTrembl"}; }
	elsif ($arg=~/^pdb$/)                 { $par{"dbBlast"}=     $par{"dbPdb"}; }
	elsif ($arg=~/^pdbuni$/)              { $par{"dbBlast"}=     $par{"dbPdbuni"}; }

				# genomes
	elsif ($arg=~/^caeel$/)               { $par{"dbBlast"}=     $par{"dbCaeel"}; }
	elsif ($arg=~/^drome$/)               { $par{"dbBlast"}=     $par{"dbDrome"}; }
	elsif ($arg=~/^yeast$/)               { $par{"dbBlast"}=     $par{"dbYeast"}; }
	elsif ($arg=~/^metja$/)               { $par{"dbBlast"}=     $par{"dbMetja"}; }
	elsif ($arg=~/^human$/)               { $par{"dbBlast"}=     $par{"dbHuman"}; }
	elsif ($arg=~/^mouse$/)               { $par{"dbBlast"}=     $par{"dbMouse"}; }
				# other
	elsif ($arg=~/^ec|enzyme$/)           { $par{"dbBlast"}=     $par{"dbEnzyme"}; }
	elsif ($arg=~/^htm83\D*$/)            { $par{"dbBlast"}=     $par{"dbHtm83_1995"}; }
	elsif ($arg=~/^pdbhtm$/)              { $par{"dbBlast"}=     $par{"dbPdbHtm"}; }
    ...
    if    ($par{"dbBlast"} =~ /^phd126/){
        ...
        $db=$par{"dbBlast"}=$par{"dbProdom"}; $db=~s/^(.*)\/.*$/$1/;   $par{"BLASTDB"}=$db; }
				# --------------------
				# PDB
    elsif ($par{"dbBlast"} =~ /^pdb5/){
        $db=$par{"dbBlast"}=$par{"dbPdb5881"};$db=~s/^(.*)\/.*$/$1/;   $par{"BLASTDB"}=$db; }
    elsif ($par{"dbBlast"} =~ /^pdb1267/){
        $db=$par{"dbBlast"}=$par{"dbPdb1267"};$db=~s/^(.*)\/.*$/$1/;   $par{"BLASTDB"}=$db; }
    elsif ($par{"dbBlast"} =~ /^pdb$/){
        $db=$par{"dbBlast"}=$par{"dbPdb"};    $db=~s/^(.*)\/.*$/$1/;   $par{"BLASTDB"}=$db; }

				# --------------------
				# locations
    elsif ($par{"dbBlast"} =~ /^proka/){
        $db=$par{"dbBlast"}=$par{"dbLocipro"};$db=~s/^(.*)\/.*$/$1/;   $par{"BLASTDB"}=$db; }
    elsif ($par{"dbBlast"} =~ /^euka/){
        $db=$par{"dbBlast"}=$par{"dbLocieuk"};$db=~s/^(.*)\/.*$/$1/;   $par{"BLASTDB"}=$db; }
    ...
    elsif ($par{"blastVersion"} =~/psi/){
	$par{"extBlast"}=$par{"extBlastpsi"}; }
    else {
    ...
    ($Lok,$msg)=
        &brIniErr($exclude);    return(&errSbrMsg("after lib:brIniErr\n",$msg)) if (! $Lok);  
				...
				# terminate if no exe
    $Lok=0;
    foreach $kwd ("exeBlastp","exeBlast3","exeBlastpsi","exeBlastpsiperl"){
...
#    $par{"fileDefaults"}=       ""; # file with defaults
    ...
    $par{"dirPubBlast"}=        $par{"dirPub"}.  "blast/";
    $par{"dirPubBlastBin"}=     $par{"dirPubBlast"}."bin/";

    $par{"dirOut"}=             ""; # directory for output files
    $par{"dirWork"}=            ""; # working directory

    $par{"dirData"}=            "/data/";
    $par{"dirBlast"}=           $par{"dirData"}."blast/"; # binary BLAST version of databases
    $par{"dirPdb"}=             $par{"dirBlast"}."pdb";
#    $par{""}=                   "";
    $par{"dbPdb"}=              $par{"dirPdb"};
				# full PDB list used to build new unique
    $par{"dbPdb5881"}=          "/home/rost/pub/molbio/db/pdb5881";
				# all pdb for unique: distance = 5 -> NOT entirely unique
    $par{"dbPdb1267"}=          "/home/rost/pub/molbio/db/pdb1267";
    $par{"dbPdbuni"}=           $par{"dirBlast"}."pdbuni";

    $par{"dbSwiss"}=            "/home/rost/data/swissprot/swiss";
    $par{"dbSwiss"}=            $par{"dirBlast"}."swiss";

    $par{"dbTrembl"}=           $par{"dirBlast"}."trembl";
    $par{"dbBig"}=              $par{"dirBlast"}."big";
				# GENOMES
    $par{"dbCaeel"}=            $par{"dirBlast"}."caeel";
    $par{"dbDrome"}=            $par{"dirBlast"}."drome";
    $par{"dbYeast"}=            $par{"dirBlast"}."yeast";
    $par{"dbMetja"}=            $par{"dirBlast"}."metja";
    $par{"dbHuman"}=            $par{"dirBlast"}."human";
    $par{"dbMouse"}=            $par{"dirBlast"}."mouse";
				# other
    $par{"dbEnzyme"}=           $par{"dirBlast"}."enzyme";
    $par{"dbPdbHtm"}=           "/home/rost/data2/htm-pdb-blast/htm-pdb-all";

    $par{"dbHtm83_1995"}=       "/data/blast/htm83_1995";
    

    $par{"dbLocipro"}=          "/home/rost/pub/molbio/db/loci-proka";
    $par{"dbLocieuk"}=          "/home/rost/pub/molbio/db/loci-euka";
...
#    $fhin=                      "FHIN";
    ...
    $par{"blastVersion"}=       0;  # if 1: format the database for BLAST
    $par{"blastCmd"}=           ""; # explicit command for psi-blast
...
#    $par{"exe"}=                "";
    ...
    $par{"exeBlastpsi"}=        $par{"dirPubBlastBin"}."blastpgp.ARCH";
    $par{"exeBlastpsiperl"}=    $par{"dirPubBlast"}."blastpgp.pl";
                                # syntax 'formatdb -t TITLE -i DIR/fasta-file -l logfile'
                                # note:  files will be created in DIR !
    ...
    $par{"exeConvertSeq"}=      "/home/rost/pub/phd/bin/convert_seq.ARCH";
#    $par{""}=                   "";
