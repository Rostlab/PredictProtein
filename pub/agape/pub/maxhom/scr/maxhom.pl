...
#-------------------------------------------------------------------------------
				...
				# --------------------
				# directories
    if    (defined $USERID && $USERID=~/rost/){
	$par{"dirHome"}=        "/home/rost/pub/";
    }
    elsif (defined $USERID && $USERID=~/predictprotein|pp|phd/){
	$par{"dirHome"}=            "/home/".$USERID."/server/pub/";
    }
    elsif (defined $USERID && $USERID=~/cubic/){
	$par{"dirHome"}=            "/home/rost/pub/";
    }
    ...
    $par{"dirMax"}=             $par{"dirHome"}. "maxhom/";     # MAXHOM central
    ...
    $par{"dirPubMolbio"}=       "/home/rost/pub/blast/";      # perl scripts needed 
#    $par{"dirPubMolbio"}=       "/usr/local/molbio/perl/";      # perl scripts needed 

    $par{"dirBinMolbio"}=       $par{"dirHome"}. "molbio/bin/"; # FORTRAN binaries of programs needed

    if ($0=~/maxhom/){
	$par{"dirScrMax"}=      $par{"dirMax"}.  "scr/";        # scripts needed for maxhom

	$par{"dirPerlScr"}=     $par{"dirMax"}.  "scr/";      # perl scripts needed
	$par{"dirPubMolbio"}=   $par{"dirMax"}.  "scr/";      # perl scripts needed 
	$par{"dirBinMolbio"}=   $par{"dirMax"}.  "bin/";      # FORTRAN binaries for BLAST
    }
...
#    $par{"dirData"}=            "/home/rost/data/";
    $par{"dirData"}=            "/data/";
#    $par{"dirDataBlast"}=       "/home/rost/pub/molbio/db/";
...
#    $par{"dbPdb"}=              $par{"dirDataBlast"}. "pdb";
#    $par{"dbSwiss"}=            $par{"dirData"}.      "swiss/swiss";
...
#    $fhin=                      "FHIN";
    ...
    $par{"BLASTDB"}=            "/home/rost/data/swissprot/swiss"; # 

    if ($0=~/maxhom/){
	$par{"BLASTMAT"}=       $par{"dirMaxMat"}. "blastapp/matrix";  # env for BLASTP
	$par{"BLASTDB"}=            "/data/swissprot/swiss"; # 
	$par{"BLASTDB"}=        $par{"dirMaxMat"}. "blastdb"; # 
    }

    $par{"doBlastp"}=           1; # if 1: runs BLASTP as pre-filter
    $par{"doBlast3"}=           0; # if 1: runs PSI-BLAST as pre-filter

#    $par{"doBlastp"}=           0; # if 1: runs BLASTP as pre-filter
#    $par{"doBlast3"}=           1; # if 1: runs PSI-BLAST as pre-filter
    ...
    $par{"doTest"}=             0; # faster search for testing
...
#    $tmp{"nopre"}=           "                   -> do NOT pre-filte, i.e. NO  BLAST, at all!";
    ...
    $tmp{"tst"}=             "                   -> more stringent faster search for testing!";
...
#						$par{"doBlast3"}=     0; }
	elsif ($arg=~/^(test|tst)$/i)         { $par{"doTest"}=       1; }
...
#-------------------------------------------------------------------------------
	...
	$par{"parBlastDb"}      if ($par{"doDirect"});
				...
				# for testing:
    if ($par{"doTest"}){
	$par{"parBlastE"}= $par{"parBlastEtst"};
	$par{"parBlastV"}= $par{"parBlastVtst"};
	$par{"parBlastB"}= $par{"parBlastBtst"};
	$par{"parBlastDb"}=$par{"parBlastDbtst"};
    }
...
#-------------------------------------------------------------------------------
	...
	if (-e $fileHssp && ! &is_hssp_empty($fileHssp) &&
	    ...
	    $cmdLoc.= " fileMatGcg=".$par{"fileMaxhomMetr"};
