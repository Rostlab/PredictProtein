...
#-------------------------------------------------------------------------------
				...
				# directories
    ($tmpLoc)=($0=~/^(.*\/)/);
    $tmpLoc="./" if(! defined $tmpLoc);
    $par{"dirHome"}=            $tmpLoc;

				# <<<<<<<<<<<<<<<<<<<<
				# normal
    $par{"dirSrc"}=             $par{"dirHome"}.   "resources/";   # all source except for binaries
    $par{"dirSrcMat"}=          $par{"dirHome"}.    "resources/";   # general material
				                             # perl libraries
    $par{"dirPerl"}=            $par{"dirHome"}.    "resources/" if (! defined $par{"dirPerl"});
    $par{"dirPerlScr"}=         $par{"dirHome"}.    "resources/";  # perl scripts needed
    $par{"dirBin"}=             $par{"dirHome"}.    "resources/";   # FORTRAN binaries of programs needed
...
#    $par{"dirBin"}=             "/home/rost/pub/phd/". "bin/";   # FORTRAN binaries of programs needed
    ...
    $par{"dirPerlPack"}=        $par{"dirPerlScr"};  # perl scripts needed
...
#-------------------------------------------------------------------------------
    ...
    $tmp{"s_k_i_p"}=         "problems,manual,hints";
				...
				# special help
    $tmp{"scrAddHelp"}=      "";
    $tmp{"special"}=         "expand,compress,nocompress,list,";
    $tmp{"special"}=         "skip,";
