...
if(defined $fileHtml){
  ...
  print $fhout1 "<H1><STRONG>Results of Nuclear Localization Signal Prediction(NLS):</STRONG></H1>";
...
#---------------------------------------------------------------------------
  ...
  system("$ENV{HOME}/server/pub/predictNLS/grep_NLS_update.pl  mode=$mode '$sequence'  out  '$dirOut'  '$jobId'  >'$runOut'"); # system call to main program   
...
if($out=~ /\w/){		# if true, this seq has NLS. 
  ...
  foreach $i (0..$#nlsList) {
   ...
   if(-e "$NlsDat"){
     if(defined $fileHtml){
       print $fhout1 "<H1><B>Statistical data for Nuclear Localization Signals present in the Input Sequence</B></H1>\n";
	...
	if(defined $fileHtml){
	print $fhout1 "<TD COLSPAN=3 ROWSPAN=1><A HREF='http://cubic.bioc.columbia.edu/srs6bin/cgi-bin/wgetz?-id+on291EKHQt+-e+[SWISSPROT-ID:$nlsList[0]]' >$nlsList[0]</TD>\n";
	print $fhout1 "<TD COLSPAN=1 ROWSPAN=1>$locList[0]</TD></TR>\n";
      }
	foreach $j (1..$#nlsList){
	  if(defined $fileHtml){
	  print $fhout1 "<TR><TD COLSPAN=3 ROWSPAN=1><A HREF='http://cubic.bioc.columbia.edu/srs6bin/cgi-bin/wgetz?-id+on291EKHQt+-e+[SWISSPROT-ID:$nlsList[$j]]' >$nlsList[$j]</A></TD>\n";
  ...
  print $fhout1 "</TABLE>";
    ...
    # DNA binding protein.
	       if(defined $fileHtml){
    print $fhout1 "<H1><B><I> This protein is predicted to bind to DNA.</I></B></H1>\n";
    ...
    print $fhout1 "<H1> DNA binding statistics for the binding NLS signals.</H1>\n";
  }
	       if(defined $fileEmail){
...
#---------------------------------------------------------------------
  ...
  if(!(defined $dirOut)){
    print STDOUT "Output directory not specified\n";
    exit;
  }
  if(!(defined $fileTrace)){
    print STDOUT "Trace file not defined\n";
    exit;
  }
  $fileErr= $fileTrace;
  ...
  $dirData= "/home/$ENV{USER}/server/pub/predictNLS/data/";
  ...
  if($html==0){
    ...
    $fileEmail =$dirOut.$fileOut;
    $fileHtml =$dirOut.$fileOut1;
  }
