...
sub fastaWrt {
    ...
    $[ =1 ;
#-------------------------------------------------------------------------------
#   fastaWrt                    writes a list of sequences in FASTA format
#       in:                     $fileOut,$tmp{} with:
...
#-------------------------------------------------------------------------------
	...
	$lenHere=length($tmp{"seq","$itPair"});
		...
		$tmpWrt2.= sprintf(" %-10s",substr($tmp{"seq","$itPair"},$itHere,$nchunk)); 
	    }
	    $tmpWrt.=$tmpWrt2."\n";
	}
    }
...
#                               $input{"id",$it}  sequence identifier ($name)
#                               $input{"seq",$it} sequence for $name
#--------------------------------------------------------------------------------
    $sbrName="msfWrt";
				# ------------------------------
	...
	push(@stringLoc,$input{"seq",$it}); } # store sequences
...
#--------------------------------------------------------------------------------
    ...
    $fhoutLoc="txt"             if ($fileOutLoc =~/txt/i);
				# ------------------------------
				# write header
				# ------------------------------
