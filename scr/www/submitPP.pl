...
#       in GLOBAL:              $htmlData{}, in particular 'HTTP_REFERER'
#       in GLOBAL:              $envPP{"fileHtmlMetaSubmitPP"}
#       out:                    ($Lok,$string_writeout)
#       err:                    (1,'ok'), (0,'message')
#-------------------------------------------------------------------------------
    $fhinLoc="FHIN_meta";
...
#-------------------------------------------------------------------------------
    $fhinLoc="FHIN_meta";
				...
				# get site sending this
    $meta_page_origin= 0;
    $meta_page_origin= $envPP{"fileHtmlMetaSubmitPP"} if (defined $envPP{"fileHtmlMetaSubmitPP"});
				# ------------------------------
	...
	$meta_html.="<BR>ERROR file=$meta_page_origin, not found<BR>";}
...
#                               in info
#                               
#                        NOTE   this gets submit_meta_phm.html
#                               watch changes for META-PP
#                               
#       in GLOBAL:              $envPP{"fileHtmlMetaSubmitPP"}
#       out:                    ($Lok,$string_writeout)
#       err:                    (1,'ok'), (0,'message')
#-------------------------------------------------------------------------------
	...
	if ($Lok) {
		...
		if  ($line=~/name=.?email/i && $emailLoc){
			...
			$line=~s/PH_EMAIL_PH/$emailLoc/;}}
				# sequence name
		if ($line=~/PH_SEQNAME_PH/i && $nameLoc){
		    $nameLoc=~s/\n//g;
		    $nameLoc=~s/^\s*|\s*$//g;
		    $line=~s/PH_SEQNAME_PH/$nameLoc/i;
#		    $add=" VALUE=\"$nameLoc\" ";
#		    $line=~s/(name=.?seqname.? \s*)value=\"[^\"]*\" /$1$add/i;
		}
				# sequence
		if ($line=~/PH_SEQUENCE_PH/i && $seqLoc){
#		    $seqLoc=~s/\n//g;
		    $line=~s/PH_SEQUENCE_PH/$seqLoc/i;
		}
		$meta_html.=$line;}
	    close($fhinLoc);}
    }
    $tmpLoc= "";
#    print "\nxx end meta-html\n<br>\n";
#    $tmpLoc.="<P> DBG sorry for inconvenienc</P>"."<P> open=$Lopenxx, meta_page_origin=$meta_page_origin DBG ok=$Lok_meta, tmp_meta=$tmp_meta,xxx</P>";
