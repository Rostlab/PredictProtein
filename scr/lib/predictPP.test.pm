...
#		  "envFastaLibs",
    ...
    foreach $kwd ("dir_default","dir_prd",
		  ...
		  "dirChop","dirChopper","dir_pplocal_home", "dirProfBval", "dirPrenup",
		  "dirIsis","dirNLProt", "dirProfTmb",  "dirLocTar",  "dirPfam",  "dirAgape","dirR4S",){
	next if (length($kwd)<1);
...
#		  "exeRepeats",
		  ...
		  "exeProfConv",  "exeProfHtmfil","exeProfHtmref",     "exeProfHtmtop", "exePrenup",
#		  "exeGlobe",
#		  "exeTopitsWrtOwn",
		  "exeTopits",    "exeTopitsMaxhomCsh","exeTopitsMakeMetr","exeTopitsMaxhom",
		  "exeThreader","exePfam","exeProfBval",
		  "exeMview", "exeProfCon", "exePcc", "exeChop", "exeChopper",
                  "exeIsis", "exeProfTmb", "exeLocTar", "exeAgape", "exeNLProt", "exeR4S",
#"exe_svcSubmitter",
		  ){
	next if (length($kwd)<1);
#	$envPP{$kwd}=         &envPP'getLocal("$kwd");    #e.e'
...
#	$envPP{$kwd}=         &envPP'getLocal("$kwd");    #e.e'
    ...
    foreach $kwd (
		  ...
		  "globe","topits","disulfind","asp","norsp", "profcon", "pcc", "profbval", "prenup",
		  "chop","chopper","isis", "proftmb", "loctar",  "agape", "NLProt",  "conseq",  "consurf"
		  ){
	$kwd2="version_".$kwd;
    ...
    foreach $kwd ("fileHelpText","fileHelpConcise","fileNewsText","fileLicText",
		 ...
		 "fileAppHtmlHead", "fileAppHtmlHeadIsis", "fileAppHtmlHeadAgape", "fileAppHtmlHeadNLProt", "fileAppHtmlHeadNLProt",
		  "fileAppHtmlFoot", "fileAppProfCon", "fileAppPcc", "fileAppHtmlHeadProfBval",    
		  "fileAppHtmlStyles",    "fileAppHtmlQuote", "fileAppChop", "fileAppChopper",
		  "fileAppIsis", "fileAppProfTmb", "fileAppNLProt", "fileAppProfBval",
		  "fileAppLocTar",  "fileAppPfam",   "fileAppAgape",  "fileAppR4S", "fileAppPrenup",
...
#	 'cyspred',	     "CYSPRED prediction(P Fariselli,P Riccobelli & R Casadio)",
	 ...
	 'profbval',	     "Neural network-based prediction method of flexibility/rigidity from sequence.(Schlessinger A, Rost B)",
...
#	push(@kwdRm,"seq");
    ...
    $job{"run"}.=",profcon"       if ($job{"out"}=~ /profcon/);
				...
				# PRENUP  
    $job{"run"}.=",profcon,prenup"       if ($job{"out"}=~ /prenup/);
    ...
    $job{"run"}.=",proftmb"       if ($job{"out"}=~ /proftmb/);
				...
				# PROFBVAL  
    $job{"run"}.=",profbval"       if ($job{"out"}=~ /profbval/);
...
#  GY - added tweak for proftmb profile
	...
	if ($job{"run"} =~ /proftmb/ || ($job{"run"} =~ /disulfind/ )){
...
# End additions
	...
	$msgHere .= "," . $file{"blastPsiMatTmb"} if ($job{"run"} =~ /proftmb/) ;
	$msgHere .= ")\n";
...
#   elsif ($optRun=~/profcon/i)                          { $optProf="both";} # GY: experimental for profcon
...
#-------------------------------------------------------------------------------
    ...
    while( <$fhInLoc> ) {
	$_=~tr/A-Z/a-z/ if ($_ !~/chain/);	# upper case -> lower *!*
...
#				    $lineLoc=~/prof\S?.*[ -]htm|prof\S?.*membrane|profhtm/);
	...
	return (0);
                               ...
                               # normal proftmb
    $optRun.=",proftmb"          if ($lineLoc =~ /prof tmb/);


                              # normal profbval
    $optRun.=",profbval"          if ($lineLoc =~ /prof bval/);
...
# $file{\"hsspPsiFil\"} $file{\"profRdb\"}"." $UserProtName ". " 
	...
	if ( $Lok == 1 ) {
	    $#tmpAppend=0;	  
	    push(@tmpAppend,$envPP{"fileAppPrenup"},
		 $file{"prenup"},$envPP{"fileAppLine"});
	...
	if ($origin =~ /^mail|^html|^testPP/i){
	    ...
	    $#tmpAppend=0;	
	    push(@tmpAppend,$envPP{"fileAppPrenup"},$file{"prenup"},$envPP{"fileAppLine"});
	    if ($#tmpAppend>0){
...
#    $msgErr=    $msgErr{":"};
	...
	$dirProfTmb = "/home/ppuser/server/pub/proftmb/proftmb-1.1/input/";
...
#	$file{"seq4ProfBval"}=    $dirWork. $fileJobId.  ".fastaSingle";
    ...
    if ($optRun =~/profbval/) {
	    ...
	    $dirProfBval = "/home/ppuser/server/pub/profbval/";
...
#	close $fhcys;
	...
	if ( $Lok == 1 ) {
	    ...
	    if ($optRun =~/loctar_only/) { # HACK: remove previouse output agape should only 
		                          # have its own output
...
#	print "arroutput length=".$#arrOutputs."\n";
	...
	$command="$nice $exeAgape $fileJobId $file{\"seq4agape\"} $dirWork";
	$msgHere="--- $sbr system '$exeAgape $command'";
