...
#    print  "--- $sbrName2: FOUND A REQUEST ($fileFound)\n";
	...
	($Lok,$msg,$fileNew,$fileResult,$user,$orig,$password)=
...
#	$cmd= "$exeNiceLoc $exePredictLoc $fileNew $fileResult $user $password $orig -s  >> $fileScanLogLoc";
	$cmd= "$exeNiceLoc $exePredictLoc $fileNew $fileResult $user $password $orig -s ";
				# write trace
...
#	$sgeExe = "$qsubBin -o \\\$HOME/server/tmp -e \\\$HOME/server/tmp -S /bin/bash -p -500 ";
	...
	$sgeExe .= $prmPriority;
#	$sgeExe = "$qsubBin -o /tmp -e /tmp -S /bin/bash -p -500 "; 
	# get PID
	$filePID=      $fileNew;	
...
#-------------------------------------------------------------------------------
    ...
    $dateTmp=&sysDate();
    $msg=  "--- predict: $dateTmp from=$user orig=$orig resp=$resp file=$fileFound";
    $cmd="echo '$msg'";
    ...
    return(1,"ok",$fileNew,$fileResult,$user,$orig,$password);
