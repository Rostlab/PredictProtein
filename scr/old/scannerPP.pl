...
while (-e $envPP{"file_scanFlag"}) {
	...
	&scannerPredict($predictScrName,$envPP{"exe_nice"},$envPP{"exe_ppPredict"},
			...
			$envPP{"suffix_res"},$envPP{"suffix_mail"},
    ...
    if    (! $Lok)  { $msgErr= "";
		      ...
		      print $msgErr; }
				...
				# --------------------------------------------------
    ($Lok,$msg)=
	&scannerSendMail($envPP{"dir_mail"},$envPP{"dir_res"},
			 $envPP{"exe_mail"},$envPP{"exe_mailHtml"},$envPP{"file_sendLog"},
			 $envPP{"suffix_res"},$envPP{"suffix_mail"},$Ldebug);

    if (! $Lok) { $msgErr= "";
		  $msgErr.="*** err=9800\n" if ($msg !~ /err\=/);
		  $msgErr.="*** ERROR $scrName: after sending mail of res (scannerSendMail)\n";
		  $msgErr.=$msg."\n";
		  print $msgErr;
		  &ctrlSendAlarm($msgErr,"PP_ERROR_scanner_mail"); }
				...
				# --------------------------------------------------
    ($Lok,$msg)=
	&scannerManageExpired($envPP{"dir_mail"},$envPP{"dir_res"},$envPP{"exe_mail"},
			      $envPP{"file_sendLog"},
			      $envPP{"suffix_res"},$envPP{"suffix_mail"},$Ldebug);

    if (! $Lok) { $msgErr= "";
		  $msgErr.="*** err=9900\n" if ($msg !~ /err\=/);
		  $msgErr.="*** ERROR $scrName: after clean results (scannerManageExpired)\n";
		  $msgErr.=$msg."\n";
		  print $msgErr;
		  &ctrlSendAlarm($msgErr,"PP_ERROR_scanner_manageExpired_loop"); }
				# --------------------------------------------------
...
#-------------------------------------------------------------------------------
	...
	($Lok,$msgSys)=&sysSystem("$cmdSystem"); }
				...
				# --------------------------------------------------
    ($Lok,$msg)=
	&ctrlReportStatus($envPP{"exe_nice"},$envPP{"exe_status"},$envPP{"file_statusFlag"},
			  $envPP{"para_status"},$envPP{"file_statusAdd"},
			  $envPP{"file_statusLog"},$envPP{"file_statusHis"},
			  $envPP{"dir_prd"},$envPP{"file_ppLog"},$Ldebug);
				# tolerate ERROR, but note it!
    if (! $Lok){ $msgErr="*** err=9290\n".
		     "-*- WARN $scrName: ctrlReportStatus failed\n".$msg;
		 print $msgErr,"\n";
		 &ctrlSendAlarm($msgErr,"PP_WARN_scanner"); }
    return(1,"ok $sbrName"); 
...
#    $dirLogTmpxx=   "/home/$ENV{USER}/server/bup/tmp-mail-in/";
#    $fileLogInputxx="/home/$ENV{USER}/server/bup/Log-of-mail-in.tmp";
...
sub scannerPredict {
    local($predictScrNameLoc,$exeNiceLoc,$exePredictLoc,$exeDuLoc,$exeQuotaLoc,$exeTarLoc,
	  ...
	  $ctrlCheckDuLoc,$ctrlCheckQuotaLoc,
...
#-------------------------------------------------------------------------------
	...
	if (! defined $fileFound || ! $fileFound);
				# <--- <--- <--- <--- <--- <--- 
				# ==============================
...
#-------------------------------------------------------------------------------
    ...
    ($Lok,$msg)=
	&fileRemoveNullChar($fileNew);
    if (! $Lok){ $msgErr= "";
		 $msgErr.="*** err=9630\n" if ($msg !~ /err\=/);
		 $msgErr.="*** ERROR $scrName:fileRemoveNullChar failed\n".$msg;
		 print $msgErr,"\n";
#		 &ctrlAbort($msgErr,"PP_ERROR_scannerPredict_null"); 
	     }
    ...
    $fileResult=~s/\/\//\//g;	# security '//' -> '/'
				...
				# --------------------------------------------------
    if ($resp=~/MAIL/) {
				# create a mail_query file
	$dirMailLoc=~s/\/$//g   if ($dirMailLoc=~/\/$/);  # purge final slash
	$fileMailQuery=$dirMailLoc."/".$fileFoundTmp . $suffixMailLoc;
	$fileMailQuery=~s/(\/)\/*/$1/g; # security '//' -> '/'
				# error??
	open (QUERY,">".$fileMailQuery) || 
	    warn("-*- WARN $sbrName3: failed opening new fileMailQuery=$fileMailQuery!\n");
	print QUERY "PPhdr from: $user\n";
	print QUERY "PPhdr orig: $orig\n";
	print QUERY "PPhdr resp: $resp\n";
	print QUERY "PPhdr want: $want\n";
	close(QUERY); }
...
#===============================================================================
sub scannerSendMail {
    local($dirMailLoc,    $dirResLoc,     $exeMailLoc,    $exeMailHtmlLoc,
	  $fileSendLogLoc,$suffixResLoc,  $suffixMailLoc, $LdbgLoc)=@_;
    local($sbrName,$tmp,$Lok,$errMsg,@MAIL,$fileQueryLoc,$fileResLoc,$fileNameLoc,$dateTmp);
    $[ =1;
#-------------------------------------------------------------------------------
#   scannerSendMail             scans directory with results and send mail
#                               
#       in:                     
#         $dir_mail             dir with traces of query 'from=user' (server/mail)
#         $dir_res              dir with results                     (server/res)
#         $exe_mail             mail executable                      (/usr/bin/Mail)
#         $file_sendLog         scanner log file                     ()
#         $suffixResLoc         files in server/res  called $prefixWorkLoc.'_'.ID.$suffix
#         $suffixMailLoc        files in server/mail called $prefixWorkLoc.'_'.ID.$suffix
#         $LdbgLoc              debug flag (if =1 write onto screen!)
...
#-------------------------------------------------------------------------------
    ...
    return(0,$errMsg."not def dirMailLoc!")       if (! defined $dirMailLoc);
    return(0,$errMsg."not def dirResLoc!")        if (! defined $dirResLoc);
    return(0,$errMsg."not def exeMailLoc!")       if (! defined $exeMailLoc);
    return(0,$errMsg."not def exeMailHtmlLoc!")   if (! defined $exeMailHtmlLoc);
    return(0,$errMsg."not def fileSendLogLoc!")   if (! defined $fileSendLogLoc);
    return(0,$errMsg."not def suffixResLoc!")     if (! defined $suffixResLoc);
    return(0,$errMsg."not def suffixMailLoc!")    if (! defined $suffixMailLoc);
    $LdbgLoc=0                                    if (! defined $LdbgLoc);

    return(0,$errMsg."no exeMail=$exeMailLoc!")   if (! -e $exeMailLoc &&
						      ! -l $exeMailLoc &&
						      ! -x $exeMailLoc);

				# ------------------------------
				# read directory with results
				# ------------------------------
    $#MAIL=0;
    opendir(DIR,$dirMailLoc) || 
	warn("-*- WARN $sbrName: failed opening dirMail=$dirMailLoc!\n");
    @MAIL=grep(/$suffixMailLoc$/, readdir(DIR));
    closedir(DIR);
				# ------------------------------
				# <--- <--- <--- <--- <--- <--- 
				# none found -> return
    return(1,"none found in $dirMailLoc") 
	if ($#MAIL < 1);
				# <--- <--- <--- <--- <--- <--- 
				# ==============================
				...
				# --------------------------------------------------
    $dirMailLoc.="/"            if ($dirMailLoc !~/\/$/); # add final slash
    $dirResLoc.= "/"            if ($dirResLoc !~/\/$/);  # add final slash

    foreach $fileNameLoc (@MAIL) {
				# ------------------------------
				# which files are expected?
				# ------------------------------

				# file flagging 'wait for results'
	$fileQueryLoc= $dirMailLoc.$fileNameLoc;
				# build the expected result file name
	$fileResLoc=   $fileNameLoc;
	$fileResLoc=~s/$suffixMailLoc/$suffixResLoc/;
	$fileResLoc=   $dirResLoc.$fileResLoc;

				# ------------------------------
				# oops result file is missing...
	next if (! -e $fileResLoc);

				# ------------------------------
				# send the result if ready
				# ------------------------------

				# extract address from result file!
	($Lok,$user)=&fileExtractHeader($fileQueryLoc,"PPhdr user");
	if (! $Lok || $user !~/\@/) {
	    ($Lok,$user)=&fileExtractHeader($fileResLoc,"PPhdr from"); }
	if (! $Lok || $user!~/\@/ || $user=~/\s/) { 
	    $msgErr="-*- WARN $sbrName err=9810 no user name for file=$fileResLoc\n";
	    print $msgErr; 
	    &ass_unlinkSecurity($fileResLoc,$fileQueryLoc);
	    &ctrlSendAlarm($msgErr,"PP_WARN_scannerSendMail"); 
	    next; }

				# extract format of result file requested
				# changed br 99-06:
				# put (1) first, since may request HTML output
				# but stored -> no HTML appended, simply message

	$want="ASCII";		# default value

				# (1): result file (first since 
	($Lok,$want)=&fileExtractHeader($fileResLoc,"PPhdr want");
				# (2): if that does not have it: original request
				#      trace
	($Lok,$want)=&fileExtractHeader($fileQueryLoc,"PPhdr want")
	    if (! $Lok);
				# (3): default is ASCII (if nothing found)


				# ------------------------------
				# send mail: HTML attached
	if ($want=~/HTML/){
	    $cmd="$exeMailHtmlLoc $fileResLoc $user subj='Result from PredictProtein'";
	    ($Lok,$msg)=&sysSystem($cmd); }

	else {			# send mail: ASCII
	    ($Lok,$msg)=
		&sysSendMail($exeMailLoc,$fileResLoc,$user,"PredictProtein"); }

	print "--- $sbrName: send mail returned=$msg!\n" if ($LdbgLoc && $Lok);
	print "*** $sbrName: failed to send ($msg)\n"    if (! $Lok);

				# log the mail sent
	$fileNameLoc= $fileResLoc; $fileNameLoc=~s/^.*\///;
	$fileSendLogLoc="ERROR_NO_FILE_SEND_LOG_IN_scannerPP.tmp"
	    if (! -e $fileSendLogLoc);
	$msg=sprintf("--- scannerSendMail date=%-15s to %-20s file %-s",
		     &sysDate(),$user,$fileNameLoc);
	($Lok,$msgSys)=&sysSystem("echo $msg >> $fileSendLogLoc",0);

				# remove the files
				# HARD CODED: security 20-02-97
	&ass_unlinkSecurity($fileResLoc,$fileQueryLoc);
    }				# end of loop over all files to be mailed
				# --------------------------------------------------

    $#MAIL=0;			# slim-is-in!
    return(1,"ok");
}				# end of scannerSendMail

#===============================================================================
sub scannerManageExpired {
    local($dirMailLoc,$dirResLoc,$exeMailLoc,$fileSendLogLoc,
	  $suffixResLoc,$suffixMailLoc,$LdbgLoc)=@_;
    local($sbrName,$fhinLoc,$tmp,$Lok);
    $[ =1 ;
#-------------------------------------------------------------------------------
#   scannerManageExpired        checks dir server/res
#                               - finds all files with no corresponding _query
#                                 file in server/mail
#                               - tries to send
#                               - deletes
#         $dir_mail             dir with traces of query 'from=user' (server/mail)
...
#-------------------------------------------------------------------------------
    $tmp=$0;$tmp=~s/^.*\/|\.pl//g;
    $sbrName="$tmp:"."scannerManageExpired"; $fhinLoc="FHIN"."$sbrName";
    $errMsg="*** err=9901\n"."*** $sbrName: ";
				# ------------------------------
				# check arguments
				# ------------------------------
    ...
    $LdbgLoc=0                                    if (! defined $LdbgLoc);
				...
				# do NOT send mail if problem here...
    $exeMailLoc=0  if (! -e $exeMailLoc && ! -l $exeMailLoc && ! -x $exeMailLoc);

				# add slash
    $dirMailLoc.="/"            if ($dirMailLoc !~ /\/$/);

				# ------------------------------
				# read directory with results
				# ------------------------------
    return(0,"*** err=9902\n".
	   "*** dir res=$dirResLoc missing'n") if (! -d $dirResLoc);
    $#resLoc=0;
    opendir(DIR,$dirResLoc)  || 
	warn("-*- WARN $sbrName: failed opening dirResLoc=$dirResLoc!\n");
    @resLoc=grep(/$suffixResLoc$/, readdir(DIR));
    closedir(DIR);
				# ------------------------------
				# <--- <--- <--- <--- <--- <--- 
				# none found -> return
    return(1,"none found in $dirResLoc")  if ($#resLoc < 1);
				# <--- <--- <--- <--- <--- <--- 
				# ==============================
				...
				# go through all results
				# --------------------------------------------------
    foreach $fileResLoc (@resLoc) {
	$name=$fileResLoc; $name=~s/^.*\/|_?$suffixResLoc$//g;
	$fileResLoc=$dirResLoc.$fileResLoc;
	next if (! -e $fileResLoc);

				# expected query file
	$fileMailLoc=$dirMailLoc.$name.$suffixMailLoc;
				# ------------------------------
				# is ok, will be handled in next
				#    round of scanner!
				# ------------------------------
	next if (-e $fileMailLoc);

				# ------------------------------
				# problem with mailer...
				# ------------------------------
	if (! $exeMailLoc || 
	    (! -e $exeMailLoc && ! -l $exeMailLoc)) {
	    $msgErr="*** ERROR $sbrName (err=9910): mail command ($exeMailLoc) wrong\n";
	    print   $msgErr;
	    next; }
				# ------------------------------
				# extract address and try to send
				# ------------------------------
				# read results
	$Lok=open($fhinLoc,$fileResLoc);
				# missing: skip!!
	if (! $Lok){ $msgErr="*** ERROR $sbrName (err=9911): opening fileRes=$fileResLoc\n";
		     print $msgErr;
		     next; }

	($Lok,$user)=&fileExtractHeader($fileResLoc,"PPhdr user");
				# address ok?
	$user=0                 if (! $Lok || ! $user ||
				    $user !~ /\.[A-Za-z][A-Za-z][A-Za-z]?$/);
				# seems ok, so send!
	if ($user) {
	    ($Lok,$msg)=
		&sysSendMail($exeMailLoc,$fileResLoc,$user,"PredictProtein");
		...
		$msg=sprintf("--- scannerSendMail date=%-15s to %-20s file %-s",
			     &sysDate(),$user,$fileNameLoc);
		($Lok,$msgSys)=&sysSystem("echo $msg >> $fileSendLogLoc"); } } 

				# ------------------------------
				# remove the files
	&ass_unlinkSecurity($fileResLoc);
    }				# end of loop over all results files
    return(1,"ok");
}				# scannerManageExpired
...
#    local($fileInLoc,$fhErrSbr) = @_ ;
    ...
    $[ =1 ;
#-------------------------------------------------------------------------------
#   scannerLicense              scans the license directory
#                               if new found: 
#                                  (1) send mail to user and PPadmin
...
#-------------------------------------------------------------------------------
    $tmp=$0;$tmp=~s/^.*\/|\.pl//g;$tmp.=":";
    $sbrName=$tmp."scannerLicense";
    $fhinLoc="FHIN_"."scannerLicense";$fhoutLoc="FHOUT_"."scannerLicense";

    return(0,"*** ERROR $sbrName: no dir_lic=".$envPP{"dir_lic"})
	if (! -d $envPP{"dir_lic"} && ! -l $envPP{"dir_lic"});
				# ------------------------------
				# watch it: do not write while
	...
	++$ct;
	last if ($ct > 10);}
    &ass_unlinkSecurity($file_licFlag);
    system("echo 'is writing' > $file_licFlag");
	
				# ------------------------------
				# append to file_lic
    $file_licence=$envPP{"file_licence"};
    $file_licNew= $envPP{"file_licNew"};
    if (-e $file_licNew) {
	($Lok,$tmp)=&sysSystem("cat < $file_licNew >> $file_licence");
	&ass_unlinkSecurity($file_licNew); }
    &ass_unlinkSecurity($file_licFlag);

    return(1,"ok $sbrName");

}				# end of scannerLicense
...
#		    open (SYST, "rsh $candidate '$ps_cmd | grep $predictScrNameLoc' | ");
		...
		printf $fhoutLoc "> $candidate busy: %s\n",$tmpDate;
		$Lbusy=1;}
				# ------------------------------
				# machine is busy -> continue ..
	    else {
