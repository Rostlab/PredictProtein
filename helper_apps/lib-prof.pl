...
#-------------------------------------------------------------------------------
	...
	open($fhoutLoc,">".$par{"fileOutErrorConv"}) ||
	    do { Carp::cluck( "*** $SBR3 failed opening = $par{fileOutErrorConv}: $!" );
		 $fhoutLoc="STDOUT"; };
    }
    if ($#FILE_ERROR){
...
#bug	$fileOutRdb=~s/$par{"extProfOut"}.*$/$par{"extProfRdb"}/;
	# lkajan: escape . of extension '.prof':
	( my $eextProfOut = $par{extProfOut} ) =~ s/\\/\\\\/go;
	$eextProfOut =~ s/\./\\./go;
	$fileOutRdb=~s/^([^\.]+.*)$eextProfOut.*?$/$1$par{extProfRdb}/;
    }
...
#	    push(@filetmp,$fileOldRdb);
    ...
    open($fhoutLoc,">", $fileOutRdb) || do {
    	Carp::cluck( "failed to open >$fileOutRdb: $!" );
...
#-------------------------------------------------------------------------------
    ...
    #$cmd.=" exeHsspFilterPack=".$par{"exeFilterHsspPack"}; # lkajan: obsolete

    $cmd.=" exeConvertSeq=".    $par{"exeConvertSeqFor"};
    $cmd.=" exeCopf=".          $par{"exeCopf"};
    #$cmd.=" exeCopfPack=".      $par{"exeCopfPack"}; # lkajan: obsolete
...
#        return(&errSbr("not executable '$exe'!",$SBR3))   if (! -l $exe && ! -x $exe ); }
	...
	if (! $LisHtm){
		...
		foreach $kwdxx ("PRMN","PR2MN","PiTo"){
		    $prd{$itres,$kwdxx}=$prd{$itres,"PMN"};
		}}
...
#	$run{$itpar,"fileout"}=$fileout;
	...
	open($fhoutLoc,">".$fileout)||
	    do { Carp::cluck( "failed to open >$fileout: $!" );
	    	 print "-*- WARN $SBR4: failed opening fileout=$fileout!\n";
		 $fileout=0;}; 
	foreach $itres (1..$ctres){
	    $#tmp=0;
	    foreach $itout (1..$prd{"NUMOUT"}){
...
#-------------------------------------------------------------------------------
    ...
    if($fhoutLoc !~/^STD/o)
    {
	if( ! open($fhoutLoc,">".$fileinLoc) )
	{
		if( $par{debug} ){ Carp::cluck( "failed to open >$fileinLoc: $!" ); }
		return(&errSbr( __FILE__.":".__LINE__." failed to open new fileinLoc=$fileinLoc!",$SBR4));
	}
    }
...
#-------------------------------------------------------------------------------
	...
	$fileOutTmp.=$par{"extHssp"};
				# ------------------------------
				# run copf
	$cmd= $par{"exeCopf"}." ".$fileInLoc." hssp fileOut=$fileOutTmp $arg ".( $par{dirWork} ? qq|"dirWork=$par{dirWork}"| : '' );
	($Lok,$msg)=
...
#warn( ">$whichPROFloc<" );
    ...
    print "---     Output files";
    print " (generic output dir=",$par{"dirOut"},")" if (length($par{"dirOut"})>1);
    print ": \n";

    $#missing=$#format=$#kwdtmp=0;
				# ini
    foreach $it (1..$nfileInLoc){ $wrt[$it]="";}
				# ------------------------------
...
#warn( join('><', $tmp2, "doRet".$tmp2, $par{"doRet".$tmp2} ) );
	...
	foreach $it (1..$nfileInLoc){
	    $tmp=$fileOut{$it}{$kwd};
	    #$tmp=~s/^.*\///go; # lkajan: why do we remove directories? nah
	    if(length($tmp)>$maxlen){ $maxlen=length($tmp); }
	    $wrt[$it].=$tmp.",";}
	push(@format,$maxlen);
	push(@kwdtmp,$kwd);
    }
